...
class DualDeviceProtocol:
    ...
    def close(self):
        ...
        if getattr(self.p, "ledger", None) is not None and hasattr(self.p, "srw_row"):
            self.p.ledger.release(self.p.srw_row, lambda: 0)
...
def recover_transaction(directory: Path):
    ...
    result, commit = directory / "cap0r2_result.json", directory / "cap0r2_commit.json"
    valid = False
    ...
    for item in list(directory.glob('*.inprogress')) + [x for x in (result, commit, directory/'cap0r2_failure.json') if x.exists()]:
