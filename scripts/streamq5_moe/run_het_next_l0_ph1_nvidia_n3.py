#!/usr/bin/env python3
"""Closed standalone PH1 NVIDIA N3 compile/physical runner."""
...
import het_next_l0_ph1_nvidia_n3_common as common
from het_next_l0_ph1_nvidia_n3_backend import CompilerFailure, DriverBackend, DriverFailure, CUDA_SOURCE, OPTIONS, compile_one_program, file_sha
from het_next_l0_ph1_nvidia_n3_transaction import atomic_failure, canonical, clean_or_quarantine, publish_bundle, verify_bundle

ROOT = Path(__file__).resolve().parents[2]
REPORTS = ROOT / "reports/streamq5_moe"
LOCK = REPORTS / "het_next_l0_ph1_nvidia_n3_source_lock.json"
VERIFY = ROOT / "scripts/streamq5_moe/verify_het_next_l0_ph1_nvidia_n3.py"
COMPILE_OUT = REPORTS / "het_next_l0_ph1_nvidia_n3_compile"
PHYSICAL_OUT = REPORTS / "het_next_l0_ph1_nvidia_n3_physical"
COMPILE_FAILURE = REPORTS / "het_next_l0_ph1_nvidia_n3_compile_failures"
PHYSICAL_FAILURE = REPORTS / "het_next_l0_ph1_nvidia_n3_physical_failures"
QUARANTINE = REPORTS / "het_next_l0_ph1_nvidia_n3_quarantine"
COMPILE_KIND = "het_next_l0_ph1_nvidia_n3_compile"
PHYSICAL_KIND = "het_next_l0_ph1_nvidia_n3_physical"
ACK = {"compile": "ACK-PH1-NVIDIA-N3-COMPILE-ONE-PROGRAM", "physical": "ACK-PH1-NVIDIA-N3-PHYSICAL-ONE-ATTEMPT"}


def validate_authorization(phase: str, acknowledgement: str):
    lock = json.loads(LOCK.read_text(encoding="utf-8"))
    if lock.get("kind") != "het_next_l0_ph1_nvidia_n3_source_lock" or lock.get("revision") != "N3":
...
def disassemble(cubin: bytes, lock):
    ...
    outputs = {}
    with tempfile.TemporaryDirectory(prefix="ph1_nvidia_n3_cubin_") as directory:
