...
from s100_phase32_common import (
    ARMS,
    RESULTS,
    compile_audit,
    make_candidate,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Phase32 Phase31-rebased H8 measure")
    parser.add_argument("--arm", choices=("parent", *ARMS, "compile"), required=True)
    ...
    out = RESULTS / f"S100_PHASE32_{args.tag.upper()}_{args.arm.upper()}_CTX{args.context}.json"
    payload = {
        "kind": "s100_phase32_measure",
        "status": "started",
        "arm": args.arm,
        "tag": args.tag,
        ...
        "started_utc": utc_now(),
        "claim_boundary": (
            "fresh-process exact target-only H8 timing; no drafter, acceptance "
            "loss, rejection or rollback cost"
        ),
    }
    runtime = None
    try:
        if args.arm == "compile":
            ...
            if args.arm == "parent":
                ...
                config = {"parent": "phase31_attention_head_m4_x2"}
            else:
                ...
                config = {
                    "arm": args.arm,
                    "dense_mode": graph.dense_mode,
                    "moe": "phase25_direct8_route_phase32_dense_shared",
                }
            after = telemetry()
            payload.update(
                {
