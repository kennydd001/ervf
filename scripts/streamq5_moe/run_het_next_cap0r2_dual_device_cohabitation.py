...
RUN_DIR = ROOT / "reports/runs/streamq5_moe/het_next_cap0r2_dual_device_cohabitation"
RESULT = RUN_DIR / "cap0r2_result.json"
COMMIT = RUN_DIR / "cap0r2_commit.json"
FAILURE = RUN_DIR / "cap0r2_failure.json"
LOCK = REPORTS / "het_next_cap0r2_runner_lock.json"
VERIFIER_LOCK = REPORTS / "het_next_cap0r2_verifier_lock.json"
PREREG = REPORTS / "HET_NEXT_CAP0R2_DUAL_DEVICE_COHABITATION_PREREGISTRATION_2026-08-13.md"
DESIGN = REPORTS / "HET_NEXT_CAP0R2_STATIC_PREFLIGHT_DESIGN_2026-08-13.md"
PROTOCOL_PATH = SCRIPTS / "het_next_cap0r2_protocol.py"
KERNEL_PATH = SCRIPTS / "het_next_cap0r2_kernels.py"
VERIFIER = SCRIPTS / "verify_het_next_cap0r2_dual_device_cohabitation.py"
PREFLIGHT = SCRIPTS / "preflight_het_next_cap0r2_static.py"
...
def intel_backend(ledger):
    from ctypes import wintypes
    from het_next_cap0r2_kernels import INTEL_BUILD_OPTIONS, INTEL_SOURCE
...
def nvidia_backend(ledger):
    from het_next_cap0r2_kernels import NVIDIA_NVRTC_OPTIONS, NVIDIA_SOURCE
    ...
    uuid_bytes=(C.c_ubyte*16)();ck(cuDeviceGetUuid(uuid_bytes,dev),'cuDeviceGetUuid');total=C.c_size_t();ck(cuDeviceTotalMem(C.byref(total),dev),'cuDeviceTotalMem');ctx=C.c_void_p();ck(cuCtxCreate(C.byref(ctx),0,dev),'cuCtxCreate');cleanup.append((ledger.create('nvidia_context','nvidia'),lambda:cuCtxDestroy(ctx)))
    ...
    try:
        ck(nvrtcCreateProgram(C.byref(program),NVIDIA_SOURCE.encode(),b'cap0r2.cu',0,None,None),'nvrtcCreateProgram');cleanup.append((ledger.create('nvidia_nvrtc_program','nvidia'),lambda:nvrtcDestroyProgram(C.byref(program))))
        ...
        ck(cuModuleLoadDataEx(C.byref(module),C.cast(pb,C.c_void_p),0,None,None),'cuModuleLoadDataEx');cleanup.append((ledger.create('nvidia_module','nvidia'),lambda:cuModuleUnload(module)));ck(cuModuleGetFunction(C.byref(kernel),module,b'cap0_nvidia_bijection'),'cuModuleGetFunction');ck(cuStreamCreate(C.byref(stream),1),'cuStreamCreate');cleanup.append((ledger.create('nvidia_stream','nvidia'),lambda:cuStreamDestroy(stream)));ck(cuMemHostAlloc(C.byref(host),BUFFER_BYTES,0),'cuMemHostAlloc');cleanup.append((ledger.create('nvidia_pinned_host','nvidia',{'bytes':BUFFER_BYTES}),lambda:cuMemFreeHost(host)));ck(cuMemAlloc(C.byref(dptr),BUFFER_BYTES),'cuMemAlloc');cleanup.append((ledger.create('nvidia_device_memory','nvidia',{'bytes':BUFFER_BYTES}),lambda:cuMemFree(dptr.value)))
        ...
        raise
    def run_once(words):
        raw=struct.pack('<1024I',*words);C.memmove(host,raw,BUFFER_BYTES);start_event,end_event=C.c_void_p(),C.c_void_p();ck(cuEventCreate(C.byref(start_event),0),'cuEventCreate_start');ck(cuEventCreate(C.byref(end_event),0),'cuEventCreate_end');erows=[(ledger.create('nvidia_start_event','nvidia'),lambda:cuEventDestroy(start_event)),(ledger.create('nvidia_end_event','nvidia'),lambda:cuEventDestroy(end_event))]
    ...
    try:
        ...
        if timer:
            try: k.CancelWaitableTimer(timer)
                ...
                if timer_row: ledger.release(timer_row, lambda: 0 if k.CloseHandle(timer) else C.get_last_error() or 1)
...
def run_capability(ack):
    ...
    from het_next_cap0r2_protocol import DualDeviceProtocol, Win32Primitives, atomic_create_json
...
def main():
    ...
    try:
        ...
        from het_next_cap0r2_protocol import atomic_create_json, recover_transaction
