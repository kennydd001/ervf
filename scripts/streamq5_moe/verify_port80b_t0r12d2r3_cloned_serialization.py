...
spec=importlib.util.spec_from_file_location('d2verify_r3',SRC)
v=importlib.util.module_from_spec(spec);sys.modules['d2verify_r3']=v;spec.loader.exec_module(v)
R=ROOT/'reports/streamq5_moe'
RUN=ROOT/'scripts/streamq5_moe/run_port80b_t0r12d2r3_cloned_serialization.py'
...
SERFAIL=R/'port80b_t0r12d2r2_shared_storage_serialization_failure.json'
...
def provenance():
 ...
 return {'bindings':all(lock.get(k)==z for k,z in actual.items()),'verifier_bound':vl.get('verifier_sha256')==actual['verifier_sha256']}

def preflight():
 configure();p=provenance();checks={'bindings':p['bindings'],'verifier_bound':p['verifier_bound'],'outputs_absent':not OUT.exists()}
 return {'kind':'d2r3_verifier_preflight','pass':all(checks.values()),'checks':checks}

def verify():
 configure();base=v.verify();r=json.loads((OUT/'t0r12d2_result.json').read_text());p=provenance()
 extra={'base_verifier_valid':base['valid_diagnostic'],'bindings':p['bindings'],'verifier_bound':p['verifier_bound'],'revision':r.get('revision')=='t0r12d2r3_cloned_serialization','repair_only':r.get('serialization_repair_only') is True,'d2_source':r.get('d2_source_sha256')==v.sha(D2RUN),'serialization_failure':r.get('serialization_failure_sha256')==v.sha(SERFAIL)}
 return {'kind':'d2r3_independent_verification','pass':False,'valid_diagnostic':all(extra.values()),'checks':extra,'base_checks':base['checks']}
