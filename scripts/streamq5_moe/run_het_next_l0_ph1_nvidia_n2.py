#!/usr/bin/env python3
"""Closed standalone PH1 NVIDIA N2 compile/physical runner."""
...
import het_next_l0_ph1_nvidia_n2_common as common
from het_next_l0_ph1_nvidia_n2_backend import CompilerFailure, DriverBackend, DriverFailure, CUDA_SOURCE, OPTIONS, compile_one_program, file_sha
from het_next_l0_ph1_nvidia_n2_transaction import atomic_failure, canonical, clean_or_quarantine, publish_bundle, verify_bundle

ROOT = Path(__file__).resolve().parents[2]
REPORTS = ROOT / "reports/streamq5_moe"
LOCK = REPORTS / "het_next_l0_ph1_nvidia_n2_source_lock.json"
VERIFY = ROOT / "scripts/streamq5_moe/verify_het_next_l0_ph1_nvidia_n2.py"
COMPILE_OUT = REPORTS / "het_next_l0_ph1_nvidia_n2_compile"
PHYSICAL_OUT = REPORTS / "het_next_l0_ph1_nvidia_n2_physical"
COMPILE_FAILURE = REPORTS / "het_next_l0_ph1_nvidia_n2_compile_failures"
PHYSICAL_FAILURE = REPORTS / "het_next_l0_ph1_nvidia_n2_physical_failures"
QUARANTINE = REPORTS / "het_next_l0_ph1_nvidia_n2_quarantine"
COMPILE_KIND = "het_next_l0_ph1_nvidia_n2_compile"
PHYSICAL_KIND = "het_next_l0_ph1_nvidia_n2_physical"
ACK = {"compile": "ACK-PH1-NVIDIA-N2-COMPILE-ONE-PROGRAM", "physical": "ACK-PH1-NVIDIA-N2-PHYSICAL-ONE-ATTEMPT"}


def validate_authorization(phase: str, acknowledgement: str):
    lock = json.loads(LOCK.read_text(encoding="utf-8"))
    if lock.get("kind") != "het_next_l0_ph1_nvidia_n2_source_lock" or lock.get("revision") != "N2":
...
def disassemble(cubin: bytes, lock):
    ...
    outputs = {}
    with tempfile.TemporaryDirectory(prefix="ph1_nvidia_n2_cubin_") as directory:
...
def main():
    ...
    try:
        ...
        root = COMPILE_FAILURE if args.phase == "compile" else PHYSICAL_FAILURE
        kind = COMPILE_KIND if args.phase == "compile" else PHYSICAL_KIND
        path = atomic_failure(root, {"kind": kind + "_failure", "stage": "authorization_or_start",
                              "error_type": type(exc).__name__, "error": str(exc), "device_opened": False,
                              "compiler_opened": False, "traceback": traceback.format_exc(limit=20),
                              "disposition": "bounded_create_new_predevice"})
        print(json.dumps({"status": "invalid_infrastructure_failure", "failure": str(path)}))
        return 3
