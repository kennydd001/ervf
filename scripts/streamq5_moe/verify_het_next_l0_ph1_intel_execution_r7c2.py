#!/usr/bin/env python3
"""Independent R7C2 authorization verifier; numerical replay remains frozen R7A."""
...
VERIFY = R / "het_next_l0_ph1_intel_execution_r7c2_independent_verification.json"
...
R7C1_AUDIT_SHA = "e75ae1897d4ce73664c1225ca499e41a029660f0506b2fc72ee4cc65ddfadeb2"
...
CHAIN = {
    "runner_sha256": S / "run_het_next_l0_ph1_intel_execution_r7c2.py",
    "verifier_sha256": Path(__file__),
    "preflight_sha256": S / "preflight_het_next_l0_ph1_intel_execution_r7c2.py",
    "prereg_sha256": R / "HET_NEXT_L0_PH1_INTEL_EXECUTION_R7C2_PREREGISTRATION_2026-08-14.md",
...
def auth_result_valid(result: dict) -> bool:
    ...
    return set(result) == {"ack", "checks", "kind", "no_payload_compiler_device", "pass", "passed", "r7p_result_sha256", "total"} and result.get("kind") == "ph1_intel_execution_r7a_authorization_preflight" and result.get("pass") is True and result.get("passed") == result.get("total") == 7 and result.get("no_payload_compiler_device") is True and result.get("ack") == R7A_ACK and result.get("r7p_result_sha256") == R7P_SHA and isinstance(checks, dict) and set(checks) == AUTH_CHECKS and all(value is True for value in checks.values())


def extension_valid(extension: dict, lock: dict, observed: dict, auth_result: dict) -> bool:
    return set(lock) == {"kind", "execution_open", "audit_token", "physical_output", "physical_verifier", *observed.keys()} and lock.get("kind") == "ph1_intel_execution_r7c2_lock" and lock.get("execution_open") is True and lock.get("audit_token") == ACK and lock.get("physical_output") == "het_next_l0_ph1_intel_execution_r7a" and lock.get("physical_verifier") == "verify_het_next_l0_ph1_intel_execution_r7c2.py" and all(lock.get(name) == digest for name, digest in observed.items()) and set(extension) == {"lock_sha256", "observed", "authorization_result_sha256", "authorization_result", "audit_token", "outer_failure_stages"} and extension.get("lock_sha256") == sha256(LOCK) and extension.get("observed") == observed and extension.get("authorization_result_sha256") == AUTH_RESULT_SHA and extension.get("authorization_result") == auth_result and extension.get("audit_token") == ACK and extension.get("outer_failure_stages") == OUTER_STAGES and observed.get("authorization_result_sha256") == AUTH_RESULT_SHA and observed.get("r7c1_audit_sha256") == R7C1_AUDIT_SHA and observed.get("r7p_result_sha256") == R7P_SHA and auth_result_valid(auth_result)


def main() -> int:
    result_path, manifest_path, commit_path = (OUT / name for name in ("result.json", "manifest.json", "commit.json")); result_bytes = result_path.read_bytes(); result = json.loads(result_bytes)
    lock = json.loads(LOCK.read_text()); auth_result = json.loads(AUTH_RESULT.read_text()); observed = {name: sha256(path) for name, path in CHAIN.items()}; extension = result.get("authorization", {}).get("r7c2_authorization", {})
    extension_pass = sha256(AUTH_RESULT) == AUTH_RESULT_SHA and extension_valid(extension, lock, observed, auth_result); checks = {"r7c2_authorization_extension": extension_pass}
    if extension_pass:
    ...
    output = {"kind": "ph1_intel_execution_r7c2_independent_verification", "checks": checks, "pass": all(checks.values()), "passed": sum(value is True for value in checks.values()), "total": len(checks), "claim": "one real expert/input Intel correctness component only"}
