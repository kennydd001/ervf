...
def linear(w,x):
 ...
 out[row]=rb(lanes[0])
 return out
def static_contract_fixture(f):
 """Payload-free gate used by preflight mutation tests; main verification is stricter."""
 return f=={'controls':22,'allocations':14,'arguments':18,'launches':4,'releases':21,'reads':9,'finishes':1,'forbidden_zero':True,'identity':'0000:00:02.0','start_ram_min':16*2**30,'reserve_min':2*2**30,'peak_max':12*2**30,'artifact_max':16*2**20}
def mutation_fixture_result():
 controls=[{'pass':True,'observed':'reject','predevice_counts':{'opencl_load':0,'context':0,'program':0,'kernel':0,'allocation':0,'launch':0}} for _ in range(22)]
 return {'kind':'ph1_intel_execution_r2_preflight_fixture','positive':True,'status':'intel_execution_positive','identity':{'name':'Intel(R) Arc(TM) Pro 140T GPU (32GB)','vendor':'Intel(R) Corporation','driver':'32.0.101.8517','pci':'0000:00:02.0'},'controls':controls,'outputs':{'gate':'aa','up':'bb','silu':'cc','activation':'dd','down':'ee'},'counters':{'gate':1,'up':1,'activation':1,'down':1},'ledger':{'allocations':14,'arguments':18,'launches':4,'reads':9,'finishes':1,'releases':21,'order':'exact','cleanup_complete':True,'forbidden_rows':0},'provenance':{'authorized':True,'packages_exact':True},'resource':{'start':16*2**30,'reserve':2*2**30,'peak':12*2**30,'artifact':16*2**20}}
def verify_mutation_fixture(r):
 e=mutation_fixture_result()
 return {'schema':r.get('kind')==e['kind'] and r.get('positive') is True and r.get('status')==e['status'],'identity':r.get('identity')==e['identity'],'controls':r.get('controls')==e['controls'],'outputs':r.get('outputs')==e['outputs'],'counters':r.get('counters')==e['counters'],'ledger':r.get('ledger')==e['ledger'],'cleanup':r.get('ledger',{}).get('cleanup_complete') is True and r.get('ledger',{}).get('releases')==21,'forbidden':r.get('ledger',{}).get('forbidden_rows')==0,'provenance':r.get('provenance')==e['provenance'],'resources':r.get('resource')==e['resource']}
def verify_dict(rrr):
 if rrr.get('kind')=='ph1_intel_execution_r2_preflight_fixture':return verify_mutation_fixture(rrr)
 ev=rrr['evidence'];outs={k:bytes.fromhex(v) for k,v in ev['outputs'].items()};inp=rr(D2,155138788,4096);iw=np.frombuffer(inp,'<u2');records={};weights={}
 for s in SPECS:records[s[0]],weights[s[0]]=codec(rr(SHARD,s[3][0],s[3][1]-s[3][0]),s)
 lut=(CPU/'bf16_silu_lut.bin').read_bytes();qg=linear(weights['gate'],iw);qu=linear(weights['up'],iw);qs=np.frombuffer(lut,'<u2')[qg];qa=np.asarray([mul(int(a),int(b)) for a,b in zip(qs,qu,strict=True)],np.uint16);qd=linear(weights['down'],qa);expected={'gate':qg.astype('<u2').tobytes(),'up':qu.astype('<u2').tobytes(),'silu':qs.astype('<u2').tobytes(),'activation':qa.astype('<u2').tobytes(),'down':qd.astype('<u2').tobytes()};led=ev['ledger'];alloc=[x for x in led if x.get('op')=='host_usm_allocate'];writes=[x for x in led if x.get('op')=='cpu_direct_write'];inits=[x for x in led if x.get('op')=='initialize'];args=[x for x in led if x.get('op')=='set_pointer_arg'];launch=[x for x in led if x.get('op')=='enqueue'];release=[x for x in led if x.get('op')=='release'];readrows=[x for x in led if x.get('op')=='cpu_direct_read'];samples=[x for x in led if x.get('op')=='resource_sample'];expected_args=[(k,i,n) for k,ns in ARGS for i,n in enumerate(ns)];expected_release=['usm:'+x[0] for x in reversed(BUFF)]+['kernel:down_linear','kernel:activation','kernel:up_linear','kernel:gate_linear','program','queue','context'];expected_ops=['identity','context_create','queue_create','program_create_binary']+['kernel_create']*4+['host_usm_allocate']*14+['cpu_direct_write']*5+['initialize']*9+['set_pointer_arg']*18+['enqueue']*4+['finish']+['cpu_direct_read']*9+['release']*21+['cleanup'];controls=rebuild_controls(records,lut);lock=json.loads(LOCK.read_text());auth=rrr['authorization'];resource=rrr['resource'];expected_writes=[('gate_record',SPECS[0][7]),('up_record',SPECS[1][7]),('down_record',SPECS[2][7]),('natural_input','5ce66a20ed658860ab4e98499e76205775cf0dd32cef15f35723dd83fc13fd3f'),('silu_lut','a3cbc779f1f1e8b0957c651e6b90a64d506568764ab34f7419ba5cc1ede9daed')];cleanup=next(x for x in led if x.get('op')=='cleanup')
 checks={'positive_schema':rrr['positive'] is True and rrr['status']=='intel_execution_positive' and rrr['claim']=='one real expert/input Intel correctness component only','oracle_outputs':set(outs)==set(expected)|{'gate_counters','up_counters','activation_counters','down_counters'} and all(outs[k]==v and sha(v)==STAGE[k] for k,v in expected.items()),'records_input_lut':all(sha(records[s[0]])==s[7] for s in SPECS) and sha(inp)=='5ce66a20ed658860ab4e98499e76205775cf0dd32cef15f35723dd83fc13fd3f' and sha(lut)=='a3cbc779f1f1e8b0957c651e6b90a64d506568764ab34f7419ba5cc1ede9daed','controls':rrr['controls']==controls,'authorization':auth['lock_sha256']==fsha(LOCK) and all(lock.get(k)==v for k,v in auth['observed'].items()),'compile_package':fsha(COMPILE/'intel_source.cl')=='f1b3ccdae6d202ed210810e3cd419f726ea89ffa8fba0c84df5c2bfca3a84d21' and fsha(COMPILE/'intel_program.bin')=='8b57db279fbb1d7d8df17ebab5cfb54203ef8da8cc31df2d136650820548f629' and (COMPILE/'intel_program.bin').stat().st_size==186352,'identity':ev['identity']==next(x['identity'] for x in led if x.get('op')=='identity') and ev['identity']['name']=='Intel(R) Arc(TM) Pro 140T GPU (32GB)' and ev['identity']['vendor']=='Intel(R) Corporation' and ev['identity']['driver']=='32.0.101.8517' and ev['identity']['pci']=='0000:00:02.0','ledger_order':[x.get('op') for x in led if x.get('op')!='resource_sample']==expected_ops,'allocations':[(x['name'],x['bytes']) for x in alloc]==list(BUFF) and all(x['type']==0x4197 and x['base']==x['pointer'] and x['queried_size']==x['bytes'] and x['pointer']%4096==0 for x in alloc),'writes':[(x['name'],x['sha256']) for x in writes]==expected_writes,'initialization':[(x['name'],x['value']) for x in inits]==[(x,'ff') for x in ('gate','up','silu','activation','down')]+[(x,'00') for x in ('gate_counters','up_counters','activation_counters','down_counters')],'args':[(x['kernel'],x['index'],x['name']) for x in args]==expected_args and all(x['pointer']==next(a['pointer'] for a in alloc if a['name']==x['name']) for x in args),'launch_finish_read':[(x['kernel'],x['global'],x['local']) for x in launch]==LAUNCH and sum(x.get('op')=='finish' for x in led)==1 and [x['name'] for x in readrows]==['gate','up','silu','activation','down','gate_counters','up_counters','activation_counters','down_counters'] and all(x['after_finish'] is True and x['bytes']==dict(BUFF)[x['name']] and x['sha256']==sha(outs[x['name']]) for x in readrows),'release':[x['name'] for x in release]==expected_release and [x['attempt_index'] for x in release]==list(range(21)) and all(x['attempted'] is True and x['code']==0 and x['exception'] is None and x['owned_after'] is False for x in release) and cleanup['cleanup_complete'] is True and cleanup['release_attempts']==21 and cleanup['live_owned_resources']==0 and cleanup['live_resource_names']==[],'extensions':ev['extension_counts']=={'clHostMemAllocINTEL':14,'clMemFreeINTEL':14,'clSetKernelArgMemPointerINTEL':18,'clGetMemAllocInfoINTEL':42},'counters':all(all(v==1 for v in struct.unpack('<'+'I'*(len(outs[k])//4),outs[k])) for k in ('gate_counters','up_counters','activation_counters','down_counters')),'resources':resource['sampling_claim']=='retained phase samples, not continuous peak monitor' and resource['start_available']>=16*2**30 and min(resource['post_payload_available'],resource['final_available'],resource['post_serialize_available'],*(x['available'] for x in samples))>=2*2**30 and resource['peak_retained_wset']<=12*2**30 and len(samples)==12,'forbidden':set(ev['forbidden_calls'])=={'clCreateBuffer','clEnqueueReadBuffer','clEnqueueWriteBuffer','clEnqueueCopyBuffer','clEnqueueMigrateMemObjects','clEnqueueMemAdviseINTEL'} and all(v==0 for v in ev['forbidden_calls'].values()) and all(x.get('api') not in ev['forbidden_calls'] for x in led),'runner_gates':all(rrr['gates'].values()) and rrr['stage_hashes']==STAGE}
 return checks
def main():
 r,m,c=(OUT/n for n in ('result.json','manifest.json','commit.json'));rrr=json.loads(r.read_text());checks=verify_dict(rrr);mm=json.loads(m.read_text());checks['bundle']=json.loads(c.read_text())=={'kind':'ph1_intel_execution_r2_commit','manifest_sha256':fsha(m),'result_sha256':fsha(r)} and mm=={'kind':'ph1_intel_execution_r2_manifest','files':[{'name':'result.json','bytes':r.stat().st_size,'sha256':fsha(r)}]} and {x.name for x in OUT.iterdir()}=={'result.json','manifest.json','commit.json'} and sum(x.stat().st_size for x in OUT.iterdir())<=16*2**20;out={'kind':'ph1_intel_execution_r2_independent_verification','checks':checks,'pass':all(checks.values()),'passed':sum(checks.values()),'total':len(checks),'claim':'one real expert/input Intel correctness component only'}
