...
PREREG = ROOT / "reports/streamq5_moe/P3A_INTEGRATED_EXPERT_DATAPLANE_PREREGISTRATION.md"
P2C_VERIFY = ROOT / "reports/streamq5_moe/p2c_physical_h2d_verification.json"
ARTIFACT = ROOT / "reports/runs/streamq5_moe/p3a_fresh_route_input_ids.safetensors"
OUTPUT = ROOT / "reports/streamq5_moe/p3a_route_input_lock.json"
PRIOR_INPUTS = (
    ...
    ROOT / "reports/runs/streamq5_moe/p2b_fresh_route_input_ids.safetensors",
)
...
if __name__ == "__main__":
    if OUTPUT.exists() or ARTIFACT.exists():
        raise FileExistsError("refusing to overwrite P3A route input lock")
    p2c = json.loads(P2C_VERIFY.read_text(encoding="utf-8"))
    if p2c.get("status") != "p2c_physical_h2d_verification_pass":
        raise RuntimeError("independent STREAMQ5 P2C pass required")
    ...
    if set(tensors) != set(DOMAINS) or any(tuple(value.shape) != (1, TOKENS) for value in tensors.values()):
        raise RuntimeError("P3A route input contract failed")
    ...
    if not disjoint:
        raise RuntimeError("P3A route context overlaps a prior decision context")

    ARTIFACT.parent.mkdir(parents=True, exist_ok=True)
    save_file(tensors, ARTIFACT, metadata={"kind": "streamq5_moe_p3a_fresh_route_inputs", "routes_opened": "false"})
    payload = {
        "kind": "streamq5_moe_p3a_route_input_lock", "locked_utc": datetime.now(timezone.utc).isoformat(),
        "preregistration_sha256": sha256(PREREG), "p2c_verification_sha256": sha256(P2C_VERIFY),
        ...
        "routes_opened": False,
        "cache": {"total_slots": 1640, "static_slots_per_layer": 20, "dynamic_slots_layers_0_7": 15, "dynamic_slots_layers_8_47": 14, "expert_record_bytes": 3035136},
        "device": {"observed_free_bytes": 7385120768, "cache_bytes": 4977623040, "int8_trunk_bytes": 1541093376, "kv_bytes": 402653184, "minimum_free_scratch_bytes": 402653184},
        "gates": {"mean_integrated_expert_wall_ms_max": 60.0, "p95_integrated_expert_wall_ms_max": 75.0},
    }
