...
def verify_bundle_contract(result_bytes,manifest,commit,file_names,total_bytes):
 try:
  ...
  return rr.get('kind')=='ph1_intel_execution_r6' and manifest=={'kind':'ph1_intel_execution_r6_manifest','files':[row]} and commit=={'kind':'ph1_intel_execution_r6_commit','manifest_sha256':sha(mb),'result_sha256':sha(result_bytes)} and file_names=={'result.json','manifest.json','commit.json'} and total_bytes<=16*2**20
 except Exception:return False
def main():
 r,m,c=(OUT/n for n in ('result.json','manifest.json','commit.json'));rb=r.read_bytes();rrr=json.loads(rb);checks=verify_dict(rrr);mm=json.loads(m.read_text());checks['bundle']=verify_bundle_contract(rb,mm,json.loads(c.read_text()),{x.name for x in OUT.iterdir()},sum(x.stat().st_size for x in OUT.iterdir()));out={'kind':'ph1_intel_execution_r6_independent_verification','checks':checks,'pass':all(checks.values()),'passed':sum(checks.values()),'total':len(checks),'claim':'one real expert/input Intel correctness component only'}
