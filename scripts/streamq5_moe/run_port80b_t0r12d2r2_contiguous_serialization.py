...
spec=importlib.util.spec_from_file_location('d2base',SRC)
d=importlib.util.module_from_spec(spec);sys.modules['d2base']=d;spec.loader.exec_module(d)
R=ROOT/'reports/streamq5_moe'
LOCK=R/'port80b_t0r12d2r2_runner_lock.json'
VER=ROOT/'scripts/streamq5_moe/verify_port80b_t0r12d2r2_contiguous_serialization.py'
VL=R/'port80b_t0r12d2r2_verifier_lock.json'
PR=R/'PORT80B_T0R12D2R2_CONTIGUOUS_SERIALIZATION_REPAIR_2026-08-13.md'
FAIL=R/'port80b_t0r12d2_postcompute_serialization_failure.json'
OUT=ROOT/'reports/runs/streamq5_moe/port80b_t0r12d2r2_contiguous_serialization'
ACK='T0R12D2R2_CONTIGUOUS_SERIALIZATION_ONLY'
...
def run():
 if OUT.exists():raise FileExistsError('R2 output exists')
 if not lockcheck()['pass']:raise RuntimeError('R2 lock')
 old_save=d.save_file
 def contiguous_save(tensors,path):
  return old_save({k:v.contiguous() for k,v in tensors.items()},path)
 d.D=OUT
 d.save_file=contiguous_save
 result=d.run()
 rp=OUT/'t0r12d2_raw.safetensors';jp=OUT/'t0r12d2_result.json'
 result['kind']='port80b_t0r12d2_full_stage_diagnostic'
 result['revision']='t0r12d2r2_contiguous_serialization'
...
def main():
 ...
 if a.phase=='lockcheck':print(json.dumps({'kind':'d2r2_lockcheck',**lockcheck(),'physical_actions':{'model':False,'forward':False,'gpu':False}}));return 0
