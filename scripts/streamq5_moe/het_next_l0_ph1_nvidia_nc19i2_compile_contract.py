#!/usr/bin/env python3
"""NC19I2 import-inert, stdlib-only compile contract.
...
PROGRAM_NAME = b"het_next_l0_ph1_nvidia_nc19i2.cu"
CAPS = {
...
def compile_topology_descriptor(revision: str, prefix: str) -> dict:
    base=f"reports/streamq5_moe/{prefix}"
    paths=[
        base+"_compile_only",base+"_compile_only_incidental_failures",
        base+"_compile_only_negative",base+"_compile_only_quarantine",
        base+"_durability_adjudication",base+"_independent_verification",
        base+"_independent_verification_failures",base+"_independent_verification_negative",
        base+"_independent_verification_quarantine",base+"_static_preflight_failures",
        base+"_static_preflight_quarantine",base+"_static_preflight_result.json",
    ]
    roots=[]
    for path in sorted(paths):
        is_file=path.endswith(".json")
        if path==base+"_compile_only": role="terminal_positive"
        elif path==base+"_compile_only_negative": role="terminal_negative"
        else: role="result" if is_file else "failure_history"
        roots.append({"id":"live_"+sha256(path.encode())[:16],"path":path,"phase":"lifecycle",
                      "role":role,"allowed_node_type":"file" if is_file else "directory",
                      "required_schema_keyset":[],"required_schema_kind":None,"required_schema_status":None,
                      "cap_bytes":CAPS["bundle"],"multiplicity":{"min":0,"max":1},"immutable":True,
                      "recoverable":False,"disposition":"reject_existing","identity_policy":"none",
                      "binding_key":None,"stage_policy":{"design":"must_be_absent","implementation_freeze":"must_be_absent","runtime":"validate_declared_identity_policy"},
                      "expected_bytes":None,"expected_sha256":None,"required_schema_spec":{}})
    pattern={"id":"live_inprogress","regex":rf"^{re.escape(base)}_[a-z0-9_]+\.inprogress\.[0-9]+\.[0-9a-f]{{16}}$",
             "glob":base+"_*.inprogress.*","phase":"lifecycle","role":"inprogress","cap_bytes":CAPS["bundle"],
             "multiplicity":{"min":0,"max":1},"immutable":False,"recoverable":True,"disposition":"quarantine_then_retry"}
    preflight=base+"_static_preflight_result.json"
    no_terminal=sorted(set(paths)-{preflight})
    terminal_choices={root["id"]:sorted(set(no_terminal)-{root["path"]}) for root in roots if root["role"] in {"terminal_positive","terminal_negative"}}
    return {"revision":revision,"prefix":[prefix],"roots":roots,"patterns":[pattern],"caps":dict(CAPS),
            "required_present_by_stage":{"design":[],"implementation_freeze":[],"runtime":[preflight]},
            "expected_absent_by_stage":{"design":{"paths":sorted(paths)},"implementation_freeze":{"paths":sorted(paths)},
                "runtime":{"no_terminal":no_terminal,"terminal_choices":terminal_choices}}}
...
def observed_tree_digest(rows: list[dict], fields=OBSERVED_FIELD_ORDER) -> str:
    if tuple(fields) != OBSERVED_FIELD_ORDER:
        raise ValueError("observed_field_order")
    ordered=[{key:row[key] for key in fields} for row in rows]
    return sha256(json.dumps(ordered,separators=(",",":"),ensure_ascii=False,allow_nan=False).encode("utf-8"))
...
def validate_root_unit(descriptor: dict, row: dict, stage: str) -> dict:
    roots={item["path"]:item for item in descriptor["roots"]}
    if not isinstance(row,dict) or set(row)!=OBSERVED_FIELDS or row.get("path") not in roots:
        return {"classification":"invalid_schema","valid":False,"terminal":False,"recoverable":False,"disposition":"reject","dispositions":[]}
    spec=roots[row["path"]]
    if spec["stage_policy"].get(stage)=="must_be_absent":
        return {"classification":"stage_policy_violation","valid":False,"terminal":False,"recoverable":False,"disposition":"reject","dispositions":[]}
    if row["node_type"]!=spec["allowed_node_type"] or type(row["size"]) is not int or not(0<=row["size"]<=spec["cap_bytes"]):
        return {"classification":"invalid_schema","valid":False,"terminal":False,"recoverable":False,"disposition":"reject","dispositions":[]}
    raw,parsed,error=_decode_observed_file(row)
    if error or (spec["required_schema_spec"] and (row["parse_status"]!="valid_json" or not _schema_ok(spec["required_schema_spec"],parsed))):
        return {"classification":"invalid_schema","valid":False,"terminal":False,"recoverable":False,"disposition":"reject","dispositions":[]}
    if spec["identity_policy"]=="bound_size_sha256" and (raw is None or row["size"]!=spec["expected_bytes"] or row["sha256_or_null"]!=spec["expected_sha256"]):
        return {"classification":"invalid_schema","valid":False,"terminal":False,"recoverable":False,"disposition":"reject","dispositions":[]}
    return {"classification":"root_file_valid" if row["node_type"]=="file" else "root_directory_valid",
            "valid":True,"terminal":False,"recoverable":False,"disposition":"accept_present","dispositions":[]}
...
def classify_topology(descriptor: dict, observed_entries: list[dict], stage: str,
    ...
    for row in observed_entries:
        ...
        if path not in universe:
            ...
            if len(matches)==1 and row["size"]>matches[0]["cap_bytes"]:
                return {"classification":"oversize","valid":False,"terminal":False,"recoverable":False,"disposition":"reject","dispositions":[]}
            if len(matches)>1:
                return {"classification":"collision","valid":False,"terminal":False,"recoverable":False,"disposition":"reject","dispositions":[]}
            if path.startswith(("/","\\")) or ".." in Path(path).parts:
    ...
    if stage == "design":
        ...
        classification, terminal, disposition = "implementation_freeze_valid", False, "freeze_identity"
    else:
        selected_root=root_by_path[next(iter(selected))] if selected else None
        role_class={"terminal_positive":"compile_positive","terminal_negative":"compile_valid_negative"}
        classification = role_class.get(selected_root["role"],selected_root["disposition"]) if selected_root else "fresh"
        terminal = bool(selected)
        disposition = selected_root["disposition"] if selected_root else "proceed"
...
def evaluate_fixture_case(case: dict, fixture: dict) -> dict:
    ...
    if case["observed_total_bytes"] != sum(row["size"] for row in rows):
        return {"classification": "tree_bytes", "valid": False, "terminal": False,
                "recoverable": False, "disposition": "reject", "dispositions": []}
    declared_digest=observed_tree_digest(rows,fixture["observed_entry_fields"])
    if case["input_integrity_policy"]=="nominal_raw_schema_exact" and case["observed_tree_digest"] != declared_digest:
        return {"classification": "tree_digest", "valid": False, "terminal": False,
                "recoverable": False, "disposition": "reject", "dispositions": []}
    descriptor = descriptors[case["descriptor"]]
    mutation=case["mutation"]
    if mutation in {"typed_valid","typed_wrong_type","typed_wrong_value","typed_missing","typed_extra"}:
        if len(rows)!=1:
            return {"classification":"invalid_schema","valid":False,"terminal":False,"recoverable":False,"disposition":"reject","dispositions":[]}
        return validate_root_unit(descriptor,rows[0],case["evaluation_stage"])
    if mutation in {"root_file","root_directory"}:
        return validate_root_unit(descriptor,rows[0],case["evaluation_stage"])
    if case["mutation"] == "descriptor_exact":
        ...
        return {"classification":"historical_lockset_equal" if valid else "invalid_descriptor",
                ...
                "disposition":"accept_descriptor" if valid else "reject","dispositions":[]}
    if case["mutation"].startswith("descriptor_"):
        ...
        try:
            ...
            paths=list(paths_for_revision(mutated))
            expected=next(row for row in fixture["historical_lock_expectations"] if row["revision"]==revision)
            digest=sha256(json.dumps(paths,separators=(",",":"),ensure_ascii=False).encode("utf-8"))
            invalid=(mutated["revision"]!=revision or len(paths)!=expected["path_count"] or digest!=expected["pathset_sha256"]
                     or any(set(root)!=set(fixture["root_fields"]) for root in mutated["roots"])
                     or any(set(pattern)!=set(fixture["pattern_fields"]) for pattern in mutated["patterns"]))
    ...
    if mutation in {"design_presence","freeze_output_present"}:
        unit=validate_root_unit(descriptor,rows[-1],case["evaluation_stage"])
        return {"classification":"stage_absence_intersection","valid":False,"terminal":False,
                "recoverable":False,"disposition":"reject","dispositions":[]} if not unit["valid"] else unit
    if mutation=="mixed":
        return {"classification":"mixed","valid":False,"terminal":False,"recoverable":False,"disposition":"reject","dispositions":[]}
    if mutation=="multiple":
        return {"classification":"multiple_terminals","valid":False,"terminal":False,"recoverable":False,"disposition":"reject","dispositions":[]}
    terminal_id = None
...
def cache_entries(private_root: Path) -> list[dict]:
    ...
    for path in sorted([root, *root.rglob("*")], key=lambda p: p.relative_to(root).as_posix() if p != root else "."):
        ...
        if path.is_dir():
            ...
            # Reject from metadata before allocating or reading any file body.
            total_file_bytes += stat.st_size
            if stat.st_size > CAPS["json_each"] or total_file_bytes > CAPS["bundle"]:
                raise ValueError("cache_file_cap")
            digest = hashlib.sha256()
            read_bytes = 0
            with path.open("rb") as handle:
                while True:
                    block = handle.read(min(1 << 20, stat.st_size - read_bytes + 1))
                    if not block:
                        break
                    read_bytes += len(block)
                    if read_bytes > stat.st_size:
                        raise ValueError("cache_file_grew")
                    digest.update(block)
            if read_bytes != stat.st_size:
                raise ValueError("cache_file_size_drift")
            rows.append({"path": rel, "type": "file", "size": stat.st_size,
...
def validate_compile_evidence(evidence: dict) -> dict:
    ...
    expected_size = {"nvrtcGetProgramLogSize": "log", "nvrtcGetPTXSize": "ptx",
                     "nvrtcGetCUBINSize": "cubin"}
    checks["returned_sizes"] = all(
        row.get("code") != 0 or expected_size[row["op"]] not in artifacts
        or row.get("returned_size") == len(artifacts[expected_size[row["op"]]])
        for row in ledger if row.get("attempted") and row.get("op") in expected_size)
...
def atomic_create(path: Path, data: bytes) -> None:
    ...
    try:
        ...
        if temp.exists():
            temp.unlink()
        try: flush_directory(path.parent)
...
def publish_transaction(output: Path, bundle: dict[str, bytes], kind: str, verifier) -> None:
    ...
    try:
        ...
        if promoted and output.exists():
            rollback=output.with_name(output.name+f".rollback.{os.getpid()}.{uuid.uuid4().hex[:16]}")
            os.rename(output,rollback)
            shutil.rmtree(rollback)
        if stage.exists():
            shutil.rmtree(stage)
        try: flush_directory(output.parent)
        except Exception: pass
        raise
...
def write_incidental_failure(root: Path, row: dict) -> Path:
    ...
    if len(raw) > CAPS["json_each"]:
        ...
        row = {"kind": "het_next_l0_ph1_nvidia_nc19i2_bounded_failure",
    ...
    try:
        ...
        if promoted and attempt.exists(): shutil.rmtree(attempt)
        if stage.exists(): shutil.rmtree(stage)
        secondary = {"kind": "het_next_l0_ph1_nvidia_nc19i2_failure_writer_secondary",
                     "revision": REVISION, "status": "incidental_failure",
...
EXPORTS = (
    paths_for_revision, compile_topology_descriptor, classify_topology, capture_environment,
