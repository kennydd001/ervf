...
ROOT=Path(__file__).resolve().parents[2];S=ROOT/'scripts/streamq5_moe';R=ROOT/'reports/streamq5_moe';OUT=ROOT/'reports/runs/streamq5_moe/het_next_cap0r3_dual_device_cohabitation'
RUN=S/'run_het_next_cap0r3_dual_device_cohabitation.py';PRO=S/'het_next_cap0r3_protocol.py';KER=S/'het_next_cap0r3_kernels.py';VER=S/'verify_het_next_cap0r3_dual_device_cohabitation.py';LOCK=R/'het_next_cap0r3_runner_lock.json';VLOCK=R/'het_next_cap0r3_verifier_lock.json'
...
def main():
 ...
 with tempfile.TemporaryDirectory(prefix='cap0_static_') as td:
  td=Path(td);checks['actual_atomic_failure_simulation']=all(ns['simulate_atomic_failure'](td).values());tx=td/'tx';tx.mkdir();(tx/'cap0r3_result.json.x.inprogress').write_bytes(b'x');rec=ns['recover_transaction'](tx);checks['actual_transaction_recovery']=len(rec)==1 and (tx/'failed_attempts').exists()
 ...
 out={'kind':'het_next_cap0r3_static_preflight','pass':all(checks.values()),'checks':checks,'device_calls':0,'model_or_weight_opens':0};print(json.dumps(out,sort_keys=True));return 0 if out['pass'] else 1
