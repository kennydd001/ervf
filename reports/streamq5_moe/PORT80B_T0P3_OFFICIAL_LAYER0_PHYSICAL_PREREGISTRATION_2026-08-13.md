# PORT80B-T0-P3 — immutable official layer-0 real-Q5 physical preregistration

Date: 2026-08-13  
Phase: physical candidate, closed until T0-R3 passes independently  
Target revision: `a19358a7659bd1f564300250ee189120c49a562f`

This is the immutable eligibility strengthening after T0-P2. Older documents
remain evidence. P3 changes no physical, codec, resource, timing or numerical
gate. It requires the new T0-R3 official cache-state and manual-decomposition
gates to pass independently before the physical phase can open.
...
from T0-R3 and reproduce its independent dequantized-Q5 oracle bit-for-bit?
...
T0-P3 is closed unless every T0-R3 gate passes and a separate CPU verifier
rehashes all raw bytes. Freeze the T0-R3 result, prompt lock, environment lock,
official index/shard, quantizer, bank builder, physical runner, verifier and
this preregistration by SHA-256. The current T0-R3 digest is bound by the R3/P3
...
Build one new immutable layer-0 bank from official shard 1 after T0-R3:
...
- Primary: the exact 32 T0-R3 held-out rows at positions 8–15 in locked order.
...
payload hashes. Compare every stored array with T0-R3's Q5 oracle under the
frozen rounding rules; primary correctness requires zero differing bits.

The bit-exact oracle is an explicit CPU emulator of the frozen CUDA operation
order, reduction tree, BF16 rounding points, SiLU evaluation and route/shared
composition; native Torch CPU matrix multiplication is diagnostic only and may
not serve as the bit-exact oracle. Emulator source and unit vectors are hashed
before execution. Any CUDA kernel-order change requires a new preregistration.
...
1. T0-R3 and independent raw-byte verification pass; every lock is current,
...
3. Natural IDs and normalized weights equal T0-R3 exactly; selected source
...
No T0-P3 outcome automatically opens endurance or a full-depth claim. The next
