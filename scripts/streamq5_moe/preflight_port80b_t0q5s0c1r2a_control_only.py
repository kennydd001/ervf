#!/usr/bin/env python3
"""S0-C1-R2A self-bound CPU/no-forward preflight; TEMP lifecycle only."""
import ast,hashlib,json,os,tempfile
from pathlib import Path
ROOT=Path(__file__).resolve().parents[2];S=ROOT/'scripts/streamq5_moe';R=ROOT/'reports/streamq5_moe';SELF=Path(__file__);RUN=S/'run_port80b_t0q5s0c1r2a_control_only.py';VER=S/'verify_port80b_t0q5s0c1r2a_control_only.py';PR=R/'PORT80B_T0Q5S0C1R2A_CONTROL_ONLY_PREREGISTRATION_2026-08-13.md';LOCK=R/'port80b_t0q5s0c1r2a_runner_lock.json';VL=R/'port80b_t0q5s0c1r2a_verifier_lock.json';PL=R/'port80b_t0q5s0c1r2a_preflight_lock.json';D=ROOT/'reports/runs/streamq5_moe/port80b_t0q5s0c1r2a_control_only'
...
def simulation():
 with tempfile.TemporaryDirectory(prefix='s0c1r2a_') as z:
...
def result_construction_after_cleanup():
 fields=[15,21,15];mutfields=[15,20,15];changed_field_count=sum(a!=b for a,b in zip(fields,mutfields));del fields,mutfields
 record={'changed_field_count':changed_field_count,'row':0,'column':0,'q':6,'q_prime':5};payload=json.loads(json.dumps({'kind':'dummy_success','record':record},sort_keys=True));return payload=={'kind':'dummy_success','record':{'changed_field_count':1,'row':0,'column':0,'q':6,'q_prime':5}}
def main():
 l=json.loads(LOCK.read_text());v=json.loads(VL.read_text());p=json.loads(PL.read_text());actual={'preflight_sha256':sha(SELF),'runner_sha256':sha(RUN),'verifier_sha256':sha(VER),'prereg_sha256':sha(PR),'runner_lock_sha256':sha(LOCK),'verifier_lock_sha256':sha(VL)};checks={'self_bound':all(p[k]==x for k,x in actual.items()),'source_locks':l['runner_sha256']==actual['runner_sha256'] and l['verifier_sha256']==actual['verifier_sha256'] and l['prereg_sha256']==actual['prereg_sha256'] and l['verifier_lock_sha256']==actual['verifier_lock_sha256'] and v['verifier_sha256']==actual['verifier_sha256'],'opened_after_audit':l['execution_open'] is True and v['execution_open'] is True and l['control_only_authorization'] is True and l['physical_run_requires_independent_source_go'] is True and l['implementation_audit_token']==v['implementation_audit_token']=='S0C1R2A_AUTHORIZED_AFTER_REPAIR_GO','output_absent':not D.exists(),'ast_no_physical':ast_clean(),'temp_atomic_simulation':simulation(),'success_result_after_cleanup':result_construction_after_cleanup()};o={'kind':'t0q5s0c1r2a_preflight','pass':all(checks.values()),'checks':checks,'physical_actions':{'temp_only':True,'shard':False,'model':False,'gpu':False}};print(json.dumps(o,indent=2));return 0 if o['pass'] else 2
