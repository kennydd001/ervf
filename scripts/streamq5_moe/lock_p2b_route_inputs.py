...
PREREG = ROOT / "reports/streamq5_moe/P2B_PHYSICAL_RESIDENCY_H2D_PREREGISTRATION.md"
P2A_VERIFY = ROOT / "reports/streamq5_moe/p2a_kernel_verification.json"
ARTIFACT = ROOT / "reports/runs/streamq5_moe/p2b_fresh_route_input_ids.safetensors"
OUTPUT = ROOT / "reports/streamq5_moe/p2b_route_input_lock.json"
PRIOR_INPUTS = (
...
if __name__ == "__main__":
    if OUTPUT.exists() or ARTIFACT.exists():
        raise FileExistsError("refusing to overwrite P2B route input lock")
    p2a = json.loads(P2A_VERIFY.read_text(encoding="utf-8"))
    if p2a.get("status") != "p2a_kernel_verification_pass":
        raise RuntimeError("independent STREAMQ5 P2A kernel pass required")
    ...
    if set(tensors) != set(DOMAINS) or any(tuple(value.shape) != (1, TOKENS) for value in tensors.values()):
        raise RuntimeError("P2B route input contract failed")
    ...
    if not disjoint:
        raise RuntimeError("P2B route context overlaps a prior decision context")

    ARTIFACT.parent.mkdir(parents=True, exist_ok=True)
    save_file(tensors, ARTIFACT, metadata={"kind": "streamq5_moe_p2b_fresh_route_inputs", "routes_opened": "false"})
    payload = {
        "kind": "streamq5_moe_p2b_route_input_lock", "locked_utc": datetime.now(timezone.utc).isoformat(),
        "preregistration_sha256": sha256(PREREG), "p2a_verification_sha256": sha256(P2A_VERIFY),
        ...
        "routes_opened": False,
        "cache": {"total_slots": 1640, "static_slots_per_layer": 32, "dynamic_slots_layers_0_7": 3, "dynamic_slots_layers_8_47": 2, "expert_record_bytes": 3035136},
        ...
        "gates": {"mean_wall_h2d_ms_max": 25.0, "p95_wall_h2d_ms_max": 35.0, "static_preload_wall_ms_max": 250.0},
    }
