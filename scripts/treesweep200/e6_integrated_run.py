"""E6 - integrated physical run: ERVF + v4 attention + deterministic accum.
...
def main() -> int:
    ...
    # Both arms are deterministic (D1) so they are comparable at all; the one
    # variable is the integrated stack: ERVF + the v4 attention kernel.
    for name, use in (("base_a", False), ("integrated", True), ("base_b", False)):
        rt.fused.use_ervf = use
        rt.deterministic_accum = True
        rt.k.attention_fp8_gqa = v4_attn if use else v1_attn
        assert rt.fused.use_ervf is use
        assert rt.deterministic_accum is True
    ...
    rt.k.attention_fp8_gqa = v1_attn

    per = {}
    for k, p in enumerate(prompts):
        a1 = arms["base_a"]["rollout"][k]["latency"]
        a2 = arms["base_b"]["rollout"][k]["latency"]
        b = arms["integrated"]["rollout"][k]["latency"]
        base = 0.5 * (a1["p50"] + a2["p50"])
        drift = abs(a2["p50"] - a1["p50"])
        per[p["id"]] = {
            "base_p50": base, "stack_p50": b["p50"], "gain_ms": base - b["p50"],
            "drift_ms": drift,
            "base_p95": 0.5 * (a1["p95"] + a2["p95"]), "stack_p95": b["p95"],
            "base_p99": 0.5 * (a1["p99"] + a2["p99"]), "stack_p99": b["p99"],
    ...
    vram_ok = arms["integrated"]["device_free_bytes"] >= arms["base_a"]["device_free_bytes"]
    allc = all(v["conclusive"] and v["gain_ms"] > 0 for v in per.values())

    payload = {
        "kind": "treesweep200_e6_integrated", "registry": "TREESWEEP200",
        "phase": "E6_INTEGRATED", "started_utc": started,
        ...
        "config": {"rollout_tokens": ROLLOUT,
                   ...
                   "capacity": 72, "max_ctx": 4096, "deterministic_accum": True, "stack": ["ERVF w16", "attention v4", "D1"]},
        "arms": arms, "per_domain": per,
        "gates": {
            "G_E6_C1_exact": {"required": "identical tokens over 3 x 512, both arms deterministic",
                                 "passed": bool(exact)},
            "G_E6_P1_latency": {
                "per_domain_gain_ms": {k: v["gain_ms"] for k, v in per.items()},
                "all_conclusive_and_positive": bool(allc),
                "passed": bool(exact and allc)},
            "G_E6_M1_vram": {"base_free": arms["base_a"]["device_free_bytes"],
                                "stack_free": arms["integrated"]["device_free_bytes"],
                                "passed": bool(vram_ok)}},
        "claim_boundary": (
    ...
    (OUT_DIR / "E6_INTEGRATED_RUN.json").write_text(
