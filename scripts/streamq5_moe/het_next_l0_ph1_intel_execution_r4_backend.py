#!/usr/bin/env python3
"""R4 lifecycle wrapper: immediate pending ownership and safe telemetry."""
...
class Backend(r0.Backend):
 ...
 def run(self,records,input_bytes,lut,authorization):
  ...
  try:evidence=super().run(records,input_bytes,lut,authorization)
   ...
   exc.evidence['forbidden_calls']={name:sum(row.get('api')==name for row in self.ledger) for name in FORBIDDEN};exc.evidence['extension_counts']=dict(self.extension_counts);exc.evidence['ownership_ledger']=self.ownership_ledger;exc.evidence['telemetry_errors']=self.telemetry_errors;raise
  evidence['forbidden_calls']={name:sum(row.get('api')==name for row in self.ledger) for name in FORBIDDEN};evidence['extension_counts']=dict(self.extension_counts);evidence['ownership_ledger']=self.ownership_ledger;evidence['telemetry_errors']=self.telemetry_errors;return evidence
