...
ROOT=Path(__file__).resolve().parents[2];REPORTS=ROOT/'reports/streamq5_moe';RUN=ROOT/'reports/runs/streamq5_moe/port80b_t0y_canonical_router';LOCK=REPORTS/'port80b_t0y_canonical_router_lock.json';RESULT=RUN/'t0y_canonical_router_result.json';RAW=RUN/'t0y_canonical_router_raw.safetensors'
def sha(p):
 h=hashlib.sha256()
