...
R8_PREREG = REPORTS / "PORT80B_T0R8_DIRECT_OFFICIAL_CAPTURE_PREREGISTRATION_2026-08-13.md"
PROMPT_GENERATION_LOCK = REPORTS / "port80b_t0r8_prompt_generation_lock.json"
P4_PREREG = REPORTS / "PORT80B_T0P4_OFFICIAL_LAYER0_PHYSICAL_PREREGISTRATION_2026-08-13.md"
PROMPT_LOCK = REPORTS / "port80b_t0r7_prompt_lock.json"
...
RUNNER_LOCK = REPORTS / "port80b_t0r8_runner_lock.json"
VERIFIER = ROOT / "scripts" / "streamq5_moe" / "verify_port80b_t0r8_official_cpu_route_repro.py"
VERIFIER_LOCK = REPORTS / "port80b_t0r8_verifier_lock.json"
...
ACK = "T0R8_OFFICIAL_CPU_CAPTURE_AFTER_PREFLIGHT"
...
PROMPT_SHA = "797f40f395b687517a4002fc4d56a36477ff3d5fea35cf28182b3b5e78343cb5"
...
def write_invocation_ledger(run_index: int) -> dict[str, Any]:
    ...
    if run_index==1:
        ...
        if (entry["pid"],entry["process_create_time_unix"],entry["nonce"]) == (prior["pid"],prior["process_create_time_unix"],prior["nonce"]):
            raise RuntimeError("capture2 is not a distinct invocation")
        forbidden=[RUN_DIR/"t0r8_capture_1_raw.safetensors",RUN_DIR/"t0r8_capture_1_result.json"]
...
def execution_lock_check() -> dict[str, Any]:
    if not RUNNER_LOCK.is_file() or not VERIFIER.is_file() or not VERIFIER_LOCK.is_file():
        raise RuntimeError("R8 execution provenance file missing")
    ...
    expected = {
        ...
        "t0r8_prereg_sha256": sha256(R8_PREREG), "prompt_generation_lock_sha256": sha256(PROMPT_GENERATION_LOCK),
    ...
    if mismatched: raise RuntimeError(f"R8 execution lock mismatch: {mismatched}")
...
def reference(run_index: int) -> dict[str, Any]:
    ...
    raw_path = RUN_DIR / f"t0r8_capture_{run_index}_raw.safetensors"
    result_path = RUN_DIR / f"t0r8_capture_{run_index}_result.json"
    ...
    result = {"kind": "port80b_t0r8_official_cpu_route_repro_reference_and_bank_stage", "run_index": run_index,
...
def smoke() -> dict[str, Any]:
    ...
    return {"kind": "port80b_t0r8_cpu_smoke", "pass": header["pass"] and codec["pass"] and meta_contract,
...
if __name__ == "__main__":
    ...
    if args.phase == "lockcheck":
        payload = {"kind":"port80b_t0r8_exact_execution_lockcheck","pass":execution_lock_check()["pass"],
        ...
        try:
            ...
            RUN_DIR.mkdir(parents=True, exist_ok=True)
            failure = RUN_DIR / f"t0r8_capture_{args.run_index}_failure.json"
            if failure.exists():
            ...
            if partial.exists():
                ...
                target = failed_dir / f"t0r8_capture_{args.run_index}_partial_bank.bin"
            ...
            memory = psutil.Process().memory_info()
            evidence = {"kind":"port80b_t0r8_official_cpu_route_repro_failure", "status":"valid_negative_or_blocked_not_pass",
