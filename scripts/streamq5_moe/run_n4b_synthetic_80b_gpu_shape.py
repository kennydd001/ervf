...
R=ROOT/"reports/streamq5_moe";PREREG=R/"N4B_SYNTHETIC_80B_GPU_SHAPE_PREREGISTRATION.md"
OUTPUT=R/"n4b_synthetic_80b_gpu_shape.json";N4A=R/"n4a_synthetic_80b_shape_capacity.json";N1C=R/"n1c_generalized_exact_reduction_autotuner.json"
...
DEFINE_80B(16)
DEFINE_80B(32)
extern "C" __global__ void swiglu_80b(float* gate,const float* up){int i=(int)blockIdx.x*blockDim.x+threadIdx.x;if(i<11*512){float g=round_bf16(gate[i]);float u=round_bf16(up[i]);gate[i]=round_bf16((g/(1.0f+expf(-g)))*u);}}
'''
...
def main():
 ...
 def plane(width,capture=False):
  ...
  return out
 ref=plane(16,True);correct={str(w):comparison(plane(w,True),ref)for w in WIDTHS}
 ...
 result={"kind":"streamq5_moe_n4b_synthetic_80b_gpu_shape","completed_utc":datetime.now(timezone.utc).isoformat(),"inputs":{"preregistration_sha256":sha256(PREREG),"n4a_sha256":sha256(N4A),"n1c_sha256":sha256(N1C),"seed":SEED},"physical":{"gpu":cp.cuda.runtime.getDeviceProperties(0)["name"].decode(),"q5_bank_bytes":int(bank.size),"layers":LAYERS,"active_experts":ACTIVE,"hidden":HIDDEN,"intermediate":INTER},"correctness":correct,"validation":validation,"selected_width":selected,"test":test,"expert_test_stats":expert,"dense_projection":{"n1c_q8_p95_ms":n1c_q8["p95"],"source_bytes":old_bytes,"official_80b_shell_bytes":shell,"byte_linear_p95_ms":dense_p95,"conservative_2x_p95_ms":dense_conservative,"projected_total_p95_ms":expert["p95"]+dense_conservative},"gates":gates,"overall_pass":all(gates.values()),"claim_boundary":"Synthetic physical Q5 active-expert shape plus byte-linear physical-Q8 projection; no checkpoint payload, quality, routing, DeltaNet timing or end-to-end tok/s."}
