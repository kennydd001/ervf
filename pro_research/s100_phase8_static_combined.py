...
"""Install exact static-hit / sparse-miss routed-down execution."""
...
def install_static_down_moe(
    ...
    max_k = int(rt.top_k)
    if max_k != 6:
        raise RuntimeError(f"phase8 requires K6 shell, got {max_k}")
    ...
    def moe(self, layer, out):
        ...
        static = static_cache.layers[int(layer)]
        ...
        def issue(slot):
            static_cache.kernels.gather_miss(
                (gather_blocks,),
                (256,),
                (
                    ...
                    mirrors[slot & 1],
                    work["nz"][
                        slot * inter:(slot + 1) * inter
                    ],
                    work["nzc"][slot:slot + 1],
                    np.int32(hidden),
                    static.expert_to_static,
                ),
            )
        ...
        for slot in range(ki):
            if slot + 1 < ki:
                with gather_stream:
                    if slot >= 1:
                        gather_stream.wait_event(
                            math_done[slot - 1]
                        )
                    issue(slot + 1)
                    gather_done[slot + 1].record(
                        gather_stream
                    )

            main.wait_event(gather_done[slot])
            static_cache.kernels.down_hybrid(
                grid_down,
                (128,),
                (
                    mirrors[slot & 1],
    ...
    rt._phase8_static_state = state

    def restore():
        rt._moe_dev = original
        sres.planes.clear()
        if hasattr(rt, "_phase8_static_state"):
            delattr(rt, "_phase8_static_state")
