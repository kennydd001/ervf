#!/usr/bin/env python3
"""Standalone S0-R1 CPU validation; no model forward, GPU, or persistent weights."""
from __future__ import annotations
import argparse,hashlib,json,math,os,shutil,sys,traceback,uuid
...
ROOT=Path(__file__).resolve().parents[2];S=ROOT/'scripts/streamq5_moe';R=ROOT/'reports/streamq5_moe';D=ROOT/'reports/runs/streamq5_moe/port80b_t0q5s0r1_selected_route_validation';RAW=D/'s0r1_raw.safetensors';RES=D/'s0r1_result.json';COM=D/'s0r1_commit.json';FAIL=D/'s0r1_failure.json';BAD=D/'failed_attempts';D2=ROOT/'reports/runs/streamq5_moe/port80b_t0r12d2r3_cloned_serialization/t0r12d2_raw.safetensors';D2R=ROOT/'reports/runs/streamq5_moe/port80b_t0r12d2r3_cloned_serialization/t0r12d2_result.json';AUD=R/'PORT80B_T0R12D2R3_INDEPENDENT_ARTIFACT_AUDIT_2026-08-13.json';PR=R/'PORT80B_T0Q5S0R1_SELECTED_ROUTE_VALIDATION_PREREGISTRATION_2026-08-13.md';LOCK=R/'port80b_t0q5s0r1_runner_lock.json';VL=R/'port80b_t0q5s0r1_verifier_lock.json';VER=S/'verify_port80b_t0q5s0r1_selected_route_validation.py';DEP=R/'port80b_t0r4_dependency_execution_lock.json';SHARD=Path.home()/'.cache/huggingface/hub/models--Qwen--Qwen3-Coder-Next/snapshots/a19358a7659bd1f564300250ee189120c49a562f/model-00001-of-00040.safetensors';ACK='T0Q5S0R1_VALIDATION_ONLY_AFTER_AUDIT';NAMES=('gate','up','down');SHAPES=((512,2048),(512,2048),(2048,512));MAX=512*2**20
...
def lockcheck():
 l=json.loads(LOCK.read_text());a={'runner_sha256':sha(__file__),'verifier_sha256':sha(VER),'verifier_lock_sha256':sha(VL),'prereg_sha256':sha(PR),'dependency_lock_sha256':sha(DEP),'d2_raw_sha256':sha(D2),'d2_result_sha256':sha(D2R),'d2_audit_sha256':sha(AUD)};return {'pass':all(l.get(k)==v for k,v in a.items()),'bindings':a}
def fsync(p):
 with Path(p).open('rb') as h:os.fsync(h.fileno())
def fsyncdir(p):
 if os.name!='nt':
...
def recover():
 ...
 if COM.exists():raise FileExistsError('committed result exists')
 BAD.mkdir(exist_ok=True);moved={};nonce=uuid.uuid4().hex
 for p in list(D.glob('*.inprogress'))+[x for x in (RAW,RES) if x.exists()]:q=BAD/f'{nonce}_{p.name}';os.rename(p,q);moved[p.name]=str(q)
...
def run():
 ...
 moved=recover()
 if D.exists() and any(D.iterdir()):raise FileExistsError('nonclean output')
 ...
 with safe_open(D2,framework='pt',device='cpu') as d2,safe_open(SHARD,framework='pt',device='cpu') as src,torch.inference_mode():
  ...
  for p in range(4):
   ...
   for n in (8,15):
    ...
    if p==0:
     ...
     a,cc=chosen;new=int(qq[a,cc]-(1 if qq[a,cc]>0 else -1));mut=q5(512,2).clone();sb=torch.from_numpy(np.frombuffer(scales,'<u2').copy()).to(torch.uint16).view(torch.bfloat16);mut[a,cc]=torch.tensor(float(new)*float(sb[a*4+cc//128]),dtype=torch.bfloat16);actual=hashlib.sha256(codes+scales+b'mutated').hexdigest();errors=checker({'expert':512,'projection':2,'shape':list(SHAPES[2])},{'expert':512,'projection':2,'shape':list(SHAPES[2])},expected,actual);mg=lambda e,j:mut if e==512 and j==2 else q5(e,j);_,ms=graph(x,ids,w,mg);kr=f'p0_n{n}_mutation_shared_raw';kg=f'p0_n{n}_mutation_shared_gated';raw[kr]=ms[n:n+1];raw[kg]=(gate*ms)[n:n+1];controls.append({'prompt':0,'position':n,'control':'shared_down_code_mutation_graph_wide','matrix_row':a,'matrix_column':cc,'source_q':int(qq[a,cc]),'mutated_q':new,'expected_digest':expected,'presented_digest':actual,'rejection_errors':errors,'raw_key':kr,'gated_raw_key':kg,'baseline_key':'p0_q5_shared_raw','gated_baseline_key':'p0_q5_shared_gated'})
  sample(resources,'computed');cache.clear();wiremeta.clear();sample(resources,'cleanup');manifest={k:{'dtype':str(v.dtype),'shape':list(v.shape),'bytes':v.numel()*v.element_size(),'sha256':hashlib.sha256(tb(v)).hexdigest()} for k,v in sorted(raw.items())}
  if not all(torch.isfinite(v.float()).all() for v in raw.values()):raise RuntimeError('finite')
  status='selected_route_validation_positive' if all(q['rel_l2']<=.08 for p in metrics.values() for a in p.values() for q in a) else 'selected_route_q5_quality_negative';runtime={'affinity':proc.cpu_affinity(),'threads':torch.get_num_threads(),'interop':torch.get_num_interop_threads(),'deterministic':torch.are_deterministic_algorithms_enabled(),'mkldnn':torch.backends.mkldnn.enabled,'matmul_precision':torch.get_float32_matmul_precision(),'autocast_cpu':torch.is_autocast_enabled('cpu'),'inference_mode':torch.is_inference_mode_enabled(),'cuda_initialized':torch.cuda.is_initialized()};nonce=uuid.uuid4().hex;rt=D/f'{RAW.name}.{nonce}.inprogress';jt=D/f'{RES.name}.{nonce}.inprogress';ct=D/f'{COM.name}.{nonce}.inprogress';save_file({k:v.clone() for k,v in raw.items()},rt);fsync(rt);result={'kind':'port80b_t0q5s0r1_validation','status':status,'claim_boundary':'validation only; no heldout/pass/complete/layer/model/GPU/performance','runner_sha256':sha(__file__),'verifier_sha256':sha(VER),'verifier_lock_sha256':sha(VL),'prereg_sha256':sha(PR),'dependency_lock_sha256':sha(DEP),'d2_raw_sha256':sha(D2),'shard_sha256':sha(SHARD),'selected_union':union,'matrix_evidence':evidence,'raw_manifest':manifest,'raw_sha256':sha(rt),'metrics':metrics,'controls':controls,'runtime':runtime,'resources':resources,'recovered':moved};jt.write_bytes(canon(result)+b'\n');fsync(jt);commit={'kind':'t0q5s0r1_commit','files':{RAW.name:{'bytes':rt.stat().st_size,'sha256':sha(rt)},RES.name:{'bytes':jt.stat().st_size,'sha256':sha(jt)}}};ct.write_bytes(canon(commit)+b'\n');fsync(ct)
  ...
  os.rename(rt,RAW);os.rename(jt,RES);fsyncdir(D);os.rename(ct,COM);fsyncdir(D);sample(resources,'final');return result
def atomic_failure(e):
 D.mkdir(parents=True,exist_ok=True);nonce=uuid.uuid4().hex;t=D/f'{FAIL.name}.{nonce}.inprogress';payload={'kind':'t0q5s0r1_failure','error_type':type(e).__name__,'error':str(e),'traceback':traceback.format_exc(),'runner_sha256':sha(__file__),'resources':{'rss':psutil.Process().memory_info().rss,'peak':getattr(psutil.Process().memory_info(),'peak_wset',0),'available':psutil.virtual_memory().available},'cuda_initialized':torch.cuda.is_initialized()};t.write_bytes(canon(payload)+b'\n');fsync(t)
...
def main():
 ...
 if a.phase=='lockcheck':print(json.dumps({'kind':'s0r1_lockcheck',**lockcheck(),'physical_actions':False}));return 0
 if a.ack!=ACK:raise SystemExit('ack')
 try:o=run();print(o['status']);return 3
