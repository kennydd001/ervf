#!/usr/bin/env python3
"""Independent PH1 NVIDIA N2 compile/physical verifier.
...
LOCK = REPORTS / "het_next_l0_ph1_nvidia_n2_verifier_lock.json"; COMPILE = REPORTS / "het_next_l0_ph1_nvidia_n2_compile"; PHYSICAL = REPORTS / "het_next_l0_ph1_nvidia_n2_physical"; OUT = REPORTS / "het_next_l0_ph1_nvidia_n2_independent_verification.json"
SOURCE_LOCK = REPORTS / "het_next_l0_ph1_nvidia_n2_source_lock.json"; CUDA_SOURCE = SCRIPTS / "het_next_l0_ph1_nvidia_n2_kernels.cu"
CUDA_SHA = "7326fedd46459671ead70ade7458b75ef5617354c8f8c454b7842bc62838b555"
...
def provenance():
 try:
  ...
  if lock.get("kind")!="het_next_l0_ph1_nvidia_n2_verifier_lock":return False
...
def verify_compile(directory):
 kind="het_next_l0_ph1_nvidia_n2_compile";directory=Path(directory);checks={"provenance":provenance(),"bundle":bundle(directory,kind)}
 try:
  ...
  "candidate_source":sha(source)==CUDA_SHA==fsha(CUDA_SOURCE) and bound=={"path":"scripts/streamq5_moe/het_next_l0_ph1_nvidia_n2_kernels.cu","sha256":CUDA_SHA} and r["artifact_manifest"]["source.cu"]["sha256"]==CUDA_SHA and r["authorization"]["observed"]["cuda_source"]==CUDA_SHA,
...
def verify_physical(directory):
 kind="het_next_l0_ph1_nvidia_n2_physical";directory=Path(directory);checks={"provenance":provenance(),"bundle":bundle(directory,kind)}
 try:
  ...
  abi_names={"cuInit","cuDriverGetVersion","cuDeviceGetCount","cuDeviceGet","cuDeviceGetName","cuDeviceGetUuid_v2","cuDeviceGetPCIBusId","cuDeviceGetAttribute","cuDeviceTotalMem_v2","cuMemGetInfo_v2","cuCtxGetCurrent","cuDevicePrimaryCtxGetState","cuDevicePrimaryCtxRetain","cuCtxPushCurrent_v2","cuCtxPopCurrent_v2","cuDevicePrimaryCtxRelease_v2","cuStreamCreate","cuStreamSynchronize","cuStreamDestroy_v2","cuModuleLoadDataEx","cuModuleGetFunction","cuModuleUnload","cuMemHostAlloc","cuMemFreeHost","cuMemAlloc_v2","cuMemFree_v2","cuMemcpyHtoDAsync_v2","cuMemcpyDtoHAsync_v2","cuMemsetD8Async","cuLaunchKernel"}
  ...
  "loader_abi":len(loader)==1 and loader[0]["path"].casefold()==r"c:\windows\system32\nvcuda.dll" and loader[0]["sha256"]=="86b41599a673f1aa4699ab458dc5c1e02b57da64d17221f45327af0393fd59a5" and loader[0]["bytes"]==4466920 and loader[0]["calling_convention"]=="WinDLL" and loader[0]["winmode"]==0x800 and set(ev["abi"])==abi_names and all(set(x)=={"argtypes","restype"} and x["restype"]=="<class 'ctypes.c_int'>" for x in ev["abi"].values()) and "c_ulong" in ev["abi"]["cuMemAlloc_v2"]["argtypes"][0],
  ...
  "sync_read":sum(x["op"]=="stream_synchronize" for x in led)==1 and [x["name"] for x in reads]==["gate","up","silu","activation","down","gate_counters","up_counters","activation_counters","down_counters"] and all(x["after_sync"] is True and x["sha256"]==sha(outs[x["name"]]) for x in reads),"outputs":stage_exact and counter_exact,
  ...
  "forbidden_runtime":ev["static_call_surface"]=="hash_bound_backend_ast_allowlist" and modules["forbidden"]==[] and modules["nvcuda_exact_count"]==1 and all("cudart" not in x["name"].casefold() and "cupy" not in x["path"].casefold() for x in modules["after"]),
