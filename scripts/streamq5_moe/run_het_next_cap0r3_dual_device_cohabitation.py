...
RUN_DIR = ROOT / "reports/runs/streamq5_moe/het_next_cap0r3_dual_device_cohabitation"
RESULT = RUN_DIR / "cap0r3_result.json"
COMMIT = RUN_DIR / "cap0r3_commit.json"
FAILURE = RUN_DIR / "cap0r3_failure.json"
LOCK = REPORTS / "het_next_cap0r3_runner_lock.json"
VERIFIER_LOCK = REPORTS / "het_next_cap0r3_verifier_lock.json"
PREREG = REPORTS / "HET_NEXT_CAP0R3_DUAL_DEVICE_COHABITATION_PREREGISTRATION_2026-08-13.md"
DESIGN = REPORTS / "HET_NEXT_CAP0R3_STATIC_PREFLIGHT_DESIGN_2026-08-13.md"
PROTOCOL_PATH = SCRIPTS / "het_next_cap0r3_protocol.py"
KERNEL_PATH = SCRIPTS / "het_next_cap0r3_kernels.py"
VERIFIER = SCRIPTS / "verify_het_next_cap0r3_dual_device_cohabitation.py"
PREFLIGHT = SCRIPTS / "preflight_het_next_cap0r3_static.py"
...
def intel_backend(ledger):
    from ctypes import wintypes
    from het_next_cap0r3_kernels import INTEL_BUILD_OPTIONS, INTEL_SOURCE
...
def nvidia_backend(ledger):
    from het_next_cap0r3_kernels import NVIDIA_NVRTC_OPTIONS, NVIDIA_SOURCE
    ...
    try:
        ck(nvrtcCreateProgram(C.byref(program),NVIDIA_SOURCE.encode(),b'cap0r3.cu',0,None,None),'nvrtcCreateProgram');cleanup.append((ledger.create('nvidia_nvrtc_program','nvidia'),lambda:nvrtcDestroyProgram(C.byref(program))))
...
def pdh_monitor(stop_event, p, ledger):
    ...
    try:
        ...
        try:ledger.release(query_row, lambda: pdh.PdhCloseQuery(query))
...
def run_capability(ack):
    ...
    from het_next_cap0r3_protocol import DualDeviceProtocol, Win32Primitives, atomic_create_json
...
def main():
    ...
    try:
        ...
        from het_next_cap0r3_protocol import atomic_create_json, recover_transaction
