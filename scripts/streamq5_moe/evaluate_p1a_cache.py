...
PREREG = REPORT_DIR / "P1A_CACHE_PREREGISTRATION.md"
INPUT_LOCK = REPORT_DIR / "p1a_route_input_lock.json"
CAPTURE = REPORT_DIR / "p1a_route_capture_result.json"
EVALUATOR_LOCK = REPORT_DIR / "p1a_cache_evaluator_lock.json"
...
if __name__ == "__main__":
    args = parse_args()
    output = REPORT_DIR / f"p1a_cache_{args.split}.json"
    report = REPORT_DIR / f"P1A_CACHE_{args.split.upper()}.md"
    if output.exists() or report.exists():
        raise FileExistsError(f"refusing to overwrite P1A cache {args.split}")
    validation_path = REPORT_DIR / "p1a_cache_validation.json"
    ...
    if sha256(Path(__file__)) != evaluator_lock["evaluator_sha256"] or sha256(INPUT_LOCK) != evaluator_lock["input_lock_sha256"] or sha256(CAPTURE) != evaluator_lock["route_capture_sha256"]:
        raise ValueError("P1A cache evaluator lock mismatch")
    ...
    if sha256(PREREG) != lock["preregistration_sha256"] or capture.get("status") != "route_capture_complete":
        raise ValueError("P1A provenance or route capture failure")
    ...
    if args.split == "validation":
        status = "p1a_validation_pass_test_authorized" if all_pass else "p1a_validation_closed"
        p1a_pass = False
    else:
        validation = json.loads(validation_path.read_text(encoding="utf-8"))
        if validation.get("status") != "p1a_validation_pass_test_authorized":
            raise RuntimeError("test not authorized by validation")
        status = "p1a_cache_pass" if all_pass and all(validation["gates"].values()) else "p1a_cache_closed"
        p1a_pass = status == "p1a_cache_pass"
    payload = {
        "kind": "streamq5_moe_p1a_cache_evaluation", "completed_utc": datetime.now(timezone.utc).isoformat(), "split": args.split, "status": status,
        ...
        "per_domain": per_domain, "aggregate": aggregate, "gates": gates, "p1a_pass": p1a_pass, "next_phase_authorized": p1a_pass,
    ...
    report.write_text(
        f"# STREAMQ5-MoE P1A - {args.split} cache\n\nUitkomst: **{status}**.\n\n"
