#!/usr/bin/env python3
"""Standalone S0-R3 CPU validation; no model forward, GPU, or persistent weights."""
...
ROOT=Path(__file__).resolve().parents[2];S=ROOT/'scripts/streamq5_moe';R=ROOT/'reports/streamq5_moe';D=ROOT/'reports/runs/streamq5_moe/port80b_t0q5s0r3_selected_route_validation';RAW=D/'s0r3_raw.safetensors';RES=D/'s0r3_result.json';COM=D/'s0r3_commit.json';FAIL=D/'s0r3_failure.json';BAD=D/'failed_attempts';D2=ROOT/'reports/runs/streamq5_moe/port80b_t0r12d2r3_cloned_serialization/t0r12d2_raw.safetensors';D2R=ROOT/'reports/runs/streamq5_moe/port80b_t0r12d2r3_cloned_serialization/t0r12d2_result.json';AUD=R/'PORT80B_T0R12D2R3_INDEPENDENT_ARTIFACT_AUDIT_2026-08-13.json';PR=R/'PORT80B_T0Q5S0R3_SELECTED_ROUTE_VALIDATION_PREREGISTRATION_2026-08-13.md';LOCK=R/'port80b_t0q5s0r3_runner_lock.json';VL=R/'port80b_t0q5s0r3_verifier_lock.json';VER=S/'verify_port80b_t0q5s0r3_selected_route_validation.py';DEP=R/'port80b_t0r4_dependency_execution_lock.json';SHARD=Path.home()/'.cache/huggingface/hub/models--Qwen--Qwen3-Coder-Next/snapshots/a19358a7659bd1f564300250ee189120c49a562f/model-00001-of-00040.safetensors';SHARD_SHA='8e9a517133bfbdc6806cf8b61793055a260efeb68e6e019fd90e4bbb1b665d0a';ACK='T0Q5S0R3_VALIDATION_ONLY_AFTER_AUDIT';NAMES=('gate','up','down');SHAPES=((512,2048),(512,2048),(2048,512));MAX=512*2**20
def sha(p):return hashlib.sha256(Path(p).read_bytes()).hexdigest()
AUDIT_TOKEN='S0R3_IMPLEMENTATION_AUDIT_GO'
...
def lockcheck():
 l=json.loads(LOCK.read_text());a={'runner_sha256':sha(__file__),'verifier_sha256':sha(VER),'verifier_lock_sha256':sha(VL),'prereg_sha256':sha(PR),'dependency_lock_sha256':sha(DEP),'d2_raw_sha256':sha(D2),'d2_result_sha256':sha(D2R),'d2_audit_sha256':sha(AUD)};auth=l.get('execution_open') is True and l.get('s0_validation_authorization') is True and l.get('implementation_audit_token')==AUDIT_TOKEN and l.get('dependency_use')=='runtime fields/hash only; dependency outputs_opened false is not S0 authorization';return {'pass':all(l.get(k)==v for k,v in a.items()) and auth,'bindings':a}
def fsync(p):
...
def main():
 ...
 if a.phase=='lockcheck':print(json.dumps({'kind':'s0r3_lockcheck',**lockcheck(),'physical_actions':False}));return 0
