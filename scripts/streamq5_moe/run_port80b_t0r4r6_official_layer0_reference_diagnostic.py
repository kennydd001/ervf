...
RUN_DIR = ROOT / "reports" / "runs" / "streamq5_moe" / "port80b_t0r4r6_official_layer0"
...
RUNNER_LOCK = REPORTS / "port80b_t0r4r6_runner_lock.json"
VERIFIER = ROOT / "scripts" / "streamq5_moe" / "verify_port80b_t0r4r6_official_layer0_reference.py"
VERIFIER_LOCK = REPORTS / "port80b_t0r4r6_verifier_lock.json"
RECORD_ARTIFACT = RUN_DIR / "layer0_513_real_q5_records.bin"
RECORD_MANIFEST = RUN_DIR / "layer0_513_real_q5_records_manifest.json"
ACK = "T0R4R6_OFFICIAL_LAYER0_CPU_REFERENCE_AFTER_INDEPENDENT_AUDIT"
...
def locked_inputs() -> dict[str, Any]:
    ...
    wanted = {PREREG: R4_SHA, P4_PREREG: P4_SHA, PROMPT_LOCK: PROMPT_SHA,
...
def router_artifacts(layer: Qwen3NextDecoderLayer, normed: torch.Tensor) -> dict[str, torch.Tensor]:
    global ROUTER_DIAGNOSTIC_CALL
    ROUTER_DIAGNOSTIC_CALL += 1
    FAILURE_STATE["stage"] = f"router_recomputation_call_{ROUTER_DIAGNOSTIC_CALL}"
    ...
    checks = {
        "ids_exact": torch.equal(ids, fp32_ids),
        "native_weights_exact_bf16_cast": torch.equal(native_weights, pre_weights.to(torch.bfloat16)),
        "fp32_sum_bound": float(fp32_sum_error.max()) <= 2**-20,
        "bf16_sum_bound": float(bf16_sum_error.max()) <= 0.00390720367431640625,
        "top10_top11_margin_strict_positive": bool((margins > 0).all()),
        "pre_weights_positive": bool((pre_weights > 0).all()),
        "native_weights_positive": bool((native_weights > 0).all()),
        "pre_weights_finite": bool(torch.isfinite(pre_weights).all()),
        "native_weights_finite": bool(torch.isfinite(native_weights.float()).all()),
        "pre_weights_nonincreasing": bool((pre_weights[:, :-1] >= pre_weights[:, 1:]).all()),
        "native_weights_nonincreasing": bool((native_weights[:, :-1] >= native_weights[:, 1:]).all()),
        "ids_nonnegative": bool((ids >= 0).all()),
        "ids_below_num_experts": bool((ids < EXPERTS).all()),
        "ids_unique_per_token": all(torch.unique(row).numel() == TOP_K for row in ids),
    }
    if not all(checks.values()):
        def first_false(mask: torch.Tensor) -> list[int] | None:
            found = torch.nonzero(~mask, as_tuple=False)
            return found[0].cpu().tolist() if found.numel() else None
        ids_diff = ids != fp32_ids
        weights_diff = native_weights.view(torch.uint16) != pre_weights.to(torch.bfloat16).view(torch.uint16)
        diagnostic = {
            "kind": "port80b_t0r4r6_router_failure_diagnostic", "call": ROUTER_DIAGNOSTIC_CALL,
            "checks": checks, "failed_conjuncts": [name for name, passed in checks.items() if not passed],
            "official_semantics": {"router_logits_dtype": str(native_logits.dtype), "softmax_dtype": "torch.float32",
                                   "top_k": layer.mlp.gate.top_k, "norm_topk_prob": layer.mlp.gate.norm_topk_prob,
                                   "returned_weights_dtype": str(native_weights.dtype)},
            "scalars": {"fp32_sum_error_max": float(fp32_sum_error.max()),
                        "bf16_sum_error_max": float(bf16_sum_error.max()), "minimum_top10_top11_margin": float(margins.min()),
                        "pre_weight_min": float(pre_weights.min()), "native_weight_min": float(native_weights.float().min()),
                        "id_mismatch_count": int(ids_diff.sum()), "weight_bit_mismatch_count": int(weights_diff.sum())},
            "first_mismatch": {"ids": first_false(~ids_diff), "weights_bf16_bits": first_false(~weights_diff),
                               "nonpositive_margin": first_false(margins > 0),
                               "pre_not_nonincreasing": first_false(pre_weights[:, :-1] >= pre_weights[:, 1:]),
                               "native_not_nonincreasing": first_false(native_weights[:, :-1] >= native_weights[:, 1:])},
            "sha256": {"native_logits": tensor_sha(native_logits), "fp32_logits": tensor_sha(fp32_logits),
                       "probs": tensor_sha(probs), "ids": tensor_sha(ids), "fp32_ids": tensor_sha(fp32_ids),
                       "native_weights": tensor_sha(native_weights), "pre_weights": tensor_sha(pre_weights),
                       "margins": tensor_sha(margins)},
        }
        RUN_DIR.mkdir(parents=True, exist_ok=True)
        path = RUN_DIR / f"t0r4r6_router_call_{ROUTER_DIAGNOSTIC_CALL}_failure.json"
        with path.open("x", encoding="utf-8") as handle:
            json.dump(diagnostic, handle, indent=2); handle.write("\n")
        FAILURE_STATE["router_diagnostic"] = {"path": str(path.relative_to(ROOT)).replace("\\", "/"),
                                               "sha256": sha256(path), "failed_conjuncts": diagnostic["failed_conjuncts"]}
...
def reference(run_index: int) -> dict[str, Any]:
    ...
    if (sha256(Path(__file__)) != lock.get("runner_sha256")
            or sha256(PREREG) != lock.get("t0r4_sha256")
            or sha256(P4_PREREG) != lock.get("t0p4_sha256")
            or sha256(DEPENDENCY_LOCK) != lock.get("dependency_lock_sha256")
            or sha256(VERIFIER) != lock.get("verifier_sha256")
            or sha256(VERIFIER_LOCK) != lock.get("verifier_lock_sha256")):
    ...
    raw_path = RUN_DIR / f"t0r4r6_run_{run_index}_raw.safetensors"
    result_path = RUN_DIR / f"t0r4r6_run_{run_index}_result.json"
    if raw_path.exists() or result_path.exists(): raise FileExistsError("refusing to overwrite T0-R4 run outputs")
    ...
    result = {"kind": "port80b_t0r4r6_official_layer0_reference_and_bank_stage", "run_index": run_index,
              "status": "run1_complete_pending_clean_replay" if run_index == 1 else "t0r4r6_reference_bank_stage_candidate_pass",
    ...
    if run_index == 2:
        prior = json.loads((RUN_DIR / "t0r4r6_run_1_result.json").read_text(encoding="utf-8"))
        prior_raw = safe_open(RUN_DIR / "t0r4r6_run_1_raw.safetensors", framework="pt", device="cpu")
        ...
        result["clean_replay"] = {"raw_tensors_bitwise_equal": replay_equal,
                                  "raw_manifest_semantic_equal": prior["raw_tensor_manifest"] == raw_manifest,
                                  "record_sha_equal": prior["record_artifact"]["sha256"] == record_manifest["sha256"],
                                  "record_manifest_semantic_equal": {
                                      k: v for k, v in prior["record_artifact"].items() if k != "second_run_compared_bytes"
                                  } == {k: v for k, v in record_manifest.items() if k != "second_run_compared_bytes"}}
...
def smoke() -> dict[str, Any]:
    ...
    return {"kind": "port80b_t0r4r6_cpu_smoke", "pass": header["pass"] and codec["pass"] and meta_contract,
...
if __name__ == "__main__":
    ...
    if args.phase == "smoke":
        ...
        try:
            ...
            RUN_DIR.mkdir(parents=True, exist_ok=True)
            failure = RUN_DIR / f"t0r4r6_run_{args.run_index}_failure.json"
            if failure.exists():
            ...
            if partial.exists():
                ...
                target = failed_dir / f"t0r4r6_run_{args.run_index}_partial_bank.bin"
            ...
            memory = psutil.Process().memory_info()
            evidence = {"kind":"port80b_t0r4r6_official_layer0_failure", "status":"valid_negative_or_blocked_not_pass",
                        ...
                        "cuda_initialized":torch.cuda.is_initialized(), "partial_bank":partial_evidence,
                        "diagnostic":FAILURE_STATE.get("router_diagnostic"),
