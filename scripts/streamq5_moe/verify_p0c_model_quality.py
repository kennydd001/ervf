...
def expected_test_status(validation_relative: float, test_relative: float, finite: bool) -> tuple[str, bool]:
    if finite and validation_relative <= 0.02 and test_relative <= 0.02:
        return "p0c_quality_pass", True
    return "p0c_quality_closed", False


def audit_result(audit: Audit, split: str, result: dict, lock: dict, validation: dict) -> None:
    prefix = f"{split}:"
    audit.check(prefix + " kind", result.get("kind") == "streamq5_moe_p0c_physical_scale_model_quality")
    ...
    audit.check(prefix + " physical scale semantics", result.get("scale_semantics") == "codes selected with FP32 maxabs scale; scale rounded to BF16 before dequantization; output rounded BF16")
    ...
    audit.check(prefix + " controls", all(controls.get(key) is True for key in ("all_finite", "all_48_layers", "fresh_exact_contexts", "test_opened_after_validation", "primary_variant_fixed")))
    ...
    if split == "validation":
        expected_validation = "p0c_validation_pass_test_authorized" if controls.get("all_finite") is True and primary_relative <= 0.025 else "p0c_validation_closed"
        audit.check(prefix + " validation status", result.get("status") == expected_validation, expected_validation)
        audit.check(prefix + " no premature pass", result.get("p0_pass") is False and result.get("next_phases_authorized") is False)
    else:
        validation_relative = validation["variants"][PRIMARY]["relative_cross_entropy_increase"]
        expected_status, expected_pass = expected_test_status(validation_relative, primary_relative, controls.get("all_finite") is True)
        audit.check(prefix + " preregistered decision", result.get("status") == expected_status, expected_status)
        audit.check(prefix + " P0 pass flag", result.get("p0_pass") is expected_pass)
        audit.check(prefix + " no repair", result.get("repair_authorized") is False)
        audit.check(prefix + " next phases flag", result.get("next_phases_authorized") is expected_pass)
...
def main() -> None:
    audit = Audit()
    prereg = REPORT_DIR / "P0C_PHYSICAL_SEMANTICS_PREREGISTRATION.md"
    lock_path = REPORT_DIR / "p0c_input_lock.json"
    evaluator_lock_path = REPORT_DIR / "p0c_evaluator_lock.json"
    input_path = ROOT / "reports/runs/streamq5_moe/p0c_fresh_input_ids.safetensors"
    old_lock_path = ROOT / "reports/coretail_moe/p2_input_lock.json"
    old_input_path = ROOT / "reports/runs/coretail_moe/p2_heldout_input_ids.safetensors"
    q4_lock_path = ROOT / "reports/streamq4_moe/p0_input_lock.json"
    q4_input_path = ROOT / "reports/runs/streamq4_moe/p0_fresh_input_ids.safetensors"
    q5_lock_path = ROOT / "reports/streamq5_moe/p0_input_lock.json"
    q5_input_path = ROOT / "reports/runs/streamq5_moe/p0_fresh_input_ids.safetensors"
    model_index = ROOT / "models/qwen3-30b-a3b-base/model.safetensors.index.json"
    evaluator = ROOT / "scripts/streamq5_moe/run_p0c_model_quality.py"
    validation_path = REPORT_DIR / "p0c_validation_model_quality.json"
    test_path = REPORT_DIR / "p0c_test_model_quality.json"
    ...
    audit.check("evaluator hash lock", sha256(evaluator) == evaluator_lock["evaluator_sha256"])
    audit.check("evaluator input lock", sha256(lock_path) == evaluator_lock["input_lock_sha256"])
    audit.check("prior artifact hashes", sha256(old_lock_path) == lock["previous_coretail_lock_sha256"] and sha256(old_input_path) == lock["previous_coretail_input_sha256"] and sha256(q4_lock_path) == lock["previous_streamq4_lock_sha256"] and sha256(q4_input_path) == lock["previous_streamq4_input_sha256"] and sha256(q5_lock_path) == lock["previous_streamq5_lock_sha256"] and sha256(q5_input_path) == lock["previous_streamq5_input_sha256"])
    evaluator_source = evaluator.read_text(encoding="utf-8")
    audit.check("physical scale evaluator implementation", "stored_scale = scale.to(torch.bfloat16).float()" in evaluator_source and "quantized * stored_scale" in evaluator_source and evaluator_source.index("quantized = torch.round(work / scale)") < evaluator_source.index("stored_scale = scale.to(torch.bfloat16).float()"))
    ...
    old_tensors = load_file(old_input_path); q4_tensors = load_file(q4_input_path); q5_tensors = load_file(q5_input_path)
    prior_disjoint = True
    for domain in DOMAINS:
        current_rows = list(tensors[f"validation_{domain}"]) + list(tensors[f"test_{domain}"])
        prior_rows = list(old_tensors[f"validation_{domain}"]) + list(old_tensors[f"test_{domain}"]) + list(q4_tensors[f"validation_{domain}"]) + list(q4_tensors[f"test_{domain}"]) + list(q5_tensors[f"validation_{domain}"]) + list(q5_tensors[f"test_{domain}"])
        prior_disjoint &= not any((left == right).all().item() for left in current_rows for right in prior_rows)
    audit.check("prior decision-context disjoint", prior_disjoint and lock["exact_context_disjoint_from_prior_decision_sets"] is True)
    ...
    for split, result in (("validation", validation), ("test", test)):
        expected_inputs = {
            ...
            "evaluator_lock_sha256": sha256(evaluator_lock_path),
            "evaluator_sha256": sha256(evaluator),
            "previous_coretail_lock_sha256": sha256(old_lock_path),
            "previous_coretail_input_sha256": sha256(old_input_path),
            "previous_streamq4_lock_sha256": sha256(q4_lock_path),
            "previous_streamq4_input_sha256": sha256(q4_input_path),
            "previous_streamq5_lock_sha256": sha256(q5_lock_path),
            "previous_streamq5_input_sha256": sha256(q5_input_path),
    ...
    status = "p0c_quality_verification_pass" if passed == total else "p0c_quality_verification_fail"
    payload = {
        "kind": "streamq5_moe_p0c_independent_verification",
        "status": status,
        ...
        "independent_recomputation": [
            ...
            "preregistered validation progression and P0 quality decision",
        ],
        "primary_test_relative_ce": test["variants"][PRIMARY]["relative_cross_entropy_increase"],
        "primary_validation_relative_ce": validation["variants"][PRIMARY]["relative_cross_entropy_increase"],
        "conclusion": "P0C quality and exact BF16-scale evaluator semantics pass independently; corrected route, physical bank, kernel, transfer, and wall-clock phases remain required.",
    }
    output = REPORT_DIR / "p0c_model_quality_verification.json"
    output.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    report = REPORT_DIR / "P0C_MODEL_QUALITY_VERIFICATION.md"
    report.write_text(
        "\n".join(
            [
                "# STREAMQ5-MoE P0C - onafhankelijke fysieke-schaal-verificatie",
                "",
                f"Uitkomst: **{status}** ({passed}/{total} controles).",
                "",
                f"De primaire testtoename in cross-entropy is **{payload['primary_test_relative_ce']:.3%}**.",
                f"Validation meet {payload['primary_validation_relative_ce']:.3%}; beide splits liggen onder de gelockte 2%-gate.",
                "Dit opent uitsluitend gecorrigeerde route-, fysieke bank-, cache- en kernelfasen; P0C bewijst geen wall-clock.",
                "",
                "Deze audit herberekent hashes, inputlocks, metriekrekenkunde, laagvolledigheid, fysieke BF16-schaalcode en de beslisregel onafhankelijk van de evaluator.",
                "",
            ]
        ),
        encoding="utf-8",
    ...
    if status != "p0c_quality_verification_pass":
        raise SystemExit(1)
