...
ROOT=Path(__file__).resolve().parents[2]; R=ROOT/'reports/streamq5_moe'; D=ROOT/'reports/runs/streamq5_moe/het_next_cap0_dual_device_cohabitation'
RESULT=D/'cap0_result.json'; COMMIT=D/'cap0_commit.json'; FAILURE=D/'cap0_failure.json'; VLOCK=R/'het_next_cap0_verifier_lock.json'
...
def verify():
 ...
 checks['kind_status']=r.get('kind')=='het_next_cap0_dual_device_cohabitation' and r.get('status')=='dual_device_cohabitation_positive' and r.get('claim_boundary')=='4KiB dual-device lifecycle/correctness only; no performance or model claim'
 ...
 checks['exact_outputs']=checks['cardinality'] and all(z.get('different_words')==0 and z.get('output_sha256')==FIX[k] and z.get('expected_sha256')==FIX[k] and z.get('done_qpc_ns',0)>z.get('submit_qpc_ns',0) for k in ('intel','nvidia') for z in reps[k]) and len({z['output_sha256'] for z in reps['intel']})==1 and len({z['output_sha256'] for z in reps['nvidia']})==1
