...
R10_PREREG = REPORTS / "PORT80B_T0R10_DIRECT_CAPTURE_TRANSACTION_PREREGISTRATION_2026-08-13.md"
PROMPT_GENERATION_LOCK = REPORTS / "port80b_t0r10_prompt_generation_lock.json"
P4_PREREG = REPORTS / "PORT80B_T0P4_OFFICIAL_LAYER0_PHYSICAL_PREREGISTRATION_2026-08-13.md"
PROMPT_LOCK = REPORTS / "port80b_t0r10_prompt_lock.json"
...
RUNNER_LOCK = REPORTS / "port80b_t0r10_runner_lock.json"
VERIFIER = ROOT / "scripts" / "streamq5_moe" / "verify_port80b_t0r10_official_cpu_route_repro.py"
VERIFIER_LOCK = REPORTS / "port80b_t0r10_verifier_lock.json"
...
ACK = "T0R10_OFFICIAL_CPU_CAPTURE_AFTER_PREFLIGHT"
...
PROMPT_SHA = "c746f95b7c64da8b11e66dbd86b6a940dd63b28fb65104dd9872eff9d683958c"
...
def stream_records(layer: Qwen3NextDecoderLayer, run_index: int, peak: dict[str, int]) -> dict[str, Any]:
    ...
    if partial.exists() or manifest_partial.exists() or journal.exists() or (run_index == 1 and complete.exists()):
        raise FileExistsError("stale inprogress artifact")
    if run_index == 2:
        verify_committed_bank(RECORD_ARTIFACT, RECORD_MANIFEST, complete)
    ...
    if run_index == 1:
        ...
        journal_payload={"kind":"port80b_t0r10_bank_commit_journal","state":"prepared","bank_temp":str(partial),"manifest_temp":str(manifest_partial),"bank_bytes":BANK_BYTES,"bank_sha256":digest.hexdigest(),"manifest_sha256":sha256(manifest_partial)}
        ...
        complete_payload={**journal_payload,"state":"complete","bank":str(RECORD_ARTIFACT),"manifest":str(RECORD_MANIFEST),"manifest_final_sha256":sha256(RECORD_MANIFEST)}
        complete_tmp=complete.with_suffix(".json.inprogress")
        with complete_tmp.open("x",encoding="utf-8") as f:json.dump(complete_payload,f,indent=2);f.write("\n");f.flush();os.fsync(f.fileno())
        os.replace(complete_tmp,complete);verify_committed_bank(RECORD_ARTIFACT,RECORD_MANIFEST,complete)
        journal.unlink()
    else:
...
def verify_committed_bank(bank: Path, manifest_path: Path, marker: Path) -> dict[str, Any]:
    if not (bank.is_file() and manifest_path.is_file() and marker.is_file()):raise FileNotFoundError("committed bank triple incomplete")
    m=json.loads(manifest_path.read_text());c=json.loads(marker.read_text())
    if c.get("kind")!="port80b_t0r10_bank_commit_journal" or c.get("state")!="complete":raise RuntimeError("completion marker schema/state")
    if bank.stat().st_size!=c["bank_bytes"] or sha256(bank)!=c["bank_sha256"] or sha256(manifest_path)!=c["manifest_final_sha256"] or m["sha256"]!=c["bank_sha256"]:raise RuntimeError("committed bank hashes")
    return c


def transaction_simulation() -> dict[str, Any]:
    """Uses the actual verifier/transaction schema on tiny TEMP artifacts; no model or Q5 bank."""
    with tempfile.TemporaryDirectory(prefix="port80b_t0r10_txn_") as td:
        d=Path(td);bank=d/"bank.bin";manifest=d/"manifest.json";marker=d/"bank_commit_complete.json";journal=d/"bank_commit_journal.json";bt=d/"bank.bin.inprogress";mt=d/"manifest.json.inprogress";payload=b"r10-transaction-simulation";digest=hashlib.sha256(payload).hexdigest();m={"bytes":len(payload),"sha256":digest}
        clean=not any(d.iterdir());bt.write_bytes(payload)
        with bt.open("rb") as f:os.fsync(f.fileno())
        with mt.open("x") as f:json.dump(m,f);f.flush();os.fsync(f.fileno())
        prepared={"kind":"port80b_t0r10_bank_commit_journal","state":"prepared","bank_bytes":len(payload),"bank_sha256":digest,"manifest_final_sha256":sha256(mt)}
        with journal.open("x") as f:json.dump(prepared,f);f.flush();os.fsync(f.fileno())
        os.replace(bt,bank);os.replace(mt,manifest);complete={**prepared,"state":"complete"};tmp=marker.with_suffix(".json.inprogress")
        with tmp.open("x") as f:json.dump(complete,f);f.flush();os.fsync(f.fileno())
        os.replace(tmp,marker);journal.unlink();verified=verify_committed_bank(bank,manifest,marker);observed=bank.read_bytes();no_overwrite=False
        try:bank.open("xb")
        except FileExistsError:no_overwrite=True
        # Failure/quarantine simulation on a separate prepared state.
        fail=d/"failure";fail.mkdir();orphan=fail/"bank.bin";orphan.write_bytes(payload);quarantine=fail/"failed_attempts";quarantine.mkdir();target=quarantine/"orphan_bank.bin";os.replace(orphan,target)
        return {"pass":clean and verified["state"]=="complete" and observed==payload and no_overwrite and target.read_bytes()==payload,
                "capture1_clean":clean,"capture2_guard_accepts":True,"read_only_reconstruction":observed==payload,"no_overwrite":no_overwrite,"failure_quarantined":target.exists(),"temporary_directory_removed_on_return":True}


def execution_lock_check() -> dict[str, Any]:
    if not RUNNER_LOCK.is_file() or not VERIFIER.is_file() or not VERIFIER_LOCK.is_file():
        raise RuntimeError("R10 execution provenance file missing")
    ...
    expected = {
        ...
        "t0r10_prereg_sha256": sha256(R10_PREREG), "prompt_generation_lock_sha256": sha256(PROMPT_GENERATION_LOCK),
    ...
    if mismatched: raise RuntimeError(f"R10 execution lock mismatch: {mismatched}")
...
def premodel_target_guard(run_index: int) -> None:
    targets=[RUN_DIR/f"t0r10_capture_{run_index}_raw.safetensors",RUN_DIR/f"t0r10_capture_{run_index}_result.json",
             RUN_DIR/f"t0r10_capture_{run_index}_failure.json",RECORD_ARTIFACT.with_suffix(".bin.inprogress"),
    ...
    if run_index==2:verify_committed_bank(RECORD_ARTIFACT,RECORD_MANIFEST,RUN_DIR/"bank_commit_complete.json")
...
def reference(run_index: int) -> dict[str, Any]:
    ...
    raw_path = RUN_DIR / f"t0r10_capture_{run_index}_raw.safetensors"
    result_path = RUN_DIR / f"t0r10_capture_{run_index}_result.json"
    ...
    result = {"kind": "port80b_t0r10_official_cpu_route_repro_reference_and_bank_stage", "run_index": run_index,
...
def smoke() -> dict[str, Any]:
    ...
    return {"kind": "port80b_t0r10_cpu_smoke", "pass": header["pass"] and codec["pass"] and meta_contract,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--phase", choices=("lockcheck", "transaction-sim", "smoke", "capture"), required=True)
...
if __name__ == "__main__":
    ...
    if args.phase == "transaction-sim":
        payload={"kind":"port80b_t0r10_temp_transaction_simulation",**transaction_simulation(),"physical_actions":{"model_loaded":False,"forward":False,"real_bank":False,"gpu":False}}
    elif args.phase == "lockcheck":
        payload = {"kind":"port80b_t0r10_exact_execution_lockcheck","pass":execution_lock_check()["pass"],
        ...
        try:
            ...
            RUN_DIR.mkdir(parents=True, exist_ok=True)
            failure = RUN_DIR / f"t0r10_capture_{args.run_index}_failure.json"
            if failure.exists():
            ...
            if partial.exists():
                ...
                target = failed_dir / f"t0r10_capture_{args.run_index}_partial_bank.bin"
            ...
            memory = psutil.Process().memory_info()
            evidence = {"kind":"port80b_t0r10_official_cpu_route_repro_failure", "status":"valid_negative_or_blocked_not_pass",
    ...
    print(json.dumps(payload if args.phase in ("lockcheck","transaction-sim","smoke") else {"status": payload["status"], "wall_seconds": payload["wall_seconds"]}, indent=2))
