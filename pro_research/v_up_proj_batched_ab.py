"""Causal A/B/A/CTL for batched up-proj ERVF GEMV (gemv_nvfp4_ervf_ind),
isolating this ONE addition on top of the already-verified V5 down_proj
batching (panel_scan/reduce_partials/accumulate). Both BASE_A and BATCHED
arms include V5's down_proj batching -- the single variable under test here
is up_kernels on/off, per the one-variable-per-measurement rule.

Step 1 (verify_up_proj_batch_kernel.py, isolated kernel unit test) passed
bit-exact before this file was run.
"""
...
OUT = result_path("PRO_UP_PROJ_BATCHED_AB.json")
...
def main() -> int:
    ...
    payload: dict[str, Any] = {
        "kind": "up_proj_batched_ab",
        "status": "started",
        "mode": args.mode,
        "started_utc": utc_now(),
        "scope": "isolates batched gemv_nvfp4_ervf_ind on top of already-verified V5 down_proj batching; single variable = up_kernels on/off",
    }
    ...
    try:
        ...
        def run_arm(with_up: bool):
            restore = install_batched_moe_dev(rt, batch_kernels, up_kernels if with_up else None)
            ids_by_prompt = {}
            ms = []
            for p in prompts:
        ...
        base_a_ids, base_a_ms = run_arm(with_up=False)
        batched_ids, batched_ms = run_arm(with_up=True)
        base_b_ids, base_b_ms = run_arm(with_up=False)

        restore_ctl = install_batched_moe_dev(rt, batch_kernels, up_kernels)
        rt._bad_pick = 1
        ctl_n = min(n, 64)
        ctl = {}
        for p in prompts:
        ...
        gates = {
            ...
            "candidate_gain_ge_0_5ms_or_2pct": bool(gain_ms is not None and (gain_ms >= 0.5 or (gain_fraction or 0) >= 0.02)),
        ...
        passed = (correctness and gates["candidate_gain_ge_0_5ms_or_2pct"] and gates["full_samples_ge_500"]) if args.mode == "full" else correctness

        payload.update({
            "arms": {"BASE_A_down_only": {"timing_ms": pa}, "BATCHED_down_and_up": {"timing_ms": pbatch}, "BASE_B_down_only": {"timing_ms": pb}},
            "parity": parity,
            "control": ctl,
            "gates": gates,
            "summary": {
        ...
        del rt, batch_kernels, up_kernels
        gc.collect()
        import cupy as cp
