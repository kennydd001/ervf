#!/usr/bin/env python3
"""Closed, CPU-only static PH1 NVIDIA N5 source preflight."""
...
ROOT=Path(__file__).resolve().parents[2];R=ROOT/"reports/streamq5_moe";S=ROOT/"scripts/streamq5_moe";LOCK=R/"het_next_l0_ph1_nvidia_n5_preflight_lock.json";OUT=R/"het_next_l0_ph1_nvidia_n5_static_preflight.json"
tx=None;BIG_BINDINGS={"d2_raw","official_shard1","cpu_stages","cpu_lut"};READ_COUNTER={"payload_bytes":0,"metadata_bytes":0,"blocked_payload_opens":0};PAYLOAD_PATHS=set()
FILES=(S/"het_next_l0_ph1_nvidia_n5_common.py",S/"het_next_l0_ph1_nvidia_n5_kernels.cu",S/"het_next_l0_ph1_nvidia_n5_backend.py",S/"het_next_l0_ph1_nvidia_n5_transaction.py",S/"run_het_next_l0_ph1_nvidia_n5.py",S/"verify_het_next_l0_ph1_nvidia_n5.py",Path(__file__))
ACK="ACK-PH1-NVIDIA-N5-STATIC-PREFLIGHT"
ABI_SOURCE={
...
def fsha(p):
 path=Path(p).resolve()
 if path in PAYLOAD_PATHS:READ_COUNTER["blocked_payload_opens"]+=1;raise PermissionError("payload_open_forbidden")
 data=path.read_bytes();READ_COUNTER["metadata_bytes"]+=len(data);return hashlib.sha256(data).hexdigest()
...
def bootstrap_contract_modules():
 global tx
 tx=load_absolute(S/"het_next_l0_ph1_nvidia_n5_transaction.py","het_next_l0_ph1_nvidia_n5_transaction")
 return load_absolute(S/"het_next_l0_ph1_nvidia_n5_common.py","het_next_l0_ph1_nvidia_n5_common")
...
def imports(tree):
 ...
 return out
def hash_lock():
 lock=json.loads(LOCK.read_text());ok=lock.get("kind")=="het_next_l0_ph1_nvidia_n5_preflight_lock"
 for name,item in lock.get("bindings",{}).items():
  p=Path(item["path"]);p=p if p.is_absolute() else ROOT/p
  if name in BIG_BINDINGS:
   PAYLOAD_PATHS.add(p.resolve());ok=ok and p.is_absolute() and p.is_file() and p.stat().st_size==item["bytes"] and len(item["sha256"])==64
  else:ok=ok and p.is_file() and fsha(p)==item["sha256"]
 return lock,ok and READ_COUNTER["payload_bytes"]==0
def payload_guard_mutation():
 before=READ_COUNTER["blocked_payload_opens"]
 try:fsha(next(iter(PAYLOAD_PATHS)));return False
 except PermissionError:return READ_COUNTER["blocked_payload_opens"]==before+1 and READ_COUNTER["payload_bytes"]==0
...
def model_path_contract(src,lock):
 expected=r"C:/Users/de_do/.cache/huggingface/hub/models--Qwen--Qwen3-Coder-Next/snapshots/a19358a7659bd1f564300250ee189120c49a562f/model-00001-of-00040.safetensors"
 texts=(src["het_next_l0_ph1_nvidia_n5_common.py"],src["verify_het_next_l0_ph1_nvidia_n5.py"])
 bound=lock["bindings"]["official_shard1"]["path"].replace("\\","/")
 baseline=bound==expected and all(t.count(expected)==1 and "Qwen5-Coder" not in t and "Qwen4-Coder" not in t for t in texts)
 return baseline and all(not all(x.count(expected)==1 for x in (t.replace("Qwen3-Coder","Qwen5-Coder",1),texts[1])) for t in texts[:1])
...
def source_contract(src):
 backend=src["het_next_l0_ph1_nvidia_n5_backend.py"];runner=src["run_het_next_l0_ph1_nvidia_n5.py"];verifier=src["verify_het_next_l0_ph1_nvidia_n5.py"]
 ...
 independent=not ({"het_next_l0_ph1_nvidia_n5_common","het_next_l0_ph1_nvidia_n5_backend","run_het_next_l0_ph1_nvidia_n5","het_next_l0_ph1_nvidia_n5_transaction"}&imports(ast.parse(verifier)))
...
def forbidden_surface(src):
 tree=ast.parse(src[Path(__file__).name]);bad_import={"torch","transformers","safetensors","cupy","pycuda"};bad_call={"WinDLL","CDLL","LoadLibrary","Popen","system","cuInit","nvrtcCompileProgram","from_pretrained"}
 ...
 if len(runs)!=1:return False
 call=runs[0];keys={k.arg for k in call.keywords};argv=ast.unparse(call.args[0])
 exact=(argv=='[sys.executable, \'-I\', \'-B\', str(child.resolve())]' and keys=={"cwd","stdin","capture_output","timeout","check","shell"})
 return exact and not(bad_import&imports(tree)) and not(bad_call&calls(tree))
def transaction_simulation():
 with tempfile.TemporaryDirectory(prefix="ph1_nvidia_n5_static_") as td:
...
def verifier_mutations():
 verify=load_inert(S/"verify_het_next_l0_ph1_nvidia_n5.py","ph1_n5_verify_static")
 ...
 with tempfile.TemporaryDirectory(prefix="ph1_n5_verifier_paths_") as td:
...
def cleanup_faults():
 backend=load_inert(S/"het_next_l0_ph1_nvidia_n5_backend.py","ph1_n5_backend_static")
 class Fake:
 ...
 return True
def topology():
 targets=(R/"het_next_l0_ph1_nvidia_n5_compile",R/"het_next_l0_ph1_nvidia_n5_physical",R/"het_next_l0_ph1_nvidia_n5_compile_failures",R/"het_next_l0_ph1_nvidia_n5_physical_failures",R/"het_next_l0_ph1_nvidia_n5_quarantine",OUT)
 return all(not p.exists() for p in targets) and not any(R.glob("het_next_l0_ph1_nvidia_n5*.inprogress*"))
def isolated_suite():
...
def main():
 ...
 child=S/"het_next_l0_ph1_nvidia_n5_fixture_child.py";isolated=subprocess.run([sys.executable,"-I","-B",str(child.resolve())],cwd=str(ROOT),stdin=subprocess.DEVNULL,capture_output=True,timeout=120,check=False,shell=False);isolated_row=json.loads(isolated.stdout) if isolated.returncode==0 else {}
 src=sources();simulation=transaction_simulation();checks={"hash_bindings":hashes,"output_absent":topology(),"payload_guard":payload_guard_mutation(),"model_path":model_path_contract(src,lock),"kernel_contract":kernel_contract(src["het_next_l0_ph1_nvidia_n5_kernels.cu"]),"source_contract":source_contract(src),"abi_ast_exact":abi_contract(src["het_next_l0_ph1_nvidia_n5_backend.py"]),"abi_mutations":abi_mutations(src["het_next_l0_ph1_nvidia_n5_backend.py"]),"schedule_ast":schedule_contract(src["het_next_l0_ph1_nvidia_n5_backend.py"]),"forbidden_surface":forbidden_surface(src),"transaction_simulation":simulation["pass"],"transaction_details":all(simulation.values()),"isolated_production_fixtures":isolated_row=={"cleanup_faults":True,"verifier_mutations":True,"payload_bytes_read":0},"lock_closed_physical":lock.get("compiler_open") is False and lock.get("physical_open") is False,"audit_binding":lock.get("bindings",{}).get("n4_source_audit",{}).get("sha256")=="98f1965a7cc817b9ce4fe6a8e8ddd475b98b2cb62f92d2921e3e102fc404c320","cardinality_literals":all(x in "\n".join(src.values()) for x in ("2_185_216","675840","post_ordinary_releases_pre_pop","range(30)","len(rows) != 22"))}
 result={"kind":"het_next_l0_ph1_nvidia_n5_static_preflight","checks":checks,"pass":all(checks.values()),"passed":sum(checks.values()),"total":len(checks),"no_payload":True,"no_compiler":True,"no_device":True}
