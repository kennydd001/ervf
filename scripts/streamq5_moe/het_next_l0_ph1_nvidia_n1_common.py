#!/usr/bin/env python3
"""CPU-only PH1 NVIDIA N1 package, codec, oracle, controls and contracts.
...
def safe_controls(records, input_bytes: bytes, lut: bytes):
    ...
    for spec in SPECS:
        ...
        for control, presented, input_digest, expected in cases:
            observed = check_record(presented, spec, input_digest)
            rows.append({"record": name, "control": control, "expected": expected, "observed": observed, "pass": observed == expected, "predevice_counts": dict(zero)})
    wrong = bytearray(lut); wrong[0] ^= 1; observed = "lut_digest" if sha(wrong) != LUT_SHA else "pass"
    rows.append({"record": "global", "control": "wrong_lut_digest", "expected": "lut_digest", "observed": observed, "pass": observed == "lut_digest", "presented_sha256": sha(wrong), "expected_sha256": LUT_SHA, "predevice_counts": dict(zero)})
