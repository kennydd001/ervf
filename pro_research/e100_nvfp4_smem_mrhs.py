"""E100-NVFP4-SMEM-MRHS: exact shared-decode MRHS component benchmark."""
...
from nvfp4_smem_mrhs_kernels import ExactNVFP4SmemMRHS

RESULT_DIR = REPO / "pro_research" / "results" / "e100_nvfp4_smem_mrhs"
OUT = RESULT_DIR / "PRO_E100_NVFP4_SMEM_MRHS.json"
PREREG = REPO / "pro_research" / "E100_NVFP4_SMEM_MRHS_PREREGISTRATION.md"
...
def main() -> int:
    ...
    args = ap.parse_args()
    payload = {
        "kind": "pro_e100_nvfp4_smem_mrhs",
        "mode": args.mode,
        "status": "started",
        "started_utc": utc_now(),
        "preregistration": str(PREREG.relative_to(REPO)),
        "claim_boundary": "exact NVFP4 shared-decode multi-RHS component; not a full-model E100 claim",
    }
    rt = k = adopted = None
    try:
        require_gpu_free()
        payload["environment_start"] = environment_snapshot((
            ...
            REPO / "pro_research" / "nvfp4_smem_mrhs_kernels.py",
        ...
        for n in ns:
            for case in cases:
                ...
                r = bench_case_adopted(
                    ...
                    "nvfp4_smem_decode_mrhs", correctness_batches, repeats, rounds,
                )
                records.append(r)
                print(
                    f"SMEM-MRHS N={n:>2} {case.name:<22} exact={r['all_exact']} "
        ...
        full_perf = None
        if args.mode == "full":
            full_perf = all(v is True for v in perf.values())
