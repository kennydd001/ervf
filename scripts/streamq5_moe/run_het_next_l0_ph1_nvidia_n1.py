#!/usr/bin/env python3
"""Closed standalone PH1 NVIDIA N1 compile/physical runner."""
...
import het_next_l0_ph1_nvidia_n1_common as common
from het_next_l0_ph1_nvidia_n1_backend import CompilerFailure, DriverBackend, DriverFailure, CUDA_SOURCE, OPTIONS, compile_one_program, file_sha
from het_next_l0_ph1_nvidia_n1_transaction import atomic_failure, canonical, clean_or_quarantine, publish_bundle, verify_bundle

ROOT = Path(__file__).resolve().parents[2]
REPORTS = ROOT / "reports/streamq5_moe"
LOCK = REPORTS / "het_next_l0_ph1_nvidia_n1_source_lock.json"
VERIFY = ROOT / "scripts/streamq5_moe/verify_het_next_l0_ph1_nvidia_n1.py"
COMPILE_OUT = REPORTS / "het_next_l0_ph1_nvidia_n1_compile"
PHYSICAL_OUT = REPORTS / "het_next_l0_ph1_nvidia_n1_physical"
COMPILE_FAILURE = REPORTS / "het_next_l0_ph1_nvidia_n1_compile_failures"
PHYSICAL_FAILURE = REPORTS / "het_next_l0_ph1_nvidia_n1_physical_failures"
QUARANTINE = REPORTS / "het_next_l0_ph1_nvidia_n1_quarantine"
COMPILE_KIND = "het_next_l0_ph1_nvidia_n1_compile"
PHYSICAL_KIND = "het_next_l0_ph1_nvidia_n1_physical"
ACK = {"compile": "ACK-PH1-NVIDIA-N1-COMPILE-ONE-PROGRAM", "physical": "ACK-PH1-NVIDIA-N1-PHYSICAL-ONE-ATTEMPT"}


def validate_authorization(phase: str, acknowledgement: str):
    lock = json.loads(LOCK.read_text(encoding="utf-8"))
    if lock.get("kind") != "het_next_l0_ph1_nvidia_n1_source_lock" or lock.get("revision") != "N1":
...
def disassemble(cubin: bytes, lock):
    ...
    outputs = {}
    with tempfile.TemporaryDirectory(prefix="ph1_nvidia_n1_cubin_") as directory:
...
def physical_phase(lock, observed):
    ...
    try:
        ...
        gates = {"stages_exact": all(stage_equal.values()), "counters_exact": all(counter_equal.values()), "finite": finite, "controls": len(package["controls"]) == 22 and all(row["pass"] for row in package["controls"]), "resources": resource_ok, "cleanup": evidence["cleanup_errors"] == [] and evidence["live_owned_resources"] == 0 and evidence["primary_released"] is True, "forbidden": evidence["cudart_loaded"] is False and all(value == 0 for value in evidence["forbidden_calls"].values())}
        positive = all(gates.values()); status = "nvidia_physical_positive" if positive else "nvidia_device_numerical_negative"
        ...
        partial = exc.evidence if isinstance(exc, DriverFailure) else {}
...
def main():
    ...
    # Authorization precedes recovery, payload, compiler and Driver operations.
