...
FILES = {
 ...
 R/"port80b_t0r4r1_runner_lock.json": "LOCK_HASH_SELF_EXCLUDED",
 ROOT/"scripts/streamq5_moe/run_port80b_t0r4r1_official_layer0_reference.py": "007c572768f5af0c03c648df151bd45d84c5fcdb51b957fe6bcb8862d357fbfe",
...
RUNNER = ROOT/"scripts/streamq5_moe/run_port80b_t0r4r1_official_layer0_reference.py"
OUT = R/"port80b_t0r4r1_ref_cpu_preflight.json"
...
def main():
 ...
 ck("runner_lock_binds_runner",json.loads((R/"port80b_t0r4r1_runner_lock.json").read_text())["runner_sha256"]==sha(RUNNER))
 ...
 required=["USE_HUB_KERNELS\"] = \"0", "torch.set_num_interop_threads(1)", "materialize=False",
           ...
           "max_bf16_ulp", "projected > MAX_PROJECTED_STEADY", "windows_peak_working_set_bytes",
           "top11[:, 9] - top11[:, 10]", "process.cpu_affinity(wanted_affinity)",
           "packed gate/up byte mismatch", "verify_prompt_lock_before_shard", "decoded_fields.max() > 30", "all-zero codes 0"]
 ...
 result={"kind":"port80b_t0r4r1_ref_cpu_preflight","pass":not fail,"status":"r4r1_ref_cpu_preflight_pass_execution_still_closed" if not fail else "blocked","checks_passed":len(checks)-len(fail),"checks_total":len(checks),"blocked_reasons":[x["name"] for x in fail],"checks":checks,"preflight_sha256":sha(Path(__file__)),"runner_lock_sha256":sha(R/"port80b_t0r4r1_runner_lock.json"),"claim_boundary":"No-forward CPU provenance/source/meta-only smoke. No official tensor payload loaded into a model, no model forward, bank build, GPU, registration or registry edit. R4-Q5 and independent verifier remain unwritten/closed."}
