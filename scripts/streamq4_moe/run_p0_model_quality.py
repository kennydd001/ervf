...
PREREG = ROOT / "reports/streamq4_moe/P0_MODEL_QUALITY_PREREGISTRATION.md"
LOCK = ROOT / "reports/streamq4_moe/p0_input_lock.json"
EVALUATOR_LOCK = ROOT / "reports/streamq4_moe/p0_evaluator_lock.json"
...
VARIANTS = (
    "bf16_teacher",
    "q4_experts_bf16_trunk",
    "bf16_experts_int8_trunk",
    "q4_experts_int8_trunk",
    "q4_experts_int4_trunk",
)
CONTEXT = 128
EXPERT_BATCH = 8
...
@torch.no_grad()
def quantize_experts_q4_(layer) -> None:
    """Symmetric group-128 RTN Q4, batched without retaining a second expert bank."""
    for projection in ("gate_proj", "up_proj", "down_proj"):
        for start in range(0, len(layer.mlp.experts), EXPERT_BATCH):
            ...
            quantize_groupwise_(work, 4, row_batch=work.shape[0])
...
if __name__ == "__main__":
    args = parse_args()
    output = ROOT / f"reports/streamq4_moe/p0_{args.split}_model_quality.json"
    report = ROOT / f"reports/streamq4_moe/P0_{args.split.upper()}_MODEL_QUALITY.md"
    if output.exists() or report.exists():
        raise FileExistsError(f"refusing to overwrite STREAMQ4 P0 {args.split}")
    validation_result = ROOT / "reports/streamq4_moe/p0_validation_model_quality.json"
    if args.split == "test" and not validation_result.is_file():
        raise RuntimeError("STREAMQ4 validation must finish before test opens")
    ...
    if sha256(Path(__file__)) != evaluator_lock["evaluator_sha256"] or sha256(LOCK) != evaluator_lock["input_lock_sha256"]:
        raise ValueError("STREAMQ4 evaluator lock mismatch")
    if sha256(PREREG) != lock["preregistration_sha256"]:
        raise ValueError("STREAMQ4 preregistration hash mismatch")
    ...
    if sha256(input_path) != lock["artifact_sha256"] or sha256(MODEL / "model.safetensors.index.json") != lock["model_index_sha256"]:
        raise ValueError("STREAMQ4 input or model hash mismatch")
    ...
    hidden = {
        ...
        "q4_experts_bf16_trunk": bf16_embedding.clone(),
        "bf16_experts_int8_trunk": int8_embedding,
        "q4_experts_int8_trunk": int8_embedding.clone(),
        "q4_experts_int4_trunk": int4_embedding,
    }
    del bf16_embedding
    ...
    for layer_index in range(48):
        ...
        quantize_experts_q4_(layer)
        hidden["q4_experts_bf16_trunk"] = forward_layer(layer, rotary, hidden["q4_experts_bf16_trunk"], device)
        quantize_trunk_(layer, 8)
        hidden["q4_experts_int8_trunk"] = forward_layer(layer, rotary, hidden["q4_experts_int8_trunk"], device)
        restore_trunk_(layer, trunk_original)
        quantize_trunk_(layer, 4)
        hidden["q4_experts_int4_trunk"] = forward_layer(layer, rotary, hidden["q4_experts_int4_trunk"], device)
    ...
    q4_bf16_metrics, _ = head_metrics(hidden["q4_experts_bf16_trunk"], input_ids, norm_weight, head, device, teacher_top1)
    variant_metrics["q4_experts_bf16_trunk"] = q4_bf16_metrics
    ...
    q4_int8_metrics, _ = head_metrics(hidden["q4_experts_int8_trunk"], input_ids, norm_weight, head, device, teacher_top1)
    variant_metrics["bf16_experts_int8_trunk"] = bf16_int8_metrics
    variant_metrics["q4_experts_int8_trunk"] = q4_int8_metrics
    head.copy_(head_original); quantize_groupwise_(head, 4, row_batch=256)
    q4_int4_metrics, _ = head_metrics(hidden["q4_experts_int4_trunk"], input_ids, norm_weight, head, device, teacher_top1)
    variant_metrics["q4_experts_int4_trunk"] = q4_int4_metrics
    ...
    primary_relative = variant_metrics["q4_experts_int8_trunk"]["relative_cross_entropy_increase"]
    finite = all(
    ...
    if args.split == "validation":
        status = "p0_validation_pass_test_authorized" if finite and primary_relative <= 0.03 else "p0_validation_closed"
        p0_pass = False
    else:
        validation = json.loads(validation_result.read_text(encoding="utf-8"))
        if validation.get("status") != "p0_validation_pass_test_authorized":
            raise RuntimeError("test was not authorized by STREAMQ4 validation")
        validation_relative = validation["variants"]["q4_experts_int8_trunk"]["relative_cross_entropy_increase"]
        if finite and validation_relative <= 0.02 and primary_relative <= 0.02:
            status, p0_pass = "p0_quality_pass", True
        else:
            status, p0_pass = "p0_quality_closed", False
    payload = {
        "kind": "streamq4_moe_p0_full_depth_model_quality",
        "completed_utc": datetime.now(timezone.utc).isoformat(),
        "started_utc": started_utc,
        "status": status, "split": args.split,
        "inputs": {"preregistration_sha256": sha256(PREREG), "lock_sha256": sha256(LOCK), "evaluator_lock_sha256": sha256(EVALUATOR_LOCK), "evaluator_sha256": sha256(Path(__file__)), "input_artifact_sha256": sha256(input_path), "previous_coretail_lock_sha256": sha256(OLD_LOCK), "previous_coretail_input_sha256": sha256(OLD_INPUT), "model_index_sha256": sha256(MODEL / "model.safetensors.index.json")},
        "data": {"domains": list(DOMAINS), "contexts_per_domain": 2, "context_tokens": CONTEXT, "labels": teacher_metrics["labels"]},
        "variants": variant_metrics,
        "layer_reports": layer_reports,
        "controls": {"all_finite": finite, "all_48_layers": len(layer_reports) == 48, "fresh_exact_contexts": lock["exact_context_disjoint_from_coretail_p2"], "test_opened_after_validation": args.split == "validation" or validation_result.is_file(), "primary_variant_fixed": lock["primary_gate"]["variant"] == "q4_experts_int8_trunk"},
        ...
        "claim_boundary": "Full-depth fresh-split quality for fixed RTN-Q4 experts and fixed fake-quantized trunks; no physical bank, cache, kernel, or wall-clock claim.",
    }
    output.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    report.write_text("\n".join([
        f"# STREAMQ4-MoE P0 - {args.split} full-depth kwaliteit", "",
        f"Uitkomst: **{status}**.", "",
        f"BF16 CE: {teacher_ce:.6f}. Q4+BF16 CE: {variant_metrics['q4_experts_bf16_trunk']['next_token_cross_entropy']:.6f} ({variant_metrics['q4_experts_bf16_trunk']['relative_cross_entropy_increase']:.3%}).",
        ...
        f"Q4+INT8 CE: {variant_metrics['q4_experts_int8_trunk']['next_token_cross_entropy']:.6f} ({primary_relative:.3%}); top-1 {variant_metrics['q4_experts_int8_trunk']['top1_agreement_vs_teacher']:.3%}.",
        f"Q4+INT4 CE: {variant_metrics['q4_experts_int4_trunk']['next_token_cross_entropy']:.6f} ({variant_metrics['q4_experts_int4_trunk']['relative_cross_entropy_increase']:.3%}).", "",
