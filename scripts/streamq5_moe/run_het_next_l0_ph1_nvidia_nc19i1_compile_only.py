#!/usr/bin/env python3
"""One-shot NC19I1 NVRTC compile-only runner; import is inert."""
...
CONTRACT_PATH = SCRIPT.with_name("het_next_l0_ph1_nvidia_nc19i1_compile_contract.py")
_SPEC = importlib.util.spec_from_file_location("nc19i1_compile_contract_runtime", CONTRACT_PATH)
...
VERIFIER_LOCK = REPORTS / "het_next_l0_ph1_nvidia_nc19i1_verifier_lock.json"
...
OUT = REPORTS / "het_next_l0_ph1_nvidia_nc19i1_compile_only"
NEGATIVE = REPORTS / "het_next_l0_ph1_nvidia_nc19i1_compile_only_negative"
FAILURES = REPORTS / "het_next_l0_ph1_nvidia_nc19i1_compile_only_incidental_failures"
QUARANTINE = REPORTS / "het_next_l0_ph1_nvidia_nc19i1_compile_only_quarantine"
VERIFY_OUT = REPORTS / "het_next_l0_ph1_nvidia_nc19i1_independent_verification.json"
VERIFY_NEGATIVE = REPORTS / "het_next_l0_ph1_nvidia_nc19i1_independent_verification_negative"
VERIFY_FAILURES = REPORTS / "het_next_l0_ph1_nvidia_nc19i1_independent_verification_failures"
VERIFY_QUARANTINE = REPORTS / "het_next_l0_ph1_nvidia_nc19i1_independent_verification_quarantine"
DURABILITY = REPORTS / "het_next_l0_ph1_nvidia_nc19i1_durability_adjudication"
PREFLIGHT_FAILURES = REPORTS / "het_next_l0_ph1_nvidia_nc19i1_static_preflight_failures"
PREFLIGHT_QUARANTINE = REPORTS / "het_next_l0_ph1_nvidia_nc19i1_static_preflight_quarantine"
INDEPENDENT = ROOT / "scripts/streamq5_moe/verify_het_next_l0_ph1_nvidia_nc19i1_compile_only.py"
ACK = "ACK_HET_NEXT_L0_PH1_NVIDIA_NC19I1_COMPILE_ONLY_ONCE"
KIND = "het_next_l0_ph1_nvidia_nc19i1_compile_only"
...
def validate_authorization(ack: str) -> tuple[bool, dict]:
    ...
    try:
        ...
        checks = (
            lock.get("kind") == "het_next_l0_ph1_nvidia_nc19i1_authorization_bootstrap_lock"
            ...
            and preflight.get("kind") == "het_next_l0_ph1_nvidia_nc19i1_static_preflight_result"
            ...
            and source_lock.get("revision") == "NC19I1" and source_lock.get("compile_open") is True
        )
...
class WinNvrtcAdapter:
    ...
    def load(self):
        ...
        for expected in (NVRTC, BUILTINS):
            ...
            during[expected.name] = {"handle": int(handle or 0), "path": resolved,
        ...
        return {"before": before, "during": during, "module": int(self.module.value or 0),
...
def verify_candidate(directory: Path) -> bool:
    try:
        command = [str(PYTHON.resolve()), "-I", "-B", str(INDEPENDENT.resolve()),
                   ...
                   "--no-write", "--token", "NC19I1_PRECOMMIT_INDEPENDENT_VERIFY"]
...
def execute(authorization: dict) -> int:
    ...
    try:
        ...
        state["stage"]="workspace"; work=REPORTS/f"het_next_l0_ph1_nvidia_nc19i1_compile_work.inprogress.{os.getpid()}.{uuid.uuid4().hex[:16]}"; work.mkdir()
...
def main() -> int:
    ...
    debris=list(REPORTS.glob("het_next_l0_ph1_nvidia_nc19i1_*.inprogress.*"))
    if OUT.exists() and verify_candidate(OUT) and not any(p.exists() for p in terminals[1:]) and not debris:
        return 0
    if debris:
    ...
    try:
        ...
        contract.write_incidental_failure(FAILURES, {
            "kind": "het_next_l0_ph1_nvidia_nc19i1_incidental_failure", "revision": "NC19I1",
