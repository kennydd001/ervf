#!/usr/bin/env python3
"""Independent verifier for R8P8 explicit topology preflight."""
...
import verify_het_next_l0_ph1_intel_execution_r8p7 as prior

SCRIPT = Path(__file__).resolve(); PREFLIGHT = S / "preflight_het_next_l0_ph1_intel_execution_r8p8.py"; LOCK = R / "het_next_l0_ph1_intel_execution_r8p8_lock.json"; PREREG = R / "HET_NEXT_L0_PH1_INTEL_EXECUTION_R8P8_PREREGISTRATION_2026-08-14.md"; AUDIT = R / "HET_NEXT_L0_PH1_INTEL_EXECUTION_R8P7_INDEPENDENT_SOURCE_AUDIT_2026-08-14.md"
RESULT = R / "het_next_l0_ph1_intel_execution_r8p8_static_preflight.json"; MANIFEST = R / "het_next_l0_ph1_intel_execution_r8p8_static_preflight.manifest.json"; COMMIT = R / "het_next_l0_ph1_intel_execution_r8p8_static_preflight.commit.json"; OUTPUT = R / "het_next_l0_ph1_intel_execution_r8p8_independent_verification.json"; FAILED = R / "het_next_l0_ph1_intel_execution_r8p8_failed_attempts"; QUARANTINE = R / "het_next_l0_ph1_intel_execution_r8p8_quarantine"; CORE = (RESULT, MANIFEST, COMMIT); CURRENT_ABSENT = (*CORE, OUTPUT, FAILED, QUARANTINE)
R8P6_FAILURE_ROOT = R / "het_next_l0_ph1_intel_execution_r8p6_failed_attempts"; R8P6_FAILURE = R8P6_FAILURE_ROOT / "attempt_71e198678f004a56a6912d07a4187dfd/failure.json"; R8P6_FAILURE_SHA = "03e48ed76dd848f0c1e993f8452245917115b1b8fb22596871dd933e4758b372"
...
CPU_STATES = set(prior.CPU_STATES); TX_KEYS = set(prior.TX_KEYS); VERIFY_TX_KEYS = set(prior.VERIFY_TX_KEYS); FAILURE_KEYS = set(prior.FAILURE_KEYS); FAILURE_SCHEMA = set(prior.FAILURE_SCHEMA); IDENTITY_KEYS = set(prior.IDENTITY_KEYS); IDENTITY_MUTATION_NAMES = list(prior.IDENTITY_MUTATION_NAMES)
CHECK_NAMES = {"local_entry_identity", "entry_mutations", "hash_bindings", "closed_pending", "runtime_lock", "runtime", "start_ram", "wheel_records", "runtime_mutations", "r7d1_failure", "r8p6_failure", "cpu_preparation", "current_transactions", "failure_simulation", "explicit_topology", "static_boundary", "topology", "base_clean"}
CHAIN = {"preflight_sha256": PREFLIGHT, "verifier_sha256": SCRIPT, "prereg_sha256": PREREG, "r8p7_audit_sha256": AUDIT, "r8p7_lock_sha256": prior.LOCK,
         **{("r8p7_" + k if k in {"preflight_sha256", "verifier_sha256", "prereg_sha256"} else k): v for k, v in prior.CHAIN.items()}}

def revision_group(i: int) -> tuple[Path, ...]:
    stem = f"het_next_l0_ph1_intel_execution_r8p{i}"; core = (R / f"{stem}_static_preflight.json", R / f"{stem}_static_preflight.manifest.json", R / f"{stem}_static_preflight.commit.json"); tail = (R / f"{stem}_independent_verification.json", R / f"{stem}_failed_attempts", R / f"{stem}_quarantine")
    return core + (tail[0], tail[2]) if i == 6 else core + tail
GROUPS = {"base": (R / "het_next_l0_ph1_intel_execution_r8", R / "het_next_l0_ph1_intel_execution_r8_failed_attempts", R / "het_next_l0_ph1_intel_execution_r8_quarantine", R / "het_next_l0_ph1_intel_execution_r8_independent_verification.json", R / "het_next_l0_ph1_intel_execution_r8_static_preflight.json", R / "het_next_l0_ph1_intel_execution_r8p_independent_verification.json"), **{f"r8p{i}": revision_group(i) for i in range(1, 8)}}
...
def state_valid(row: dict) -> bool:
    state = row.get("cpu_slice_state")
    if not isinstance(state, str) or state not in CPU_STATES: return False
    return row.get("cpu_frozen_slice_read_started") is (state != "not_started") and row.get("cpu_frozen_slice_read_completed") is (state == "completed")
...
def ancestor_contract(groups: dict[str, tuple[Path, ...]] = GROUPS) -> bool:
    lengths = {"base": 6, "r8p1": 6, "r8p2": 6, "r8p3": 6, "r8p4": 6, "r8p5": 6, "r8p6": 5, "r8p7": 6}; flat = [p for values in groups.values() for p in values]
    return set(groups) == set(lengths) and all(len(groups[k]) == n for k, n in lengths.items()) and len(flat) == len(set(flat)) == 47 and tuple(flat) == ANCESTOR_ABSENT
def topology_mutations() -> dict:
    out = {"baseline": ancestor_contract()}
    for name in ("r8p1", "r8p2", "r8p3", "r8p4", "r8p5", "r8p6", "r8p7"):
        d = {k: tuple(v) for k, v in GROUPS.items()}; d[name] = d[name][1:]; out["drop_" + name] = not ancestor_contract(d)
        d = {k: tuple(v) for k, v in GROUPS.items()}; d[name] = (*d[name], d[name][0]); out["duplicate_" + name] = not ancestor_contract(d)
    d = {k: tuple(v) for k, v in GROUPS.items()}; d["r8p1"] = d["r8p2"]; out["wrong_chain_depth"] = not ancestor_contract(d); return out
def r8p6_failure_valid() -> bool:
    ...
    row = json.loads(R8P6_FAILURE.read_text()); return set(row) == FAILURE_SCHEMA and row["kind"] == "ph1_intel_execution_r8p6_failure" and row["stage"] == "identity" and row["error"] == "RuntimeError:exact_invocation" and row["device_opened"] is row["compiler_opened"] is False and row["identity"]["direct_entry"] is False
def lock_valid() -> bool:
    lock = json.loads(LOCK.read_text()); observed = {k: sha256(v) for k, v in CHAIN.items()}; return set(lock) == {"kind", "execution_open", "audit_token", *STATIC, *observed} and lock["kind"] == "ph1_intel_execution_r8p8_lock" and lock["execution_open"] is False and lock["audit_token"] == "PENDING" and all(lock.get(k) == v for k, v in STATIC.items()) and all(lock.get(k) == v for k, v in observed.items()) and observed["r8p7_audit_sha256"] == "00de2f823af2a2c1f10dc8aa2239ccdc7de20ffca1cfe07a6bfe6bccf98e03fc"
...
def topology_valid() -> bool:
    expected = {R / "het_next_l0_ph1_intel_execution_r8_lock.json", *(R / f"het_next_l0_ph1_intel_execution_r8p{i}_lock.json" for i in range(1, 9)), R8P6_FAILURE_ROOT, *CORE}; family = {p for p in R.iterdir() if p.name.startswith("het_next_l0_ph1_intel_execution_r8")}
    return family == expected and all(not p.exists() for p in (OUTPUT, FAILED, QUARANTINE)) and not list(R.glob("het_next_l0_ph1_intel_execution_r8*.inprogress.*")) and ancestor_contract() and r8p6_failure_valid()
def stored_topology_valid(x: object) -> bool:
    ...
    return isinstance(x, dict) and set(x) == {"ancestor_absent", "current_absent", "temps", "family"} and set(x["ancestor_absent"]) == {str(p) for p in ANCESTOR_ABSENT} and set(x["current_absent"]) == {str(p) for p in CURRENT_ABSENT} and all(v is False for v in (*x["ancestor_absent"].values(), *x["current_absent"].values())) and x["temps"] == [] and x["family"] == expected
def static_boundary() -> bool:
    tree = ast.parse(PREFLIGHT.read_text(encoding="utf-8")); imports = {n.names[0].name for n in tree.body if isinstance(n, ast.Import)}; required = {f"preflight_het_next_l0_ph1_intel_execution_r8p{i}" for i in range(1, 7)}; ancestor_identity = any(isinstance(n, ast.Call) and isinstance(n.func, ast.Attribute) and n.func.attr == "identity" for n in ast.walk(tree)); forbidden = {"Popen", "run", "CDLL", "LoadLibrary", "clBuildProgram", "cuLaunchKernel", "nvrtcCompileProgram", "from_pretrained"}
    ...
    return required <= imports and not ancestor_identity and prior.static_boundary()

def result_valid(row: object, preparation: dict, wheels: dict) -> bool:
    if not isinstance(row, dict): return False
    checks = row.get("checks", {}); tx = row.get("transaction_simulation"); failure = row.get("failure_simulation"); mutations = row.get("topology_mutations")
    schema = {"kind", "ack", "identity", "pre_run_topology", "topology_mutations", "checks", "pass", "passed", "total", "runtime", "wheel_records", "preparation", "preparation_digest", "transaction_simulation", "failure_simulation", "identity_mutations_rejected", "runtime_mutations_rejected", "cpu_slice_state", "cpu_frozen_slice_read_started", "cpu_frozen_slice_read_completed", "model_forward", "compiler_opened", "opencl_opened", "device_opened"}
    return set(row) == schema and row["kind"] == KIND and row["ack"] == ACK and identity_valid(row["identity"], PREFLIGHT_NATIVE, PREFLIGHT_ARGV, PREFLIGHT) and stored_topology_valid(row["pre_run_topology"]) and isinstance(mutations, dict) and set(mutations) == TOPOLOGY_MUTATION_KEYS and all(v is True for v in mutations.values()) and set(checks) == CHECK_NAMES and all(v is True for v in checks.values()) and row["pass"] is True and row["passed"] == row["total"] == len(CHECK_NAMES) and BASE.runtime_static_valid(row["runtime"]) and row["runtime"]["available"] >= 16 * 2**30 and row["wheel_records"] == wheels and row["preparation"] == preparation and row["preparation_digest"] == BASE.PREPARATION_DIGEST == sha_bytes(canon(preparation)) and isinstance(tx, dict) and set(tx) == TX_KEYS and all(v is True for v in tx.values()) and isinstance(failure, dict) and set(failure) == FAILURE_KEYS and all(v is True for v in failure.values()) and row["identity_mutations_rejected"] == IDENTITY_MUTATION_NAMES and row["cpu_slice_state"] == "completed" and state_valid(row) and row["model_forward"] is row["compiler_opened"] is row["opencl_opened"] is row["device_opened"] is False
def result_mutations(row: dict, preparation: dict, wheels: dict) -> list[str]:
    cases = {"drop_topology": lambda x: x["topology_mutations"].pop("drop_r8p1"), "extra_topology": lambda x: x["topology_mutations"].__setitem__("extra", True), "false_topology": lambda x: x["topology_mutations"].__setitem__("baseline", False), "empty_tx": lambda x: x.__setitem__("transaction_simulation", {}), "entry_name": lambda x: x["identity"].__setitem__("entry_name", "imported"), "entry_spec": lambda x: x["identity"].__setitem__("entry_spec_is_none", False), "entry_file": lambda x: x["identity"].__setitem__("entry_file", "wrong"), "direct": lambda x: x["identity"].__setitem__("direct_entry", False), "state_list": lambda x: x.__setitem__("cpu_slice_state", []), "device": lambda x: x.__setitem__("device_opened", True)}; out = []
    for name, fn in cases.items():
        candidate = copy.deepcopy(row); fn(candidate)
...
def main() -> int:
    ...
    raw = RESULT.read_bytes(); row = json.loads(raw); preparation = BASE.independent_preparation(); wheels = {"psutil": BASE.wheel_record(BASE.FILES["psutil_record"][0]), "numpy": BASE.wheel_record(BASE.FILES["numpy_record"][0])}; writer = output_writer_simulation(); mutations = topology_mutations(); rejected = result_mutations(row, preparation, wheels); live_rejected = identity_mutations(ident, EXPECTED_NATIVE, EXPECTED_ARGV, SCRIPT)
    checks = {"live_identity": True, "live_identity_mutations": live_rejected == IDENTITY_MUTATION_NAMES, "lock": lock_valid(), "bundle": bundle_valid(), "topology": topology_valid(), "explicit_topology": ancestor_contract() and set(mutations) == TOPOLOGY_MUTATION_KEYS and all(mutations.values()), "static_boundary": static_boundary(), "r8p6_failure": r8p6_failure_valid(), "runtime": BASE.runtime_static_valid(BASE.collect_runtime()), "wheel_records": wheels["psutil"]["hashed_files_verified"] == 17 and wheels["numpy"]["hashed_files_verified"] == 899, "preparation": sha_bytes(canon(preparation)) == BASE.PREPARATION_DIGEST, "result": result_valid(row, preparation, wheels), "independent_output_transactions": set(writer) == VERIFY_TX_KEYS and all(writer.values()), "result_mutations": rejected == ["drop_topology", "extra_topology", "false_topology", "empty_tx", "entry_name", "entry_spec", "entry_file", "direct", "state_list", "device"]}
    output = {"kind": "ph1_intel_execution_r8p8_independent_verification", "checks": checks, "pass": all(checks.values()), "passed": sum(v is True for v in checks.values()), "total": len(checks), "result_sha256": sha_bytes(raw), "manifest_sha256": sha256(MANIFEST), "commit_sha256": sha256(COMMIT), "independent_topology_mutations": mutations, "independent_output_transactions": writer, "model_forward": False, "compiler_opened": False, "opencl_opened": False, "device_opened": False}
