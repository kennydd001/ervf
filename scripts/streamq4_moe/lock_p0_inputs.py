...
ARTIFACT = ROOT / "reports/runs/streamq4_moe/p0_fresh_input_ids.safetensors"
OUTPUT = ROOT / "reports/streamq4_moe/p0_input_lock.json"
CONTEXT = 128
PER_SPLIT = 256
...
if __name__ == "__main__":
    if OUTPUT.exists() or ARTIFACT.exists():
        raise FileExistsError("refusing to overwrite STREAMQ4 input lock")
    ...
    if set(tensors) != expected or any(tuple(value.shape) != (2, CONTEXT) for value in tensors.values()):
        raise RuntimeError("STREAMQ4 tensor contract failed")
    ...
    if not all_disjoint:
        raise RuntimeError("fresh split overlaps an earlier exact CORETAIL P2 context")

    ARTIFACT.parent.mkdir(parents=True, exist_ok=True)
    save_file(tensors, ARTIFACT, metadata={"kind": "streamq4_moe_p0_fresh_inputs", "outputs_opened": "false"})
    payload = {
        "kind": "streamq4_moe_p0_input_lock",
        ...
        "exact_context_disjoint_from_coretail_p2": all_disjoint,
        "variants": ["bf16_teacher", "q4_experts_bf16_trunk", "bf16_experts_int8_trunk", "q4_experts_int8_trunk", "q4_experts_int4_trunk"],
        "quantization": {
            "group_size": 128, "q4_codes": [-7, 7], "int8_codes": [-127, 127],
            "int4_codes": [-7, 7], "rounding": "torch_round_nearest_even", "dequant_dtype": "bfloat16",
        },
        "primary_gate": {"variant": "q4_experts_int8_trunk", "validation_progression_relative_ce_max": 0.03, "validation_relative_ce_max": 0.02, "test_relative_ce_max": 0.02},
    }
