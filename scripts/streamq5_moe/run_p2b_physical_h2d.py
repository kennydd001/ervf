...
PREREG = R / "P2B_PHYSICAL_RESIDENCY_H2D_PREREGISTRATION.md"
LOCK_PATH = R / "p2b_route_input_lock.json"
CAPTURE_PATH = R / "p2b_route_capture_result.json"
EVALUATOR_LOCK = R / "p2b_h2d_evaluator_lock.json"
...
STATIC_SLOTS, TOTAL_SLOTS = 32, 1640
...
if __name__ == "__main__":
    args = parse_args()
    output = R / f"p2b_physical_h2d_{args.split}.json"
    report = R / f"P2B_PHYSICAL_H2D_{args.split.upper()}.md"
    if output.exists() or report.exists():
        raise FileExistsError(f"refusing to overwrite P2B {args.split}")
    validation_path = R / "p2b_physical_h2d_validation.json"
    if args.split == "test" and not validation_path.exists():
        raise RuntimeError("P2B validation required before test")
    ...
    if sha256(Path(__file__)) != evaluator_lock["evaluator_sha256"] or sha256(LOCK_PATH) != evaluator_lock["input_lock_sha256"] or sha256(CAPTURE_PATH) != evaluator_lock["route_capture_sha256"] or sha256(PREREG) != lock["preregistration_sha256"]:
        raise ValueError("P2B evaluator provenance mismatch")
    if args.split == "test":
        validation = json.loads(validation_path.read_text(encoding="utf-8"))
        if validation.get("status") != "p2b_validation_pass_test_authorized":
            raise RuntimeError("P2B test not authorized")
    ...
    if args.split == "validation":
        status, phase_pass = ("p2b_validation_pass_test_authorized", False) if all_pass else ("p2b_validation_closed", False)
    else:
        validation = json.loads(validation_path.read_text(encoding="utf-8"))
        status = "p2b_physical_h2d_pass" if all_pass and all(validation["gates"].values()) else "p2b_physical_h2d_closed"
        phase_pass = status == "p2b_physical_h2d_pass"
    payload = {
        "kind": "streamq5_moe_p2b_actual_pinned_bank_fragmented_h2d", "started_utc": started_utc,
        "completed_utc": datetime.now(timezone.utc).isoformat(), "split": args.split, "status": status,
        "inputs": {"preregistration_sha256": sha256(PREREG), "input_lock_sha256": sha256(LOCK_PATH), "route_capture_sha256": sha256(CAPTURE_PATH), "evaluator_lock_sha256": sha256(EVALUATOR_LOCK), "evaluator_sha256": sha256(Path(__file__)), "p1d_verification_sha256": sha256(P1D_VERIFY), "bank_result_sha256": sha256(BANK_RESULT), "route_artifact_sha256": route_hashes},
        "policy": {"total_slots": TOTAL_SLOTS, "static_slots_per_layer": STATIC_SLOTS, "dynamic_slots_layers_0_7": 3, "dynamic_slots_layers_8_47": 2, "calibration_count_sha256": count_hashes, "split_tokens": [begin, end]},
        ...
        "per_domain": per_domain, "aggregate": aggregate, "gates": gates, "p2b_pass": phase_pass, "next_phase_authorized": phase_pass,
