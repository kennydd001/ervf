...
def main():
 ...
 try:
  be=Backend();send(z,0,'ready',{'device':'intel','pid':__import__('os').getpid(),'topology':top,'identity':be.identity})
  x=words();want=expected('intel',x)
  for epoch in range(1,4):
   c=command();
   if c!=['START',str(epoch)]:raise RuntimeError('command')
   s,d,o=be.run(x);send(z,epoch,'result',{'device':'intel','epoch':epoch,'submit_ns':s,'done_ns':d,'output_words':o,'sha256':digest(o),'different_words':sum(i!=j for i,j in zip(o,want))})
  receive({'cmd':'STOP','nonce':z.nonce});cleanup=be.close();be=None;send(z,4,'cleanup',{'device':'intel','rows':cleanup});return 0
 except BaseException as e:send(z,99,'failure',{'device':'intel','error':f'{type(e).__name__}:{e}','traceback':traceback.format_exc()});return 2
 finally:
