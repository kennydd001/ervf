#!/usr/bin/env python3
"""Create-new bundle and bounded failure lifecycle for PH1 NVIDIA N5."""
...
def atomic_failure(failure_root: Path, payload):
    ...
    if len(raw) > MAX_ARTIFACT_BYTES:
        reduced = {"kind": payload.get("kind", "ph1_nvidia_n5_failure"), "stage": payload.get("stage"), "error": payload.get("error"), "error_type": payload.get("error_type"), "device_opened": payload.get("device_opened"), "oversized_original_bytes": len(raw), "original_sha256": sha(raw), "disposition": "bounded_summary"}
        raw = canonical(reduced)
