...
PREREG = ROOT / "reports/streamq5_moe/P0_MODEL_QUALITY_PREREGISTRATION.md"
LOCK = ROOT / "reports/streamq5_moe/p0_input_lock.json"
EVALUATOR_LOCK = ROOT / "reports/streamq5_moe/p0_evaluator_lock.json"
...
if __name__ == "__main__":
    args = parse_args()
    output = ROOT / f"reports/streamq5_moe/p0_{args.split}_model_quality.json"
    report = ROOT / f"reports/streamq5_moe/P0_{args.split.upper()}_MODEL_QUALITY.md"
    if output.exists() or report.exists():
        raise FileExistsError(f"refusing to overwrite STREAMQ5 P0 {args.split}")
    validation_result = ROOT / "reports/streamq5_moe/p0_validation_model_quality.json"
    if args.split == "test" and not validation_result.is_file():
        raise RuntimeError("STREAMQ5 validation must finish before test opens")
    ...
    if sha256(Path(__file__)) != evaluator_lock["evaluator_sha256"] or sha256(LOCK) != evaluator_lock["input_lock_sha256"]:
        raise ValueError("STREAMQ5 evaluator lock mismatch")
    if sha256(PREREG) != lock["preregistration_sha256"]:
        raise ValueError("STREAMQ5 preregistration hash mismatch")
    ...
    if sha256(input_path) != lock["artifact_sha256"] or sha256(MODEL / "model.safetensors.index.json") != lock["model_index_sha256"]:
        raise ValueError("STREAMQ5 input or model hash mismatch")
    ...
    if args.split == "validation":
        status = "p0_validation_pass_test_authorized" if finite and primary_relative <= 0.025 else "p0_validation_closed"
        p0_pass = False
    else:
        ...
        if validation.get("status") != "p0_validation_pass_test_authorized":
            raise RuntimeError("test was not authorized by STREAMQ5 validation")
    ...
    payload = {
        "kind": "streamq5_moe_p0_full_depth_model_quality",
        "completed_utc": datetime.now(timezone.utc).isoformat(),
        "started_utc": started_utc,
        "status": status, "split": args.split,
        "inputs": {"preregistration_sha256": sha256(PREREG), "lock_sha256": sha256(LOCK), "evaluator_lock_sha256": sha256(EVALUATOR_LOCK), "evaluator_sha256": sha256(Path(__file__)), "input_artifact_sha256": sha256(input_path), "previous_coretail_lock_sha256": sha256(OLD_LOCK), "previous_coretail_input_sha256": sha256(OLD_INPUT), "previous_streamq4_lock_sha256": sha256(Q4_LOCK), "previous_streamq4_input_sha256": sha256(Q4_INPUT), "model_index_sha256": sha256(MODEL / "model.safetensors.index.json")},
        ...
        "claim_boundary": "Full-depth fresh-split quality for fixed RTN-Q5 experts and fixed fake-quantized trunks; no physical bank, cache, kernel, or wall-clock claim.",
    }
    output.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    report.write_text("\n".join([
        f"# STREAMQ5-MoE P0 - {args.split} full-depth kwaliteit", "",
        f"Uitkomst: **{status}**.", "",
