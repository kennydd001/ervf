...
R12_PREREG = REPORTS / "PORT80B_T0R12_OFFICIAL_CPU_REFERENCE_ONLY_PREREGISTRATION_2026-08-13.md"
PROMPT_GENERATION_LOCK = REPORTS / "port80b_t0r12_prompt_generation_lock.json"
PROMPT_LOCK = REPORTS / "port80b_t0r12_prompt_lock.json"
...
RUNNER_LOCK = REPORTS / "port80b_t0r12_runner_lock.json"
VERIFIER = ROOT / "scripts" / "streamq5_moe" / "verify_port80b_t0r12_official_cpu_reference_only.py"
VERIFIER_LOCK = REPORTS / "port80b_t0r12_verifier_lock.json"
INVOCATION_LEDGER = RUN_DIR / "invocation_ledger"
ACK = "T0R12_OFFICIAL_CPU_CAPTURE_AFTER_PREFLIGHT"
...
def execution_lock_check() -> dict[str, Any]:
    if not RUNNER_LOCK.is_file() or not VERIFIER.is_file() or not VERIFIER_LOCK.is_file():
        raise RuntimeError("R12 execution provenance file missing")
    ...
    expected = {
        ...
        "t0r12_prereg_sha256": sha256(R12_PREREG), "prompt_generation_lock_sha256": sha256(PROMPT_GENERATION_LOCK),
    ...
    if mismatched: raise RuntimeError(f"R12 execution lock mismatch: {mismatched}")
...
def premodel_target_guard(run_index: int) -> None:
    targets=[RUN_DIR/f"t0r12_capture_{run_index}_raw.safetensors",RUN_DIR/f"t0r12_capture_{run_index}_result.json",
             RUN_DIR/f"t0r12_capture_{run_index}_failure.json",INVOCATION_LEDGER/f"capture_{run_index}_pre_model.json"]
...
def reference(run_index: int) -> dict[str, Any]:
    ...
    raw_path = RUN_DIR / f"t0r12_capture_{run_index}_raw.safetensors"
    result_path = RUN_DIR / f"t0r12_capture_{run_index}_result.json"
    ...
    result = {"kind": "port80b_t0r12_official_cpu_reference_only", "run_index": run_index,
...
def smoke() -> dict[str, Any]:
    ...
    return {"kind": "port80b_t0r12_cpu_smoke", "pass": header["pass"] and meta_contract,
...
if __name__ == "__main__":
    ...
    if args.phase == "ledger-unit":
        payload={"kind":"port80b_t0r12_ledger_unit",**ledger_schema_unit_test(),"physical_actions":{"model_loaded":False,"forward":False,"bank":False,"gpu":False}}
    elif args.phase == "lockcheck":
        payload = {"kind":"port80b_t0r12_exact_execution_lockcheck","pass":execution_lock_check()["pass"],
        ...
        try:
            ...
            failure = RUN_DIR / f"t0r12_capture_{args.run_index}_failure.json"
            if failure.exists():
                raise RuntimeError(f"refusing to overwrite failure evidence: {failure}") from error
            memory=psutil.Process().memory_info()
            evidence={"kind":"port80b_t0r12_official_cpu_reference_only_failure","status":"valid_negative_or_blocked_not_pass","run_index":args.run_index,
                        ...
                        "cuda_initialized":torch.cuda.is_initialized(),
                        "existing_paths":{"raw":(RUN_DIR/f"t0r12_capture_{args.run_index}_raw.safetensors").exists(),"result":(RUN_DIR/f"t0r12_capture_{args.run_index}_result.json").exists()},
