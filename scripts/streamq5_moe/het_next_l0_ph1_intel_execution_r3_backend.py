#!/usr/bin/env python3
"""R3 evidence wrapper over the hash-bound frozen R0 Intel backend."""
...
class Backend(r0.Backend):
 def __init__(self):
  super().__init__();self.api_counts={name:0 for name in FORBIDDEN};self.ownership={};self.extension_counts={name:0 for name in EXTENSION_ABI};self.host_pointers=set()
 def extension(self,platform,name,prototype):
  ...
  def wrapped(*values):
   self.extension_counts[name]+=1;result=raw(*values)
