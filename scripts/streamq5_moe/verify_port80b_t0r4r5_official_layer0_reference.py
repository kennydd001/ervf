#!/usr/bin/env python3
"""Independent reconstruction verifier for PORT80B T0-R4-REF-R5."""
...
RUNNER=ROOT/"scripts/streamq5_moe/run_port80b_t0r4r5_official_layer0_reference.py"
LOCK=REPORTS/"port80b_t0r4r5_runner_lock.json"; VLOCK=REPORTS/"port80b_t0r4r5_verifier_lock.json"
...
def preflight():
 l=json.loads(LOCK.read_text());v=json.loads(VLOCK.read_text());c={"runner_bound":sha(RUNNER)==l["runner_sha256"],"verifier_bound":sha(__file__)==l["verifier_sha256"]==v["verifier_sha256"],"verifier_lock_bound":sha(VLOCK)==l["verifier_lock_sha256"],"outputs_absent":not RUN_DIR.exists() or not any(RUN_DIR.glob("*_result.json")),"schema":v["schema_version"]=="PORT80B_T0R4R5_REF_V1"};return {"kind":"port80b_t0r4r5_verifier_preflight","pass":all(c.values()),"checks":c,"checks_passed":sum(c.values()),"checks_total":len(c)}

def verify(i):
 rp=RUN_DIR/f"t0r4r5_run_{i}_result.json"; ap=RUN_DIR/f"t0r4r5_run_{i}_raw.safetensors"; r=json.loads(rp.read_text()); vals={}
 ...
 c["raw_sha"]=sha(ap)==r["raw_artifact_sha256"];c["result_provenance"]=r["kind"]=="port80b_t0r4r5_official_layer0_reference_and_bank_stage" and r["run_index"]==i and r["runner_sha256"]==sha(RUNNER) and r["runner_lock_sha256"]==sha(LOCK) and r["verifier_sha256"]==sha(__file__) and r["verifier_lock_sha256"]==sha(VLOCK)
 ...
 if i==2:
  r1=json.loads((RUN_DIR/"t0r4r5_run_1_result.json").read_text());a1=RUN_DIR/"t0r4r5_run_1_raw.safetensors";equal=True
 ...
 return {"kind":"port80b_t0r4r5_independent_reconstruction","run_index":i,"pass":all(c.values()),"checks":c,"checks_passed":sum(c.values()),"checks_total":len(c),"recomputed_manual_ulp":manual,"claim_boundary":"Independent R4-REF bank/reference verification only; Q5 execution and T0-P4 closed."}

def failure(i):
 p=RUN_DIR/f"t0r4r5_run_{i}_failure.json";r=json.loads(p.read_text());c={"identity":r["kind"]=="port80b_t0r4r5_official_layer0_failure" and r["status"]=="valid_negative_or_blocked_not_pass" and r["run_index"]==i,"provenance":r["runner_sha256"]==sha(RUNNER) and r["runner_lock_sha256"]==sha(LOCK) and r["verifier_sha256"]==sha(__file__) and r["verifier_lock_sha256"]==sha(VLOCK),"never_pass":r["claim_boundary"]=="Failure evidence only; never a scientific pass.","cpu_only":r["cuda_initialized"] is False};return {"kind":"port80b_t0r4r5_failure_adjudication","pass":False,"valid_failure_evidence":all(c.values()),"checks":c,"status":"valid_negative_or_blocked_not_pass" if all(c.values()) else "invalid_failure_evidence"}
def main():
