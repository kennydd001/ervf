#!/usr/bin/env python3
"""Independent reconstruction verifier for PORT80B T0-R7."""
...
RUN_DIR=ROOT/"reports/runs/streamq5_moe/port80b_t0r10_official_cpu_route_repro"
RUNNER=ROOT/"scripts/streamq5_moe/run_port80b_t0r10_official_cpu_route_repro.py"
LOCK=REPORTS/"port80b_t0r10_runner_lock.json"; VLOCK=REPORTS/"port80b_t0r10_verifier_lock.json"
...
def expected_schema():
 ...
 cap={"input_norm":("torch.bfloat16",[1,16,2048]),"gdn":("torch.bfloat16",[1,16,2048]),"post_norm":("torch.bfloat16",[1,16,2048]),"router_input":("torch.bfloat16",[16,2048]),"official_router_logits":("torch.bfloat16",[16,512]),"official_router_weights":("torch.bfloat16",[16,10]),"official_router_ids":("torch.int64",[16,10]),"experts":("torch.bfloat16",[16,2048]),"shared":("torch.bfloat16",[16,2048]),"shared_gate":("torch.bfloat16",[16,1])}
 routes={"router_logits_native_bf16":("torch.bfloat16",[16,512]),"router_logits_fp32":("torch.float32",[16,512]),"router_probs_fp32":("torch.float32",[16,512]),"router_ids":("torch.int64",[16,10]),"router_top10_top11_margin_fp32":("torch.float32",[16]),"router_weights_precast_fp32":("torch.float32",[16,10]),"router_weights_native_bf16":("torch.bfloat16",[16,10]),"router_boundary_tie_mask":("torch.bool",[16,512]),"router_selected_boundary_mask":("torch.bool",[16,10]),"router_top11_ids":("torch.int64",[16,11]),"router_top11_native_bf16_logits":("torch.bfloat16",[16,11])}
 ...
 for p in range(4):
  ...
  for k,v in cap.items():s[f"p{p}_whole_{k}"]=v
  ...
  s[f"p{p}_whole_cache_conv"]=("torch.bfloat16",[1,8192,4]);s[f"p{p}_whole_cache_recurrent"]=("torch.float32",[1,32,128,128])
 return s
...
def preflight():
 l=json.loads(LOCK.read_text());v=json.loads(VLOCK.read_text());c={"runner_bound":sha(RUNNER)==l["runner_sha256"],"verifier_bound":sha(__file__)==l["verifier_sha256"]==v["verifier_sha256"],"verifier_lock_bound":sha(VLOCK)==l["verifier_lock_sha256"],"outputs_absent":not RUN_DIR.exists() or not any(RUN_DIR.glob("*_result.json")),"schema":v["schema_version"]=="PORT80B_T0R10_REF_V1"};return {"kind":"port80b_t0r10_verifier_preflight","pass":all(c.values()),"checks":c,"checks_passed":sum(c.values()),"checks_total":len(c)}

def verify(i):
 rp=RUN_DIR/f"t0r10_capture_{i}_result.json"; ap=RUN_DIR/f"t0r10_capture_{i}_raw.safetensors"; r=json.loads(rp.read_text()); vals={}
 ...
 c["raw_sha"]=sha(ap)==r["raw_artifact_sha256"];c["result_provenance"]=r["kind"]=="port80b_t0r10_official_cpu_route_repro_reference_and_bank_stage" and r["run_index"]==i and r["runner_sha256"]==sha(RUNNER) and r["runner_lock_sha256"]==sha(LOCK) and r["verifier_sha256"]==sha(__file__) and r["verifier_lock_sha256"]==sha(VLOCK)
 ...
 for p in range(4):
  ...
  native=vals[f"p{p}_whole_router_logits_native_bf16"]; logits=native.float(); probs=torch.softmax(logits,dim=-1); top11,top11ids=torch.topk(probs,11,dim=-1);w=top11[:,:10];ids=top11ids[:,:10];w=w/w.sum(-1,keepdim=True);margin=top11[:,9]-top11[:,10];nw=w.to(torch.bfloat16);ties=probs==top11[:,9:10];selected_ties=torch.gather(ties,1,ids);native11=torch.gather(native,1,top11ids)
  route &= torch.equal(native,vals[f"p{p}_whole_official_router_logits"]) and torch.equal(logits,vals[f"p{p}_whole_router_logits_fp32"]) and torch.equal(probs,vals[f"p{p}_whole_router_probs_fp32"]) and torch.equal(ids,vals[f"p{p}_whole_router_ids"]) and torch.equal(w,vals[f"p{p}_whole_router_weights_precast_fp32"]) and torch.equal(nw,vals[f"p{p}_whole_router_weights_native_bf16"]) and torch.equal(margin,vals[f"p{p}_whole_router_top10_top11_margin_fp32"]) and torch.equal(ties,vals[f"p{p}_whole_router_boundary_tie_mask"]) and torch.equal(selected_ties,vals[f"p{p}_whole_router_selected_boundary_mask"]) and torch.equal(top11ids,vals[f"p{p}_whole_router_top11_ids"]) and torch.equal(native11,vals[f"p{p}_whole_router_top11_native_bf16_logits"]) and bool((w>0).all() and (w[:,:-1]>=w[:,1:]).all() and (nw>0).all() and (nw[:,:-1]>=nw[:,1:]).all() and ((w.sum(-1)-1).abs().max()<=2**-20) and ((nw.float().sum(-1)-1).abs().max()<=0.00390720367431640625) and all(torch.unique(x).numel()==10 for x in ids))
 ...
 c["whole_prefix16_cache_equal"]=all(torch.equal(vals[f"p{p}_whole_cache_conv"],vals[f"p{p}_s16_cache_conv"]) and torch.equal(vals[f"p{p}_whole_cache_recurrent"],vals[f"p{p}_s16_cache_recurrent"]) for p in range(4))
 c["direct_official_router_tuple"]=all(torch.equal(vals[f"p{p}_whole_official_router_logits"],vals[f"p{p}_whole_router_logits_native_bf16"]) and torch.equal(vals[f"p{p}_whole_official_router_weights"],vals[f"p{p}_whole_router_weights_native_bf16"]) and torch.equal(vals[f"p{p}_whole_official_router_ids"],vals[f"p{p}_whole_router_ids"]) for p in range(4))
 ...
 return {"kind":"port80b_t0r10_independent_reconstruction","run_index":i,"pass":all(c.values()),"checks":c,"checks_passed":sum(c.values()),"checks_total":len(c),"recomputed_manual_ulp":manual,"claim_boundary":"Independent R4-REF bank/reference verification only; Q5 execution and T0-P4 closed."}

def compare():
 r1=json.loads((RUN_DIR/"t0r10_capture_1_result.json").read_text());r2=json.loads((RUN_DIR/"t0r10_capture_2_result.json").read_text());raw=[]
 for i in (1,2):
  d={}
  with safe_open(RUN_DIR/f"t0r10_capture_{i}_raw.safetensors",framework="pt",device="cpu") as h:
   for k in h.keys():d[k]=h.get_tensor(k)
  raw.append(d)
 route_keys=[f"p{p}_whole_{name}" for p in range(4) for name in ("router_ids","router_weights_native_bf16")]
 ties=[f"p{p}_whole_{name}" for p in range(4) for name in ("router_boundary_tie_mask","router_selected_boundary_mask","router_top11_ids","router_top11_native_bf16_logits","router_top10_top11_margin_fp32")]
 p1=RUN_DIR/"invocation_ledger/capture_1_pre_model.json";p2=RUN_DIR/"invocation_ledger/capture_2_pre_model.json";inv1=json.loads(p1.read_text());inv2=json.loads(p2.read_text())
 c={"capture_verifiers":verify(1)["pass"] and verify(2)["pass"],"distinct_invocations":inv1["pid"]!=inv2["pid"] and inv1["process_create_time_unix"]!=inv2["process_create_time_unix"] and inv1["nonce"]!=inv2["nonce"],"ledger_bound_to_results":r1["invocation"]["sha256"]==sha(p1) and r2["invocation"]["sha256"]==sha(p2) and r1["invocation"]["capture"]==1 and r2["invocation"]["capture"]==2 and r1["invocation"]["commandline"]==inv1["commandline"] and r2["invocation"]["commandline"]==inv2["commandline"] and "--run-index" in inv1["commandline"] and "--run-index" in inv2["commandline"],"capture2_no_scientific_read_permission":inv2["capture1_scientific_outputs_read_permission"] is False,"route_ids_weights_bitwise":all(torch.equal(raw[0][k],raw[1][k]) for k in route_keys),"tie_classification_bitwise":all(torch.equal(raw[0][k],raw[1][k]) for k in ties),"raw_keys_exact":set(raw[0])==set(raw[1]),"bank_manifest_semantic":{k:v for k,v in r1["record_artifact"].items() if k!="second_run_compared_bytes"}=={k:v for k,v in r2["record_artifact"].items() if k!="second_run_compared_bytes"},"cross_backend_not_claimed":True}
 return {"kind":"port80b_t0r10_independent_clean_process_compare","pass":all(c.values()),"status":"t0r10_official_cpu_route_repro_candidate_pass" if all(c.values()) else "negative","checks":c,"claim_boundary":"Pinned official CPU backend reproducibility only. Cross-backend invariance remains R4 negative; no Q5 physical/T0-P claim."}

def failure(i):
 p=RUN_DIR/f"t0r10_capture_{i}_failure.json";r=json.loads(p.read_text());c={"identity":r["kind"]=="port80b_t0r10_official_cpu_route_repro_failure" and r["status"]=="valid_negative_or_blocked_not_pass" and r["run_index"]==i,"provenance":r["runner_sha256"]==sha(RUNNER) and r["runner_lock_sha256"]==sha(LOCK) and r["verifier_sha256"]==sha(__file__) and r["verifier_lock_sha256"]==sha(VLOCK),"never_pass":r["claim_boundary"]=="Failure evidence only; never a scientific pass.","cpu_only":r["cuda_initialized"] is False};return {"kind":"port80b_t0r10_failure_adjudication","pass":False,"valid_failure_evidence":all(c.values()),"checks":c,"status":"valid_negative_or_blocked_not_pass" if all(c.values()) else "invalid_failure_evidence"}
def main():
 p=argparse.ArgumentParser();p.add_argument("--phase",choices=("preflight","verify","compare","failure"),required=True);p.add_argument("--run-index",type=int,choices=(1,2));a=p.parse_args();r=preflight() if a.phase=="preflight" else (compare() if a.phase=="compare" else (failure(a.run_index) if a.phase=="failure" else verify(a.run_index)));print(json.dumps(r,indent=2));return 0 if r.get("pass") else (3 if r.get("valid_failure_evidence") else 2)
