"""NERVF-4: gatherless down. A/B on one variable, on top of ERVF.

Preregistered gates (frozen here, before the run):
  G-NERVF-4C  generation bit-identical to the frozen V35 anchor and between arms
  G-NERVF-4P  >= 80% of the gather time removed (N2 measured it at 8.192 ms/token
              with a local drift of 0.040), read as: MoE-block gain >= 6.55 ms
  G-NERVF-4T  token-level gain must exceed its own bracketed drift to be reported

Arms: A = production (gather + masked GEMV from the device mirror);
      C = gatherless (masked GEMV reads the mapped host record directly).
ERVF is ON in every arm, so the one variable is the gather.
...
def main() -> int:
    ...
    arms, ref = {}, None
    for name, use in (("gather_a", False), ("gatherless", True), ("gather_b", False)):
        rt.fused.use_ervf = True
        rt.fused.gatherless_down = use
        assert rt.fused.use_ervf is True
        assert rt.fused.gatherless_down is use
        print("arm " + name + ": gatherless=" + str(use) + " (ervf on in every arm)", flush=True)
        ...
        arms[name] = {"arm": name, "gatherless": bool(rt.fused.gatherless_down), "use_ervf": bool(rt.fused.use_ervf), "generation": gen,
                      "parity_arm": bool(same_arm), "parity_anchor": bool(same_anchor),
                      "sweep": merged}
    rt.fused.use_ervf = False
    rt.fused.gatherless_down = False

    contexts = [str(c) for c in args.contexts if c < args.max_ctx - 8]
    per_ctx = {}
    for ctx in contexts:
        a1 = arms["gather_a"]["sweep"][ctx]
        a2 = arms["gather_b"]["sweep"][ctx]
        b = arms["gatherless"]["sweep"][ctx]
    ...
    gates = {
        "G_NERVF_4C_exact": {"arms": {k: v["parity_arm"] for k, v in arms.items()},
                             "anchor": {k: v["parity_anchor"] for k, v in arms.items()},
                             "passed": bool(parity)},
        "G_NERVF_4P_gather_removed": {"required_moe_gain_ms": GATE_4P_MS,
                               "n2_gather_ms": N2_GATHER_MS,
                               "measured_best_moe_gain_ms": max(v["moe_gain_ms"] for v in per_ctx.values()),
                               "measured_best_moe_speedup": best_sp,
                               "per_context": {c: v["moe_speedup"]
                                               for c, v in per_ctx.items()},
                               "passed": bool(parity and max(v["moe_gain_ms"] for v in per_ctx.values()) >= GATE_4P_MS)},
        "G_NERVF_4T_token": {c: {"gain_ms": v["token_gain_ms"],
    ...
    payload = {
        "kind": "nervf_nemotron_gatherless_ab",
        "namespace": "NERVF_NEMOTRON", "phase": "NERVF_4",
        "started_utc": started,
    ...
    (OUT_DIR / "nervf4_gatherless_ab.json").write_text(
        json.dumps(payload, indent=2) + "\n", encoding="utf-8")

    print(f"\n  G-NERVF-3C exact : {parity}")
    print(f"  G-NERVF-4P (>= {GATE_4P_MS:.2f} ms): "
          f"{gates['G_NERVF_4P_gather_removed']['passed']}")
    print("\nwritten nervf4_gatherless_ab.json")
    return 0
