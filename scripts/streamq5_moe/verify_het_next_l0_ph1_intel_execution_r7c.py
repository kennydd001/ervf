#!/usr/bin/env python3
"""Standalone R7C authorization verifier; numerical replay is frozen R7A."""
...
VERIFY = R / "het_next_l0_ph1_intel_execution_r7c_independent_verification.json"
...
R7B_AUDIT_SHA = "20ccffff336cb57f855749e4cd1ee9e0901b39c8df91d07447ebea3d9fe902cd"
AUTH_CHECKS = {
...
CHAIN = {
    "runner_sha256": S / "run_het_next_l0_ph1_intel_execution_r7c.py",
    "verifier_sha256": Path(__file__),
    "preflight_sha256": S / "preflight_het_next_l0_ph1_intel_execution_r7c.py",
    "prereg_sha256": R / "HET_NEXT_L0_PH1_INTEL_EXECUTION_R7C_PREREGISTRATION_2026-08-14.md",
...
def extension_valid(extension: dict, lock: dict, observed: dict, auth_result: dict) -> bool:
    expected_lock_keys = {"kind", "execution_open", "audit_token", "physical_output", "physical_verifier", *observed.keys()}
    return (
        ...
        and extension.get("lock_sha256") == sha256(LOCK)
        ...
        and extension.get("audit_token") == ACK
        and extension.get("outer_failure_stages") == OUTER_STAGES
        and observed.get("authorization_result_sha256") == AUTH_RESULT_SHA
        and observed.get("r7b_audit_sha256") == R7B_AUDIT_SHA
        and observed.get("r7p_result_sha256") == R7P_SHA
        and auth_result_valid(auth_result)
    )
...
def main() -> int:
    ...
    auth_result = json.loads(AUTH_RESULT.read_text())
    observed = {name: sha256(path) for name, path in CHAIN.items()}
    extension = result.get("authorization", {}).get("r7c_authorization", {})
    extension_pass = sha256(AUTH_RESULT) == AUTH_RESULT_SHA and extension_valid(extension, lock, observed, auth_result)
    checks = {"r7c_authorization_extension": extension_pass}
    if extension_pass:
        # Import and execute the frozen independent numerical verifier only after R7C authorization passes.
        ...
        checks["bundle"] = numerical.verify_bundle_contract(
            result_bytes,
            manifest,
    ...
    output = {
        "kind": "ph1_intel_execution_r7c_independent_verification",
        "checks": checks,
