...
RUN_DIR = ROOT / "reports/runs/streamq5_moe/het_next_cap0_dual_device_cohabitation"
RESULT = RUN_DIR / "cap0_result.json"
COMMIT = RUN_DIR / "cap0_commit.json"
FAILURE = RUN_DIR / "cap0_failure.json"
LOCK = REPORTS / "het_next_cap0_runner_lock.json"
VERIFIER_LOCK = REPORTS / "het_next_cap0_verifier_lock.json"
PREREG = REPORTS / "HET_NEXT_CAP0_DUAL_DEVICE_COHABITATION_PREREGISTRATION_2026-08-13.md"
DESIGN = REPORTS / "HET_NEXT_CAP0_STATIC_PREFLIGHT_DESIGN_2026-08-13.md"
PROTOCOL_PATH = SCRIPTS / "het_next_cap0_protocol.py"
KERNEL_PATH = SCRIPTS / "het_next_cap0_kernels.py"
VERIFIER = SCRIPTS / "verify_het_next_cap0_dual_device_cohabitation.py"
PREFLIGHT = SCRIPTS / "preflight_het_next_cap0_static.py"
...
def intel_backend(ledger):
    from ctypes import wintypes
    from het_next_cap0_kernels import INTEL_BUILD_OPTIONS, INTEL_SOURCE
    ...
    CL_DEVICE_GLOBAL_MEM_SIZE = 0x101F; CL_DEVICE_MAX_MEM_ALLOC_SIZE = 0x1010; CL_DEVICE_ADDRESS_BITS = 0x100D; CL_DEVICE_QUEUE_PROPERTIES = 0x102A
    ...
    for platform in platforms:
        ...
        for device in devices:
            ...
            if "Intel" in vendor and "Arc" in name and "cl_intel_unified_shared_memory" in extensions: candidates.append((platform, device, vendor, name, extensions))
    if len(candidates) != 1: raise RuntimeError(f"intel_candidate_count:{len(candidates)}")
    platform, device, vendor, name, extensions = candidates[0]; error = C.c_int(); devices1 = (C.c_void_p * 1)(device)
    context = queue = program = kernel = None; pointer = None; mem_free = None
    identity = {"vendor": vendor, "name": name, "driver": text_info(device, get_device_info, CL_DRIVER_VERSION), "device_version": text_info(device, get_device_info, CL_DEVICE_VERSION), "opencl_c": text_info(device, get_device_info, CL_DEVICE_OPENCL_C_VERSION), "extensions": extensions, "global_mem": scalar_info(device, CL_DEVICE_GLOBAL_MEM_SIZE, C.c_ulonglong), "max_alloc": scalar_info(device, CL_DEVICE_MAX_MEM_ALLOC_SIZE, C.c_ulonglong), "address_bits": scalar_info(device, CL_DEVICE_ADDRESS_BITS, C.c_uint), "queue_properties": scalar_info(device, CL_DEVICE_QUEUE_PROPERTIES, C.c_ulonglong), "usm_capabilities": {key: scalar_info(device, value, C.c_ulonglong) for key, value in USM_INFO.items()}}
...
def nvidia_backend(ledger):
    ...
    from het_next_cap0_kernels import NVIDIA_NVRTC_OPTIONS, NVIDIA_SOURCE
...
def pdh_monitor(stop_event, p, ledger):
    ...
    try:
        ...
        for row in [r for r in ledger.rows if r["kind"] == "pdh_counter" and r["final_state"] == "live"]: ledger.release(row, lambda: 0)
...
def run_capability(ack):
    from het_next_cap0_protocol import DualDeviceProtocol, Win32Primitives, atomic_create_json
    ...
    for name, expected_output, digest in (("intel", intel_expected, INTEL_SHA256), ("nvidia", nvidia_expected, NVIDIA_SHA256)):
        for row in state["results"][name]:
            row["different_words"] = sum(a != b for a, b in zip(row.pop("output_words"), expected_output)); row["expected_sha256"] = digest
...
def main():
    ...
    try:
        ...
        from het_next_cap0_protocol import atomic_create_json
