"""Indirect exact-M NVFP4 kernels for route-adaptive Ornith H4 buckets."""
...
_TEMPLATE = r"""
extern "C" __global__ void nvfp4_route_m__BATCH___warp32_indirect_l2(
    ...
    const int cols)
{
    ...
    for (int wi = 0; wi < 8; ++wi) {
        ...
        #pragma unroll
        for (int m = 0; m < __BATCH__; ++m) part[m][wi] = acc[m];
    }
...
class OrnithNVFP4RouteAdaptive:
    def __init__(self):
        import cupy as cp

        names = tuple(f"nvfp4_route_m{m}_warp32_indirect_l2" for m in range(1, 5))
        self.module = cp.RawModule(
            code=_source(), options=("-std=c++14",), name_expressions=names
        )
        self.functions = {
            m: self.module.get_function(f"nvfp4_route_m{m}_warp32_indirect_l2")
            for m in range(1, 5)
        }
    ...
    def nvfp4(
        ...
        if multiplicity not in self.functions:
            raise ValueError(f"unsupported multiplicity {multiplicity}")
        if tuple(out.shape) != (groups, multiplicity, rows):
            raise ValueError(
                f"out shape mismatch: {out.shape} != {(groups, multiplicity, rows)}"
            )
        if int(slots.size) != groups or int(input_ids.size) != groups * multiplicity:
