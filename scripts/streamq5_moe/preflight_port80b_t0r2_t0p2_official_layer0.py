#!/usr/bin/env python3
"""CPU-only immutable-input preflight for PORT80B T0-R2/T0-P2.
...
T0R2 = REPORTS / "PORT80B_T0R2_OFFICIAL_LAYER0_REFERENCE_PREREGISTRATION_2026-08-13.md"
T0P2 = REPORTS / "PORT80B_T0P2_OFFICIAL_LAYER0_PHYSICAL_PREREGISTRATION_2026-08-13.md"
PROMPT_LOCK = REPORTS / "port80b_t0r1_prompt_lock.json"
ENV_LOCK = REPORTS / "port80b_t0r1_reference_environment_lock.json"
OUT = REPORTS / "port80b_t0r2_t0p2_cpu_preflight.json"
...
T0R2_SHA256 = "0470663d1213ac369ef7096c72f92f7fd6db1d76499cc5add3917e70e6bc647c"
T0P2_SHA256 = "d2cea182c685f3d6f40f5e8f4037e54ff240aaa177129b2036562f4102f37483"
...
CODEC_SOURCES = {
    ROOT / "scripts" / "streamq5_moe" / "build_p1d_physical_bank.py":
        "c92e59b49458fbb9edd2cbfed4bcdecbfc940e686633d561b5b4755c8cdca259",
    ROOT / "scripts" / "streamq5_moe" / "verify_p1d_physical_bank.py":
        "a977f7b744b3bfc21135d3e255559f92d7078a18f329b2511c639ea525c9f518",
    REPORTS / "P1D_CORRECTED_PHYSICAL_BANK_PREREGISTRATION.md":
        "8f96211121d02381633b45e6eaf42b79630559587f81164b63762504104e428b",
}
...
def codec_sentinel() -> dict[str, Any]:
    """Exercise biased-q5 wire mapping independently, including invalid 31."""
    def decode(payload: bytes, code_count: int) -> tuple[list[int], list[int]]:
        observed_fields: list[int] = []
        for begin in range(0, len(payload), 5):
            word = int.from_bytes(payload[begin:begin + 5], "little")
            observed_fields.extend((word >> (slot * 5)) & 31 for slot in range(8))
        observed_fields = observed_fields[:code_count]
        if any(field == 31 for field in observed_fields):
            raise ValueError("reserved q5 field 31")
        return observed_fields, [field - 15 for field in observed_fields]

    signed = list(range(-15, 16)) + [-15]
    fields = [value + 15 for value in signed]
    packed = bytearray()
    for begin in range(0, len(fields), 8):
        word = 0
        for slot, field in enumerate(fields[begin:begin + 8]):
            word |= field << (slot * 5)
        packed.extend(word.to_bytes(5, "little"))
    decoded_fields, decoded = decode(bytes(packed), len(signed))
    invalid = bytearray(packed)
    invalid[0:5] = ((int.from_bytes(invalid[0:5], "little") & ~31) | 31).to_bytes(5, "little")
    try:
        decode(bytes(invalid), len(signed))
    except ValueError:
        invalid_field_31_rejected = True
    else:
        invalid_field_31_rejected = False
    return {
        "pass": (fields == decoded_fields and signed == decoded
                 and min(fields) == 0 and max(fields) == 30
                 and len(packed) == 20 and invalid_field_31_rejected),
        "signed_codes": signed,
        "stored_fields": fields,
        "packed_hex": packed.hex(),
        "packed_bytes": len(packed),
        "decoded_codes": decoded,
        "field_31_rejected": invalid_field_31_rejected,
    }
...
def main() -> int:
    ...
    checks.append(row("immutable_preregistrations_exact",
                      T0R2.is_file() and T0P2.is_file()
                      and sha256(T0R2) == T0R2_SHA256 and sha256(T0P2) == T0P2_SHA256,
                      {"t0r2": sha256(T0R2) if T0R2.is_file() else None,
                       "t0p2": sha256(T0P2) if T0P2.is_file() else None}))
    ...
    codec_detail = {str(path.relative_to(ROOT)): {
        "sha256": sha256(path) if path.is_file() else None,
        "expected_sha256": expected,
    } for path, expected in CODEC_SOURCES.items()}
    builder_text = next(iter(CODEC_SOURCES)).read_text(encoding="utf-8")
    verifier_path = ROOT / "scripts" / "streamq5_moe" / "verify_p1d_physical_bank.py"
    verifier_text = verifier_path.read_text(encoding="utf-8")
    sentinel = codec_sentinel()
    codec_contract = (
        all(path.is_file() and sha256(path) == expected for path, expected in CODEC_SOURCES.items())
        and "codes.numpy().astype(np.int16) + 15" in builder_text
        and "& 31).astype(np.int8) - 15" in verifier_text
        and "unsigned=code+15; eight little-order 5-bit codes in five bytes" in builder_text
        and "two's-complement" not in T0R2.read_text(encoding="utf-8").lower()
        and "q+15 in [0,30]" in T0R2.read_text(encoding="utf-8")
        and "field 31 is invalid" in T0P2.read_text(encoding="utf-8")
        and sentinel["pass"]
    )
    codec_detail["sentinel"] = sentinel
    checks.append(row("streamq5_biased_codec_source_and_sentinel_contract", codec_contract, codec_detail))
    ...
    result = {
        "kind": "port80b_t0r2_t0p2_cpu_preflight",
        ...
        "inputs": {"revision": REVISION, "t0r2_sha256": T0R2_SHA256,
                   "t0p2_sha256": T0P2_SHA256, "prompt_lock_sha256": PROMPT_LOCK_SHA256,
