...
PREREG = ROOT / "reports/streamq5_moe/P1A_CACHE_PREREGISTRATION.md"
LOCK = ROOT / "reports/streamq5_moe/p1a_route_input_lock.json"
EVALUATOR_LOCK = ROOT / "reports/streamq5_moe/p1a_route_evaluator_lock.json"
INPUT = ROOT / "reports/runs/streamq5_moe/p1a_fresh_route_input_ids.safetensors"
RUN_DIR = ROOT / "reports/runs/streamq5_moe/p1a_routes"
LAYER_DIR = ROOT / "reports/streamq5_moe/p1a_route_layers"
RESULT = ROOT / "reports/streamq5_moe/p1a_route_capture_result.json"
REPORT = ROOT / "reports/streamq5_moe/P1A_ROUTE_CAPTURE.md"
...
if __name__ == "__main__":
    if RESULT.exists() or REPORT.exists() or RUN_DIR.exists() or LAYER_DIR.exists():
        raise FileExistsError("refusing to overwrite P1A route capture")
    ...
    for layer_index in range(LAYERS):
        ...
        save_file(routes, artifact, metadata={
            "kind": "streamq5_moe_p1a_candidate_routes", "layer": str(layer_index),
            "input_lock_sha256": sha256(LOCK), "evaluator_sha256": sha256(Path(__file__)),
        })
        payload = {
            "kind": "streamq5_moe_p1a_route_layer", "layer": layer_index,
    ...
    result = {
        "kind": "streamq5_moe_p1a_route_capture", "status": "route_capture_complete" if finite else "route_capture_nonfinite",
