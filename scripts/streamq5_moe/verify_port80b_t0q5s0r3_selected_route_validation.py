#!/usr/bin/env python3
"""Independent S0-R3 verifier; reruns source/Q5 graphs directly from shard+D2."""
...
ROOT=Path(__file__).resolve().parents[2];S=ROOT/'scripts/streamq5_moe';R=ROOT/'reports/streamq5_moe';D=ROOT/'reports/runs/streamq5_moe/port80b_t0q5s0r3_selected_route_validation';RAW=D/'s0r3_raw.safetensors';RES=D/'s0r3_result.json';COM=D/'s0r3_commit.json';D2=ROOT/'reports/runs/streamq5_moe/port80b_t0r12d2r3_cloned_serialization/t0r12d2_raw.safetensors';D2R=ROOT/'reports/runs/streamq5_moe/port80b_t0r12d2r3_cloned_serialization/t0r12d2_result.json';AUD=R/'PORT80B_T0R12D2R3_INDEPENDENT_ARTIFACT_AUDIT_2026-08-13.json';SHARD=Path.home()/'.cache/huggingface/hub/models--Qwen--Qwen3-Coder-Next/snapshots/a19358a7659bd1f564300250ee189120c49a562f/model-00001-of-00040.safetensors';SHARD_SHA='8e9a517133bfbdc6806cf8b61793055a260efeb68e6e019fd90e4bbb1b665d0a';RUN=S/'run_port80b_t0q5s0r3_selected_route_validation.py';SELF=Path(__file__);PR=R/'PORT80B_T0Q5S0R3_SELECTED_ROUTE_VALIDATION_PREREGISTRATION_2026-08-13.md';DEP=R/'port80b_t0r4_dependency_execution_lock.json';LOCK=R/'port80b_t0q5s0r3_runner_lock.json';VL=R/'port80b_t0q5s0r3_verifier_lock.json';NAMES=('gate','up','down');SHAPES=((512,2048),(512,2048),(2048,512))
...
def verify():
 ...
 if proc.cpu_affinity()!=want or torch.get_num_threads()!=1 or torch.get_num_interop_threads()!=1 or not torch.are_deterministic_algorithms_enabled() or not torch.backends.mkldnn.enabled or torch.get_float32_matmul_precision()!='highest' or torch.is_autocast_enabled('cpu') or torch.cuda.is_initialized():raise RuntimeError('verifier runtime')
 r=json.loads(RES.read_text());commit=json.loads(COM.read_text());checks={'commit':set(commit['files'])=={RAW.name,RES.name} and commit['files'][RAW.name]=={'bytes':RAW.stat().st_size,'sha256':sha(RAW)} and commit['files'][RES.name]=={'bytes':RES.stat().st_size,'sha256':sha(RES)},'bindings':r['runner_sha256']==sha(RUN) and r['verifier_sha256']==sha(SELF) and r['verifier_lock_sha256']==sha(VL) and r['runner_lock_sha256']==sha(LOCK) and r['runner_lock_content']==json.loads(LOCK.read_text()) and r['prereg_sha256']==sha(PR) and r['dependency_lock_sha256']==sha(DEP) and r['d2_raw_sha256']==sha(D2) and r['d2_result_sha256']==sha(D2R) and r['d2_audit_sha256']==sha(AUD) and r['shard_expected_sha256']==SHARD_SHA and r['shard_sha256']==sha(SHARD)==SHARD_SHA,'claim':r['kind']=='port80b_t0q5s0r3_validation' and r['status'] in ('selected_route_validation_positive','selected_route_q5_quality_negative') and 'no heldout/pass/complete/layer' in r['claim_boundary']};stored={}
