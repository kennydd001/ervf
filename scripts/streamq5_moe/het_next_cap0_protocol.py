...
class Win32Primitives:
    def __init__(self):
        ...
        self.w = wintypes; self.k = C.WinDLL("kernel32", use_last_error=True); self.calls = []
        k = self.k
    ...
    def new_lock(self):
        lock = C.c_void_p(); self.k.InitializeSRWLock(C.byref(lock)); return lock
    ...
    def close(self, event): self._ok(self.k.CloseHandle(event), "CloseHandle"); self.calls.append(["CloseHandle", int(event)])


class Cell:
    def __init__(self, primitives, value=0):
        self.p = primitives; self.value = [int(value)] if isinstance(primitives, MockPrimitives) else C.c_longlong(value)
    def read(self): return self.p.read(self.value)
    def set(self, value): return self.p.exchange(self.value, value)
...
class DualDeviceProtocol:
    ...
    def close(self):
        for c in self.channels.values():
            for e in (c.command, c.initialized, c.ready, c.done, c.stop): self.p.close(e)
...
def simulate_protocol():
    ...
    negatives = {}
    protocol.channels["intel"].ack.set(0)
    try: protocol.publish(4); negatives["stale_ack"] = False
    ...
    except RuntimeError: negatives["premature_read"] = True
    protocol.close()
