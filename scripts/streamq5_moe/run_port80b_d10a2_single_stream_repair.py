...
PREREG = R / "PORT80B_D10A2_SINGLE_STREAM_REPAIR_PREREGISTRATION.md"
COMPILE_OUT = R / "port80b_d10a2_single_stream_repair_preflight.json"
COMPILE_REPORT = R / "PORT80B_D10A2_SINGLE_STREAM_REPAIR_PREFLIGHT_REPORT_2026-08-13.md"
COMPONENT_OUT = R / "port80b_d10a2_single_stream_repair.json"
ENDURANCE_OUT = R / "port80b_d10a2_single_stream_repair_endurance_10k.json"
REPORT = R / "PORT80B_D10A2_SINGLE_STREAM_REPAIR_REPORT_2026-08-13.md"
...
def compile_phase() -> None:
    ...
    try:
        ...
        required_source_contract = {
            ...
            "exact_conv_oracle": "CONV_EXPECTED_NONZERO_STEP0" in source,
    ...
    result = {
        "kind": "port80b_d10a2_single_stream_repair_cpu_preflight",
    ...
    COMPILE_REPORT.write_text(
        "# PORT80B-D10A2 CPU-only preflight report\n\n"
        ...
        "and did not scan the bank. The executable D10A2 component gate is implemented; "
        "the 10,000-step executor remains closed "
...
def component_phase() -> None:
    ...
    if COMPONENT_OUT.exists() or REPORT.exists():
        raise FileExistsError("refusing to overwrite D10A2 component evidence")
    ram_before = int(psutil.virtual_memory().available)
    if ram_before < MIN_RAM_BEFORE:
        raise RuntimeError(f"hard stop: available RAM below D10A2 frozen {START_RAM_GATE}-byte start gate")
    ...
    result = {
        "kind": "port80b_d10a2_single_stream_repair",
        "completed_utc": datetime.now(timezone.utc).isoformat(),
    ...
    REPORT.write_text(
        "# PORT80B-D10A2 — deterministic single-stream repair report\n\n"
...
def endurance_phase(acknowledgement: str | None) -> None:
    locked_compile()
    raise RuntimeError(
        "D10A2 endurance is unconditionally fail-closed; a new post-component preregistration and runner are required"
    )
