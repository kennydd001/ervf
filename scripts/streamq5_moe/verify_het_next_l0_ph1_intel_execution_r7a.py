...
def linear(w,x):
 ...
 for row in range(r):
  ...
  out[row]=rb(lanes[0])
 return out
def linear_all_row_sentinel(linear_fn=linear):
 evidence={}
 for label,rows,cols in (('gate_up',512,2048),('down',2048,512)):
  ...
  first=linear_fn(w,x).astype('<u2').tobytes();second=linear_fn(w,x).astype('<u2').tobytes();target=expected.astype('<u2').tobytes();evidence[label]={'rows':rows,'cols':cols,'bytes':len(target),'expected_sha256':sha(target),'first_sha256':sha(first),'second_sha256':sha(second),'all_rows_equal':first==target,'repeat_equal':first==second}
 return {'pass':all(x['all_rows_equal'] and x['repeat_equal'] and x['first_sha256']==x['expected_sha256']==x['second_sha256'] for x in evidence.values()),'shapes':evidence}
...
def verify_bundle_contract(result_bytes,manifest,commit,file_names,total_bytes):
 try:
  ...
  return rr.get('kind')=='ph1_intel_execution_r7a' and manifest=={'kind':'ph1_intel_execution_r7a_manifest','files':[row]} and commit=={'kind':'ph1_intel_execution_r7a_commit','manifest_sha256':sha(mb),'result_sha256':sha(result_bytes)} and file_names=={'result.json','manifest.json','commit.json'} and total_bytes<=16*2**20
 except Exception:return False
def main():
 r,m,c=(OUT/n for n in ('result.json','manifest.json','commit.json'));rb=r.read_bytes();rrr=json.loads(rb);checks=verify_dict(rrr);mm=json.loads(m.read_text());checks['bundle']=verify_bundle_contract(rb,mm,json.loads(c.read_text()),{x.name for x in OUT.iterdir()},sum(x.stat().st_size for x in OUT.iterdir()));out={'kind':'ph1_intel_execution_r7a_independent_verification','checks':checks,'pass':all(checks.values()),'passed':sum(checks.values()),'total':len(checks),'claim':'one real expert/input Intel correctness component only'}
