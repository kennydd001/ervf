...
ARTIFACT = ROOT / "reports/runs/streamq5_moe/p0_fresh_input_ids.safetensors"
OUTPUT = ROOT / "reports/streamq5_moe/p0_input_lock.json"
CONTEXT = 128
PER_SPLIT = 256
...
if __name__ == "__main__":
    if OUTPUT.exists() or ARTIFACT.exists():
        raise FileExistsError("refusing to overwrite STREAMQ5 input lock")
    ...
    if not all_disjoint:
        raise RuntimeError("fresh split overlaps an earlier CORETAIL or STREAMQ4 exact context")

    ARTIFACT.parent.mkdir(parents=True, exist_ok=True)
    save_file(tensors, ARTIFACT, metadata={"kind": "streamq5_moe_p0_fresh_inputs", "outputs_opened": "false"})
    payload = {
        "kind": "streamq5_moe_p0_input_lock",
