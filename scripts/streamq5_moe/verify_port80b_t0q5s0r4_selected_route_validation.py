#!/usr/bin/env python3
"""Independent S0-R4 verifier; reruns source/Q5 graphs directly from shard+D2."""
...
ROOT=Path(__file__).resolve().parents[2];S=ROOT/'scripts/streamq5_moe';R=ROOT/'reports/streamq5_moe';D=ROOT/'reports/runs/streamq5_moe/port80b_t0q5s0r4_selected_route_validation';RAW=D/'s0r4_raw.safetensors';RES=D/'s0r4_result.json';COM=D/'s0r4_commit.json';D2=ROOT/'reports/runs/streamq5_moe/port80b_t0r12d2r3_cloned_serialization/t0r12d2_raw.safetensors';D2R=ROOT/'reports/runs/streamq5_moe/port80b_t0r12d2r3_cloned_serialization/t0r12d2_result.json';AUD=R/'PORT80B_T0R12D2R3_INDEPENDENT_ARTIFACT_AUDIT_2026-08-13.json';SHARD=Path.home()/'.cache/huggingface/hub/models--Qwen--Qwen3-Coder-Next/snapshots/a19358a7659bd1f564300250ee189120c49a562f/model-00001-of-00040.safetensors';SHARD_SHA='8e9a517133bfbdc6806cf8b61793055a260efeb68e6e019fd90e4bbb1b665d0a';RUN=S/'run_port80b_t0q5s0r4_selected_route_validation.py';SELF=Path(__file__);PR=R/'PORT80B_T0Q5S0R4_SELECTED_ROUTE_VALIDATION_PREREGISTRATION_2026-08-13.md';DEP=R/'port80b_t0r4_dependency_execution_lock.json';LOCK=R/'port80b_t0q5s0r4_runner_lock.json';VL=R/'port80b_t0q5s0r4_verifier_lock.json';NAMES=('gate','up','down');SHAPES=((512,2048),(512,2048),(2048,512))
...
def expected_schema():
 ...
 return s
def verify():
 proc=__import__('psutil').Process();want=json.loads(DEP.read_text())['runtime']['process_affinity'];proc.cpu_affinity(want);torch.set_num_threads(1);torch.set_num_interop_threads(1);torch.use_deterministic_algorithms(True);torch.set_float32_matmul_precision('highest');torch.backends.mkldnn.enabled=True;torch.set_flush_denormal(False)
 subnormal=torch.nextafter(torch.zeros(1,dtype=torch.float32),torch.ones(1,dtype=torch.float32));subnormal_product=subnormal*torch.ones_like(subnormal);flush_denormal_witness=int(subnormal.view(torch.uint32)[0])==1 and int(subnormal_product.view(torch.uint32)[0])==1
 if proc.cpu_affinity()!=want or torch.get_num_threads()!=1 or torch.get_num_interop_threads()!=1 or not torch.are_deterministic_algorithms_enabled() or not torch.backends.mkldnn.enabled or torch.get_float32_matmul_precision()!='highest' or torch.is_autocast_enabled('cpu') or torch.cuda.is_initialized() or not flush_denormal_witness:raise RuntimeError('verifier runtime')
 r=json.loads(RES.read_text());commit=json.loads(COM.read_text());checks={'commit':set(commit['files'])=={RAW.name,RES.name} and commit['files'][RAW.name]=={'bytes':RAW.stat().st_size,'sha256':sha(RAW)} and commit['files'][RES.name]=={'bytes':RES.stat().st_size,'sha256':sha(RES)},'bindings':r['runner_sha256']==sha(RUN) and r['verifier_sha256']==sha(SELF) and r['verifier_lock_sha256']==sha(VL) and r['runner_lock_sha256']==sha(LOCK) and r['runner_lock_content']==json.loads(LOCK.read_text()) and r['prereg_sha256']==sha(PR) and r['dependency_lock_sha256']==sha(DEP) and r['d2_raw_sha256']==sha(D2) and r['d2_result_sha256']==sha(D2R) and r['d2_audit_sha256']==sha(AUD) and r['shard_expected_sha256']==SHARD_SHA and r['shard_sha256']==sha(SHARD)==SHARD_SHA,'claim':r['kind']=='port80b_t0q5s0r4_validation' and r['status'] in ('selected_route_validation_positive','selected_route_q5_quality_negative') and 'no heldout/pass/complete/layer' in r['claim_boundary']};stored={}
  ...
  for p in range(4):
   ...
   for row in [x for x in r['controls'] if x['prompt']==p]:
    ...
    if row['control']=='wrong_expert_isolated_route':changed=ids.clone();changed[n,0]=row['presented']['expert'];ur,_=graph(x,changed,w,q5);expected=ur[n:n+1];baseline=qr[n:n+1];safe='expert' in row['rejection_errors']
     ...
     v=src.get_tensor(key(512,2));codes,scales,q,_,_=quant(v);act=F.silu(F.linear(x,q5(512,0)))*F.linear(x,q5(512,1));qq=q.reshape(2048,512);chosen=next(((a,c) for a in range(2048) for c in range(512) if qq[a,c]!=0 and act[n,c]!=0),None);a,cc=chosen;new=int(qq[a,cc]-(1 if qq[a,cc]>0 else -1));mutcodes=bytearray(codes);linear=a*512+cc;block=linear//8;slot=linear%8;word=int.from_bytes(mutcodes[block*5:block*5+5],'little');word=(word&~(31<<(5*slot)))|((new+15)<<(5*slot));mutcodes[block*5:block*5+5]=word.to_bytes(5,'little');mut=decode(bytes(mutcodes),scales,*SHAPES[2]);mg=lambda e,j:mut if e==512 and j==2 else q5(e,j);_,ms=graph(x,ids,w,mg);expected=ms[n:n+1];baseline=qs[n:n+1];actual=hashlib.sha256(bytes(mutcodes)+scales).hexdigest();safe=chosen==(row['matrix_row'],row['matrix_column']) and row['source_q']==int(qq[a,cc]) and row['mutated_q']==new and row['expected_digest']==hashlib.sha256(codes+scales).hexdigest() and row['presented_digest']==actual and row['expected_digest']!=actual and 'codes_scales_digest' in row['rejection_errors'];controls.append(safe and torch.equal((gate*ms)[n:n+1],stored[row['gated_raw_key']]))
    controls.append(bool(safe and torch.equal(expected,stored[row['raw_key']]) and (expected!=baseline).any()))
 checks['source_q5_graph_replay']=bool(replay);checks['metrics_exact']=metrics==r['metrics'];finite_metrics=all(math.isfinite(float(v)) for p in metrics.values() for a in p.values() for q in a for v in q.values());quality=finite_metrics and all(q['rel_l2']<=.08 for p in metrics.values() for a in p.values() for q in a);checks['finite_metrics']=finite_metrics;checks['status_quality']=r['status'] in ('selected_route_validation_positive','selected_route_q5_quality_negative') and r['status']==('selected_route_validation_positive' if quality else 'selected_route_q5_quality_negative');checks['controls']=len(controls)==20 and all(controls);checks['flush_denormal_witness']=flush_denormal_witness;checks['runtime_resources']=r['runtime']=={'affinity':json.loads(DEP.read_text())['runtime']['process_affinity'],'threads':1,'interop':1,'deterministic':True,'mkldnn':True,'matmul_precision':'highest','autocast_cpu':False,'inference_mode':True,'cuda_initialized':False} and r['resources'][0]['available']>=16*2**30 and {x['stage'] for x in r['resources']}>= {'start','computed','cleanup','post_serialization'} and all(x['available']>=2*2**30 and x['peak']<=12*2**30 for x in r['resources']) and sum(p.stat().st_size for p in (RAW,RES,COM))<=512*2**20;out={'kind':'t0q5s0r4_independent_verification','pass':all(checks.values()),'checks':checks};return out
def main():
