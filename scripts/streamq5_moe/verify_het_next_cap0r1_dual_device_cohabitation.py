...
ROOT=Path(__file__).resolve().parents[2]; R=ROOT/'reports/streamq5_moe'; D=ROOT/'reports/runs/streamq5_moe/het_next_cap0r1_dual_device_cohabitation'
RESULT=D/'cap0r1_result.json'; COMMIT=D/'cap0r1_commit.json'; FAILURE=D/'cap0r1_failure.json'; VLOCK=R/'het_next_cap0r1_verifier_lock.json'
...
def verify():
 ...
 if FAILURE.exists() and not (RESULT.exists() or COMMIT.exists()):
  f=json.loads(FAILURE.read_text());checks['valid_failure']=f.get('kind')=='het_next_cap0_failure' and f.get('status')=='valid_failure' and f.get('no_model_or_weight_access') is True and bool(f.get('error')) and bool(f.get('traceback'));return {'kind':'het_next_cap0_independent_verification','pass':False,'valid_failure':all(checks.values()),'checks':checks,'bindings':bindings}
 ...
 checks['kind_status']=r.get('kind')=='het_next_cap0r1_dual_device_cohabitation' and r.get('status')=='dual_device_cohabitation_positive' and r.get('claim_boundary')=='4KiB dual-device lifecycle/correctness only; no performance or model claim'
 ...
 ledger=r.get('ledger',[]);expected={'intel_context':1,'intel_queue':1,'intel_program':1,'intel_kernel':1,'intel_host_usm':1,'intel_event':3,'nvidia_nvrtc_temp':1,'nvidia_module':1,'nvidia_stream':1,'nvidia_pinned_host':1,'nvidia_device_memory':1,'nvidia_start_event':3,'nvidia_end_event':3,'pdh_query':1,'pdh_counter':3,'pdh_waitable_timer':1};actual={k:sum(z.get('kind')==k for z in ledger) for k in expected};checks['cleanup']=actual==expected and len({z.get('id') for z in ledger})==len(ledger)==sum(expected.values()) and not r.get('cleanup_errors') and not r.get('errors') and all(z.get('release_attempts')==1 and z.get('release_code')==0 and z.get('final_state')=='released' for z in ledger)
 pc=r.get('primitive_calls',[]);event_creates=[z for z in pc if z and z[0]=='CreateEventW'];event_closes=[z for z in pc if z and z[0]=='CloseHandle'];checks['event_lifecycle']=len(event_creates)==18 and len(event_closes)==18 and len({z[1] for z in event_creates})==18
 ...
 checks['resources_pci']=r.get('resources',{}).get('available_ram_start',0)>=r.get('resources',{}).get('minimum_ram')==2<<30 and r.get('pci_distinct') is True and ii.get('eligible_device_count')==1 and isinstance(ii.get('pci'),dict) and bool(ni.get('pci_bus_id'))
