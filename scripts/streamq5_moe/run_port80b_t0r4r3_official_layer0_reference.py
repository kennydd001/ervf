...
RUNNER_LOCK = REPORTS / "port80b_t0r4r3_runner_lock.json"
VERIFIER = ROOT / "scripts" / "streamq5_moe" / "verify_port80b_t0r4r3_official_layer0_reference.py"
VERIFIER_LOCK = REPORTS / "port80b_t0r4r3_verifier_lock.json"
RECORD_ARTIFACT = RUN_DIR / "layer0_513_real_q5_records.bin"
RECORD_MANIFEST = RUN_DIR / "layer0_513_real_q5_records_manifest.json"
ACK = "T0R4R3_OFFICIAL_LAYER0_CPU_REFERENCE_AFTER_INDEPENDENT_AUDIT"
...
def reference(run_index: int) -> dict[str, Any]:
    ...
    raw_path = RUN_DIR / f"t0r4r3_run_{run_index}_raw.safetensors"
    result_path = RUN_DIR / f"t0r4r3_run_{run_index}_result.json"
    ...
    result = {"kind": "port80b_t0r4r3_official_layer0_reference_and_bank_stage", "run_index": run_index,
              "status": "run1_complete_pending_clean_replay" if run_index == 1 else "t0r4r3_reference_bank_stage_candidate_pass",
    ...
    if run_index == 2:
        prior = json.loads((RUN_DIR / "t0r4r3_run_1_result.json").read_text(encoding="utf-8"))
        prior_raw = safe_open(RUN_DIR / "t0r4r3_run_1_raw.safetensors", framework="pt", device="cpu")
...
def smoke() -> dict[str, Any]:
    ...
    return {"kind": "port80b_t0r4r3_cpu_smoke", "pass": header["pass"] and codec["pass"] and meta_contract,
