# PORT80B-T0Q5-S0-C1-R2A — observable shared-down control sentinel

## Status and claim boundary

This is the immutable C1-R2A authorization-only revision after C1-R2 source/repair GO and a closed preflight PASS 7/7. It changes no arithmetic, evidence, control, threshold, token selection or field selection. It only binds the archived R2 closed preflight and opens the audit-specific lock/token. S0-C1-R2A is a CPU-only control-sensitivity validation; it is not a new numerical-quality experiment, held-out test, model/layer pass, performance result, or breakthrough claim. It may reuse the immutable S0-R5 result but shall not rerun or reinterpret its 759-record quality computation.
