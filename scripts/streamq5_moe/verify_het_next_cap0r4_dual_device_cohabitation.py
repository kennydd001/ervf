...
ROOT=Path(__file__).resolve().parents[2]; R=ROOT/'reports/streamq5_moe'; D=ROOT/'reports/runs/streamq5_moe/het_next_cap0r4_dual_device_cohabitation'
RESULT=D/'cap0r4_result.json'; COMMIT=D/'cap0r4_commit.json'; FAILURE=D/'cap0r4_failure.json'; VLOCK=R/'het_next_cap0r4_verifier_lock.json'
...
def verify():
 ...
 checks['kind_status']=r.get('kind')=='het_next_cap0r4_dual_device_cohabitation' and r.get('status')=='dual_device_cohabitation_positive' and r.get('claim_boundary')=='4KiB dual-device lifecycle/correctness only; no performance or model claim'
 ...
 init=r.get('initialization',{});ii=init.get('intel',{});ni=init.get('nvidia',{});ib=bytes.fromhex(ii.get('binary_hex',''));nb=bytes.fromhex(ni.get('binary_hex',''));checks['identities']=set(init)=={'intel','nvidia'} and 'Intel' in ii.get('vendor','') and 'Arc' in ii.get('name','') and ii.get('allocation')=={'type':0x4197,'base_matches':True,'bytes':4096,'api':'clHostMemAllocINTEL','cl_mem':False,'write_read_buffer':False,'migrate_prefetch':False} and 'cl_intel_unified_shared_memory' in ii.get('extensions',[]) and ii.get('usm_capabilities',{}).get('host',0)!=0 and ni.get('allocation')=={'pinned_host_bytes':4096,'device_bytes':4096,'managed':False} and ni.get('free_memory_start',0)>=64<<20 and len(nb)==ni.get('binary_bytes',0)>0 and len(ib)==ii.get('binary_bytes',0)>0 and hashlib.sha256(ib).hexdigest()==ii.get('binary_sha256') and hashlib.sha256(nb).hexdigest()==ni.get('binary_sha256') and ii.get('build_options')=='-cl-std=CL2.0 -cl-fp32-correctly-rounded-divide-sqrt' and ni.get('compile_options')==['--std=c++14','--fmad=false'] and ni.get('binary_kind')=='PTX' and isinstance(ii.get('build_log'),str) and isinstance(ni.get('compile_log'),str)
 ...
 mon=r.get('monitor',{});samples=mon.get('samples',[]);intervals=mon.get('interval_ms',[]);bounds=mon.get('boundaries',{});paths=[r'\Memory\Page Reads/sec',r'\Memory\Pages Input/sec',r'\Paging File(_Total)\% Usage'];checks['monitor']=mon.get('paths')==paths and mon.get('valid_protocol') is True and bounds.get('pre_ms',-1)>=500 and bounds.get('post_ms',-1)>=500 and len(samples)>=11 and len(intervals)==len(samples)-1 and all(80<=v<=120 for v in intervals) and all(z.get('lateness_ms',999)<=20 and set(z.get('values',{}))==set(paths) and all(math.isfinite(v) for v in z['values'].values()) and all(s.get('return')==0 and s.get('cstatus')==0 for s in z.get('statuses',{}).values()) for z in samples)
