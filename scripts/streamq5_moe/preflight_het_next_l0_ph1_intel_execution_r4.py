#!/usr/bin/env python3
"""No-device/no-payload static preflight for PH1 Intel execution R4."""
...
RUN=S/'run_het_next_l0_ph1_intel_execution_r4.py';BACK=S/'het_next_l0_ph1_intel_execution_r4_backend.py';COMMON=S/'het_next_l0_ph1_intel_execution_r4_common.py';VER=S/'verify_het_next_l0_ph1_intel_execution_r4.py';R0B=S/'het_next_l0_ph1_intel_execution_r0_backend.py';R0R=S/'run_het_next_l0_ph1_intel_execution_r0.py';PR=R/'HET_NEXT_L0_PH1_INTEL_EXECUTION_R4_PREREGISTRATION_2026-08-14.md';LOCK=R/'het_next_l0_ph1_intel_execution_r4_lock.json';OUT=R/'het_next_l0_ph1_intel_execution_r4';RESULT=R/'het_next_l0_ph1_intel_execution_r4_static_preflight.json'
...
def atomic_ownership_faults(back):
 ...
 try:
  back.r0.Backend.bind=lambda _self:None;b=back.Backend();b.lib=Lib()
  b.lib.clCreateContext=Raw(101);b.lib.clCreateCommandQueue=Raw(102);b.lib.clCreateProgramWithBinary=Raw(103);b.lib.clCreateKernel=Raw(104);b.lib.clEnqueueNDRangeKernel=Raw(0);b.lib.clFinish=Raw(0);b.bind()
  if int(b.lib.clCreateContext())!=101 or int(b.context)!=101:return False
  if int(b.lib.clCreateCommandQueue())!=102 or int(b.queue)!=102:return False
  if int(b.lib.clCreateProgramWithBinary())!=103 or int(b.program)!=103:return False
  if int(b.lib.clCreateKernel())!=104 or b.pending_kernels!=[104]:return False
  if not all(x['registered_pending'] for x in b.ownership_ledger):return False
  b2=back.Backend();b2.lib=Lib();b2.lib.clCreateContext=Raw(exc=OSError('create_injected'));b2.lib.clCreateCommandQueue=Raw(0);b2.lib.clCreateProgramWithBinary=Raw(0);b2.lib.clCreateKernel=Raw(0);b2.lib.clEnqueueNDRangeKernel=Raw(0);b2.lib.clFinish=Raw(0);b2.bind()
  try:b2.lib.clCreateContext();return False
  except OSError:pass
  if len(b2.ownership_ledger)!=1 or b2.ownership_ledger[0]['exception']!='OSError:create_injected':return False
  b3=back.Backend();b3.host_pointers=set();b3.pending_usm=[];b3.extension_counts={n:0 for n in back.EXTENSION_ABI};b3.ownership_ledger=[]
  back.r0.Backend.extension=lambda *_args:Raw(4096)
  proto=type('P',(),{'_restype_':back.C.c_void_p,'_argtypes_':back.EXTENSION_ABI['clHostMemAllocINTEL'][1]})
  alloc=b3.extension(1,'clHostMemAllocINTEL',proto);pointer=int(alloc())
  if not(pointer==4096 and b3.pending_usm==[4096] and b3.ownership_ledger[-1]['registered_pending'] is True):return False
  freed=[];b3.context=101;b3.mem_free_fn=lambda _c,p:freed.append(int(p.value)) or 0;b3.lib=Lib();b3.lib.clReleaseContext=lambda _h:0;b3.close()
  if freed!=[4096] or next(x for x in b3.ledger if x.get('op')=='release')['name']!='pending_usm:4096':return False
  b4=back.Backend();b4.lib=Lib();b4.lib.clCreateContext=Raw(505);b4.lib.clCreateCommandQueue=Raw(0);b4.lib.clCreateProgramWithBinary=Raw(0);b4.lib.clCreateKernel=Raw(0);b4.lib.clEnqueueNDRangeKernel=Raw(0);b4.lib.clFinish=Raw(0);released=[];b4.lib.clReleaseContext=lambda h:released.append(int(h)) or 0;b4.bind();b4.lib.clCreateContext();b4.close()
  return released==[505] and any(x.get('name')=='context' and x.get('owned_after') is False for x in b4.ledger)
 finally:back.r0.Backend.bind=old;back.r0.Backend.extension=old_ext
def bundle_mutations(verify):
 result=(json.dumps({'kind':'ph1_intel_execution_r4','positive':True},sort_keys=True,separators=(',',':'))+'\n').encode();row={'name':'result.json','bytes':len(result),'sha256':hashlib.sha256(result).hexdigest()};manifest={'kind':'ph1_intel_execution_r4_manifest','files':[row]};mb=(json.dumps(manifest,sort_keys=True,separators=(',',':'))+'\n').encode();commit={'kind':'ph1_intel_execution_r4_commit','manifest_sha256':hashlib.sha256(mb).hexdigest(),'result_sha256':hashlib.sha256(result).hexdigest()};names={'result.json','manifest.json','commit.json'}
...
def transaction_sim(run):
 ...
 try:
  with tempfile.TemporaryDirectory() as td:
   root=Path(td);run.R=root;run.OUT=root/'out';run.FAILED=root/'failed';run.QUAR=root/'quar';run.configure();run.OUT.mkdir();result={'kind':'ph1_intel_execution_r4','positive':True};run.base.write(run.OUT/'result.json',run.base.canon(result));row={'name':'result.json','bytes':(run.OUT/'result.json').stat().st_size,'sha256':run.fs(run.OUT/'result.json')};run.base.write(run.OUT/'manifest.json',run.base.canon({'kind':'ph1_intel_execution_r4_manifest','files':[row]}));run.base.write(run.OUT/'commit.json',run.base.canon({'kind':'ph1_intel_execution_r4_commit','manifest_sha256':run.fs(run.OUT/'manifest.json'),'result_sha256':run.fs(run.OUT/'result.json')}));
   ...
   run.base.archive(run.FAILED,'attempt',{'kind':'ph1_intel_execution_r4_failure','device_opened':False})
...
def lifecycle_sim(run):
 ...
 try:
  with tempfile.TemporaryDirectory() as td:
   ...
   if run.FAILED.exists() or run.OUT.exists() or run.QUAR.exists():return False
   run.authorize=lambda:{'lock_sha256':'x','observed':{}};run.OUT.mkdir();result={'kind':'ph1_intel_execution_r4','positive':False};run.base.write(run.OUT/'result.json',run.base.canon(result));row={'name':'result.json','bytes':(run.OUT/'result.json').stat().st_size,'sha256':run.fs(run.OUT/'result.json')};run.base.write(run.OUT/'manifest.json',run.base.canon({'kind':'ph1_intel_execution_r4_manifest','files':[row]}));run.base.write(run.OUT/'commit.json',run.base.canon({'kind':'ph1_intel_execution_r4_commit','manifest_sha256':run.fs(run.OUT/'manifest.json'),'result_sha256':run.fs(run.OUT/'result.json')}))
...
def main():
 import sys;sys.path.insert(0,str(S));import het_next_l0_ph1_intel_execution_r4_common as common;import het_next_l0_ph1_intel_execution_r4_backend as back;import run_het_next_l0_ph1_intel_execution_r4 as run;import verify_het_next_l0_ph1_intel_execution_r4 as verify
