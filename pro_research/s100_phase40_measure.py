"""Fresh-process Phase40 H8 arm runner."""
...
from s100_phase40_h8_pipeline import make_candidate, make_parent
...
def main() -> int:
    ...
    parser.add_argument(
        "--arm", choices=("base_a", "pipeline_b3", "base_b", "compile"),
        required=True,
    )
    ...
    out = RESULTS / f"S100_PHASE40_{args.arm.upper()}_CTX{args.context}.json"
    payload = {
        "kind": "s100_phase40_measure",
        "status": "started",
        "arm": args.arm,
        ...
        "started_utc": utc_now(),
        "preregistration": "pro_research/S100_PHASE40_H8_DOWN_PIPELINE_PREREGISTRATION.md",
        "claim_boundary": "exact synchronous target-only H8; no drafter or speculative throughput",
    }
    rt = None
    try:
        if args.arm == "compile":
            ...
            required = int(args.context) + 8 * (int(args.blocks) + int(args.warmup)) + 1
            if required > len(tokens):
                raise RuntimeError(f"canonical trace too short: need {required}")
            allocation_context = int(args.context) + 8 * (
                int(args.blocks) + int(args.warmup)
            )
            if args.arm == "pipeline_b3":
                rt, graph, keep = make_candidate(allocation_context)
                config = {"pipeline_ranges": [[0, 16], [16, 32], [32, 48]], "gather_y": 4}
            else:
            ...
            payload.update({
                ...
                "environment": environment_snapshot((
                    REPO / "pro_research" / "s100_phase40_h8_pipeline.py",
                    REPO / "pro_research" / "s100_phase40_h8_pipeline_kernels.py",
                )),
