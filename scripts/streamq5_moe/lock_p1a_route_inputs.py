...
P0_VERIFY = ROOT / "reports/streamq5_moe/p0_model_quality_verification.json"
ARTIFACT = ROOT / "reports/runs/streamq5_moe/p1a_fresh_route_input_ids.safetensors"
OUTPUT = ROOT / "reports/streamq5_moe/p1a_route_input_lock.json"
PRIOR_INPUTS = (
...
if __name__ == "__main__":
    if OUTPUT.exists() or ARTIFACT.exists():
        raise FileExistsError("refusing to overwrite P1A route input lock")
    p0 = json.loads(P0_VERIFY.read_text(encoding="utf-8"))
    if p0.get("status") != "p0_quality_verification_pass":
        raise RuntimeError("independent STREAMQ5 P0 quality pass required")
    ...
    if set(tensors) != set(DOMAINS) or any(tuple(value.shape) != (1, TOKENS) for value in tensors.values()):
        raise RuntimeError("P1A route input contract failed")
    prior = [load_file(path) for path in PRIOR_INPUTS]
    disjoint = True
    for domain in DOMAINS:
        chunks = tensors[domain].reshape(-1, 128)
        for old in prior:
            old_rows = [value for name, tensor in old.items() if name.endswith(f"_{domain}") for value in tensor]
            disjoint &= not any((left == right).all().item() for left in chunks for right in old_rows)
    if not disjoint:
        raise RuntimeError("P1A route context overlaps a prior decision context")

    ARTIFACT.parent.mkdir(parents=True, exist_ok=True)
    save_file(tensors, ARTIFACT, metadata={"kind": "streamq5_moe_p1a_fresh_route_inputs", "routes_opened": "false"})
    payload = {
        "kind": "streamq5_moe_p1a_route_input_lock", "locked_utc": datetime.now(timezone.utc).isoformat(),
