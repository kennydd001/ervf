...
def expected_test_status(validation_relative: float, test_relative: float, finite: bool) -> tuple[str, bool]:
    if finite and validation_relative <= 0.02 and test_relative <= 0.02:
        return "p0_quality_pass", True
    return "p0_quality_closed", False


def audit_result(audit: Audit, split: str, result: dict, lock: dict, validation: dict) -> None:
    prefix = f"{split}:"
    audit.check(prefix + " kind", result.get("kind") == "streamq5_moe_p0_full_depth_model_quality")
    ...
    if split == "validation":
        expected_validation = "p0_validation_pass_test_authorized" if controls.get("all_finite") is True and primary_relative <= 0.025 else "p0_validation_closed"
...
def main() -> None:
    audit = Audit()
    prereg = REPORT_DIR / "P0_MODEL_QUALITY_PREREGISTRATION.md"
    lock_path = REPORT_DIR / "p0_input_lock.json"
    evaluator_lock_path = REPORT_DIR / "p0_evaluator_lock.json"
    input_path = ROOT / "reports/runs/streamq5_moe/p0_fresh_input_ids.safetensors"
    ...
    evaluator = ROOT / "scripts/streamq5_moe/run_p0_model_quality.py"
    validation_path = REPORT_DIR / "p0_validation_model_quality.json"
    test_path = REPORT_DIR / "p0_test_model_quality.json"
    ...
    audit.check("prior artifact hashes", sha256(old_lock_path) == lock["previous_coretail_lock_sha256"] and sha256(old_input_path) == lock["previous_coretail_input_sha256"] and sha256(q4_lock_path) == lock["previous_streamq4_lock_sha256"] and sha256(q4_input_path) == lock["previous_streamq4_input_sha256"])
    ...
    old_tensors = load_file(old_input_path); q4_tensors = load_file(q4_input_path)
    prior_disjoint = True
    for domain in DOMAINS:
        current_rows = list(tensors[f"validation_{domain}"]) + list(tensors[f"test_{domain}"])
        prior_rows = list(old_tensors[f"validation_{domain}"]) + list(old_tensors[f"test_{domain}"]) + list(q4_tensors[f"validation_{domain}"]) + list(q4_tensors[f"test_{domain}"])
    ...
    status = "p0_quality_verification_pass" if passed == total else "p0_quality_verification_fail"
    payload = {
        "kind": "streamq5_moe_p0_independent_verification",
        "status": status,
        ...
        "conclusion": "P0 quality passes independently; physical bank, cache, and kernel phases are authorized, not wall-clock by P0 alone.",
    }
    output = REPORT_DIR / "p0_model_quality_verification.json"
    output.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    report = REPORT_DIR / "P0_MODEL_QUALITY_VERIFICATION.md"
    report.write_text(
        "\n".join(
            [
                "# STREAMQ5-MoE P0 - onafhankelijke kwaliteitsverificatie",
                "",
                ...
                "Dit opent uitsluitend fysieke bank-, cache- en kernelfasen; P0 bewijst geen wall-clock.",
                "",
    ...
    if status != "p0_quality_verification_pass":
        raise SystemExit(1)
