...
PREREG = ROOT / "reports/streamq5_moe/P1D_CORRECTED_PHYSICAL_BANK_PREREGISTRATION.md"
P1C_VERIFY = ROOT / "reports/streamq5_moe/p1c_cache_verification.json"
PRODUCER_LOCK = ROOT / "reports/streamq5_moe/p1d_bank_producer_lock.json"
RUN_DIR = ROOT / "reports/runs/streamq5_moe/p1d_q5_bank"
LAYER_DIR = ROOT / "reports/streamq5_moe/p1d_bank_layers"
RESULT = ROOT / "reports/streamq5_moe/p1d_physical_bank_result.json"
REPORT = ROOT / "reports/streamq5_moe/P1D_PHYSICAL_BANK_REPORT.md"
...
if __name__ == "__main__":
    ...
    if RESULT.exists() or REPORT.exists():
        raise FileExistsError("refusing to overwrite completed P1D result")
    p1c = json.loads(P1C_VERIFY.read_text(encoding="utf-8"))
    producer_lock = json.loads(PRODUCER_LOCK.read_text(encoding="utf-8"))
    if p1c.get("status") != "p1c_cache_verification_pass":
        raise RuntimeError("independent P1C pass required")
    if sha256(Path(__file__)) != producer_lock["producer_sha256"] or sha256(PREREG) != producer_lock["preregistration_sha256"] or sha256(P1C_VERIFY) != producer_lock["p1c_verification_sha256"]:
        raise ValueError("P1D producer lock mismatch")
    ...
    for layer in selected:
        ...
        payload = {
            "kind": "streamq5_moe_p1d_layer", "layer": layer, "completed_utc": datetime.now(timezone.utc).isoformat(),
    ...
    result = {
        "kind": "streamq5_moe_p1d_physical_bank", "status": "physical_bank_built_pending_independent_verification",
        "completed_utc": datetime.now(timezone.utc).isoformat(),
        "inputs": {"preregistration_sha256": sha256(PREREG), "p1c_verification_sha256": sha256(P1C_VERIFY), "producer_lock_sha256": sha256(PRODUCER_LOCK), "producer_sha256": sha256(Path(__file__)), "model_index_sha256": sha256(MODEL / "model.safetensors.index.json")},
        "format": {"header_bytes": HEADER_BYTES, "record_alignment": 4096, "matrix_record_bytes": RECORD_BYTES, "expert_bytes": EXPERT_BYTES, "layer_bytes": LAYER_BYTES, "code_mapping": "unsigned=code+15; eight little-order 5-bit codes in five bytes", "scales": "raw_bfloat16_group128", "decode_semantics": "codes selected with FP32 scale; persisted BF16 scale used for dequant; BF16 output"},
