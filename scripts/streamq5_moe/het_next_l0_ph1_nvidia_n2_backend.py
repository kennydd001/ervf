#!/usr/bin/env python3
"""Direct NVRTC compiler and CUDA Driver backend for PH1 NVIDIA N2.
...
from het_next_l0_ph1_nvidia_n2_common import (
    ARGUMENT_MAPS, BUFFER_TABLE, LAUNCHES, RESOURCE_STAGES, host_sample, sha,
)

ROOT = Path(__file__).resolve().parents[2]
CUDA_SOURCE = Path(__file__).with_name("het_next_l0_ph1_nvidia_n2_kernels.cu")
...
def compile_one_program(source: bytes):
    ...
    evidence = {"kind": "ph1_nvidia_n2_one_program_compile", "options": list(OPTIONS), "ledger": [], "artifacts": {}, "cudart_loaded": False}
    ...
    try:
        ...
        code = library.nvrtcCreateProgram(C.byref(program), source, b"ph1_nvidia_n2.cu", 0, None, None)
...
class DriverBackend:
    ...
    def _bind(self):
        ...
        self.abi = {name: {"argtypes": [str(arg) for arg in args], "restype": str(result)}
                    for name, (args, result) in table.items()}
    ...
    def _cuda_related_modules():
        ...
        for mapping in psutil.Process().memory_maps(grouped=True):
            ...
            if key not in seen and any(token in name for token in ("cuda", "nvrtc", "cupy")) and Path(path).is_file():
                seen.add(key)
