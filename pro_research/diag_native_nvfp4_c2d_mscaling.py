"""C2d: how far does the free-M dimension go? M in {1,2,4,8,16}, cold.
...
OUT = REPO / "pro_research" / "results" / "native_nvfp4" / "C2D_M_SCALING.json"
...
def main() -> int:
    payload: dict[str, Any] = {
        "kind": "s100_native_nvfp4_c2d_m_scaling",
        "status": "started",
        "started_utc": utc_now(),
    ...
    try:
        ...
        for name, n, k, reps, calls in SHAPES:
            ...
            for m in M_LIST:
                rec[f"M{m}_over_M1"] = rec[f"M{m}_p50_ms"] / rec["M1_p50_ms"]
                rec[f"M{m}_per_token_ms"] = rec[f"M{m}_p50_ms"] / m
                rec[f"M{m}_speedup_per_token_vs_ervf"] = e / (rec[f"M{m}_p50_ms"] / m)
                # 1.0 = perfect sharing (M positions for the price of one pass)
                rec[f"M{m}_sharing_efficiency"] = m / (rec[f"M{m}_p50_ms"] / rec["M1_p50_ms"]) / m
            arms[name] = rec
            del bs_list
            torch.cuda.empty_cache()

        ervf_tok = sum(ERVF_MS[n] * c for n, _, _, _, c in SHAPES)
        per_tok = {"ervf": ervf_tok}
        for m in M_LIST:
            per_tok[f"native_M{m}"] = sum(arms[n][f"M{m}_p50_ms"] / m * c
                                          for n, _, _, _, c in SHAPES)
            per_tok[f"saving_M{m}_ms"] = ervf_tok - per_tok[f"native_M{m}"]
            # against the 19.60 ms V18 record
            per_tok[f"projected_token_ms_M{m}"] = 19.60 - per_tok[f"saving_M{m}_ms"]
            per_tok[f"projected_tok_s_M{m}"] = 1000.0 / per_tok[f"projected_token_ms_M{m}"]

        payload.update({
            "arms": arms,
            "per_token_ms_over_nvfp4_shapes": per_tok,
            "projection_note": "applies the measured per-token rate ONLY to the four already-NVFP4 shapes (6.302 ms of the 19.60 ms V18 record); the rest of the token is untouched, so this is a floor on what an M-way path would give, not a prediction",
            "status": "measured",
    ...
    if payload.get("arms"):
        for n, r in payload["arms"].items():
            ratios = "  ".join(f"M{m}/M1={r[f'M{m}_over_M1']:.3f}" for m in M_LIST)
            print(f"  {n:12s} ervf={r['ervf_p50_ms_same_protocol']*1000:8.2f}us "
                  f"M1={r['M1_p50_ms']*1000:8.2f}us | {ratios}")
