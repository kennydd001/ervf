#!/usr/bin/env python3
"""C0-R5 static contract and real capability-only backend; source/validation closed."""
from __future__ import annotations
import argparse, hashlib, json, math, os, struct, sys, time, traceback, uuid
from pathlib import Path

ROOT=Path(__file__).resolve().parents[2];S=ROOT/'scripts/streamq5_moe';R=ROOT/'reports/streamq5_moe'
LOCK=R/'het_next_l0_c0r5_runner_lock.json';VLOCK=R/'het_next_l0_c0r5_verifier_lock.json';VER=S/'verify_het_next_l0_c0r5_capability_contract.py';PREF=S/'preflight_het_next_l0_c0r5_static.py';KERNEL=S/'het_next_l0_c0r5_ergv_kernels.py'
...
OUT=ROOT/'reports/runs/streamq5_moe/het_next_l0_c0r5_capability';CAP=OUT/'c0r5_capability.json';FAIL=OUT/'c0r5_capability_failure.json';COM=OUT/'c0r5_capability_commit.json'
...
def d2_specs(header):
 ...
 return out
class SealedReader:
 def __init__(self,path,specs,tests_open=False):self.path=Path(path);self.specs=dict(specs);self.tests_open=tests_open;self.ledger=[]
 def read(self,key,phase):
  if key not in self.specs:raise PermissionError('key_not_allowlisted')
  s=self.specs[key];row=s['row']
  if row>0 and not self.tests_open:raise PermissionError('test_payload_sealed')
  if (phase=='validation')!=(row==0):raise PermissionError('phase_row_mismatch')
  a,b=s['absolute'];entry={'key':key,'row':row,'phase':phase,'absolute':[a,b],'completed':False};self.ledger.append(entry)
...
def intel_capability(kernel_source):
 import ctypes as C
 from ctypes import wintypes
 CL_SUCCESS=0;CL_DEVICE_TYPE_GPU=4;CL_PLATFORM_NAME=0x0902;CL_PLATFORM_VENDOR=0x0903;CL_DEVICE_NAME=0x102B;CL_DEVICE_VENDOR=0x102C;CL_DEVICE_EXTENSIONS=0x1030;CL_DEVICE_GLOBAL_MEM_SIZE=0x101F;CL_DEVICE_MAX_MEM_ALLOC_SIZE=0x1010;CL_CONTEXT_PLATFORM=0x1084;CL_TRUE=1
 lib=C.WinDLL('OpenCL.dll');cleanup=[];counts={'context':0,'queue':0,'program':0,'kernel':0,'usm':0,'event':0}
 ...
 lib.clReleaseEvent.argtypes=[C.c_void_p];lib.clReleaseKernel.argtypes=[C.c_void_p];lib.clReleaseProgram.argtypes=[C.c_void_p];lib.clReleaseCommandQueue.argtypes=[C.c_void_p];lib.clReleaseContext.argtypes=[C.c_void_p]
 def ck(x,n):
 ...
 for p in plats:
  ...
  for d in ds:
   ...
   if 'Intel' in vendor and ('Arc' in name or '140T' in name):chosen=(p,d);identity={'platform':info(p,lib.clGetPlatformInfo,CL_PLATFORM_NAME),'platform_vendor':info(p,lib.clGetPlatformInfo,CL_PLATFORM_VENDOR),'device':name,'vendor':vendor};break
  if chosen:break
 ...
 props=(C.c_ssize_t*3)(CL_CONTEXT_PLATFORM,int(p),0);err=C.c_int();devices=(C.c_void_p*1)(d);ctx=lib.clCreateContext(props,1,devices,None,None,C.byref(err));ck(err.value,'context');counts['context']=1
 q=lib.clCreateCommandQueue(ctx,d,0,C.byref(err));ck(err.value,'queue');counts['queue']=1
 src=kernel_source.encode();sp=C.c_char_p(src);sl=C.c_size_t(len(src));prog=lib.clCreateProgramWithSource(ctx,1,C.byref(sp),C.byref(sl),C.byref(err));ck(err.value,'program');counts['program']=1;opts=b'-cl-std=CL2.0 -cl-fp32-correctly-rounded-divide-sqrt';ck(lib.clBuildProgram(prog,1,devices,opts,None,None),'build')
 lib.clCreateKernel.restype=C.c_void_p;k=lib.clCreateKernel(prog,b'c0r5_usm_sentinel',C.byref(err));ck(err.value,'kernel');counts['kernel']=1
 lib.clGetExtensionFunctionAddressForPlatform.restype=C.c_void_p
 ...
 ptr=alloc(ctx,None,4096,64,C.byref(err));ck(err.value,'usm_alloc');counts['usm']=1;arr=(C.c_ubyte*4096).from_address(ptr);before=bytes((i*17+3)&255 for i in range(4096));arr[:]=before;ck(setptr(k,0,ptr),'setptr');n=C.c_uint(4096);ck(lib.clSetKernelArg(k,1,C.sizeof(n),C.byref(n)),'set_n');gs=C.c_size_t(4096);ls=C.c_size_t(64);event=C.c_void_p();ck(lib.clEnqueueNDRangeKernel(q,k,1,None,C.byref(gs),C.byref(ls),0,None,C.byref(event)),'enqueue');counts['event']=1;ck(lib.clFinish(q),'finish');after=bytes(arr);expected=bytes((x^0x5a) for x in before);ok=after==expected
 lib.clReleaseEvent(event);counts['event']=0;ck(free(ctx,ptr),'free');counts['usm']=0;lib.clReleaseKernel(k);counts['kernel']=0;lib.clReleaseProgram(prog);counts['program']=0;lib.clReleaseCommandQueue(q);counts['queue']=0;lib.clReleaseContext(ctx);counts['context']=0
 if not ok:raise RuntimeError('intel_sentinel')
 return {**identity,'extensions':ext,'required_extensions':list(required),'buffer_bytes':4096,'alignment':64,'before_sha256':hashlib.sha256(before).hexdigest(),'after_sha256':hashlib.sha256(after).hexdigest(),'expected_after_sha256':hashlib.sha256(expected).hexdigest(),'copyless_host_usm':True,'used_cl_mem':False,'used_enqueue_write':False,'used_migrate':False,'compiler_options':opts.decode(),'cleanup_counts':counts}
def nvidia_capability(kernel_source):
 import cupy as cp
 dev=cp.cuda.Device();props=cp.cuda.runtime.getDeviceProperties(dev.id);before=bytes((i*29+7)&255 for i in range(4096));x=cp.asarray(bytearray(before),dtype=cp.uint8);mod=cp.RawModule(code=kernel_source,options=('--std=c++14','--fmad=false'),name_expressions=('c0r5_cuda_sentinel',));fn=mod.get_function('c0r5_cuda_sentinel');fn((16,),(256,),(x,4096));cp.cuda.runtime.deviceSynchronize();after=bytes(cp.asnumpy(x));expected=bytes(((v+0x33)&255) for v in before);ok=after==expected;del x,fn,mod;cp.get_default_memory_pool().free_all_blocks()
 ...
 return {'device_id':dev.id,'name':props['name'].decode() if isinstance(props['name'],bytes) else str(props['name']),'pci_bus_id':pci,'compute_capability':[int(props['major']),int(props['minor'])],'total_global_mem':int(props['totalGlobalMem']),'buffer_bytes':4096,'before_sha256':hashlib.sha256(before).hexdigest(),'after_sha256':hashlib.sha256(after).hexdigest(),'expected_after_sha256':hashlib.sha256(expected).hexdigest(),'compiler_options':['--std=c++14','--fmad=false'],'cleanup_pool_used_bytes':int(cp.get_default_memory_pool().used_bytes())}
def capability():
 ...
 if OUT.exists():raise FileExistsError('capability_output_not_clean')
 from het_next_l0_c0r5_ergv_kernels import INTEL_CAPABILITY_SOURCE,NVIDIA_CAPABILITY_SOURCE
 started=time.time_ns();intel=intel_capability(INTEL_CAPABILITY_SOURCE);nvidia=nvidia_capability(NVIDIA_CAPABILITY_SOURCE);sync=simulate_sync();payload={'kind':'het_next_l0_c0r5_capability','status':'capability_positive','runner_sha256':sha(__file__),'runner_lock_sha256':sha(LOCK),'verifier_sha256':sha(VER),'verifier_lock_sha256':sha(VLOCK),'kernel_sha256':sha(KERNEL),'started_ns':started,'ended_ns':time.time_ns(),'device_payload_reads':0,'weight_payload_reads':0,'sentinel_total_bytes':8192,'intel':intel,'nvidia':nvidia,'sync':sync,'topology_required_lps':list(LP),'claim_boundary':'capability-only; no weights, benchmark, source-build, validation or component result'};atomic_json(CAP,payload);commit={'kind':'het_next_l0_c0r5_capability_commit','files':{CAP.name:{'bytes':CAP.stat().st_size,'sha256':sha(CAP)}}};atomic_json(COM,commit);return payload
def main():
 global ARGS
 ...
 if ARGS.phase=='contract':print(json.dumps({'kind':'c0r5_contract','lockcheck':lockcheck(),'schedule_count':len(schedule()),'sync':simulate_sync()}));return 0
 ...
 try:o=capability();print(o['status']);return 0
  ...
  atomic_json(FAIL,{'kind':'het_next_l0_c0r5_capability_failure','error_type':type(e).__name__,'error':str(e),'traceback':traceback.format_exc(),'runner_sha256':sha(__file__),'runner_lock_sha256':sha(LOCK),'no_weight_payload':True});raise
