...
ROOT=Path(__file__).resolve().parents[2]; R=ROOT/'reports/streamq5_moe'; D=ROOT/'reports/runs/streamq5_moe/het_next_cap0r3_dual_device_cohabitation'
RESULT=D/'cap0r3_result.json'; COMMIT=D/'cap0r3_commit.json'; FAILURE=D/'cap0r3_failure.json'; VLOCK=R/'het_next_cap0r3_verifier_lock.json'
...
def verify():
 ...
 checks['kind_status']=r.get('kind')=='het_next_cap0r3_dual_device_cohabitation' and r.get('status')=='dual_device_cohabitation_positive' and r.get('claim_boundary')=='4KiB dual-device lifecycle/correctness only; no performance or model claim'
