...
RUN_DIR = ROOT / "reports" / "runs" / "streamq5_moe" / "port80b_t0r4_official_layer0"
...
def router_artifacts(layer: Qwen3NextDecoderLayer, normed: torch.Tensor) -> dict[str, torch.Tensor]:
    ...
    margins = pre_weights[:, -1] - torch.topk(probs, TOP_K + 1, dim=-1).values[:, -1]
...
def reference(run_index: int) -> dict[str, Any]:
    ...
    raw_path = RUN_DIR / f"t0r4_run_{run_index}_raw.safetensors"
    result_path = RUN_DIR / f"t0r4_run_{run_index}_result.json"
    ...
    result = {"kind": "port80b_t0r4_official_layer0_reference_and_bank_stage", "run_index": run_index,
              "status": "run1_complete_pending_clean_replay" if run_index == 1 else "t0r4_reference_bank_stage_candidate_pass",
    ...
    if run_index == 2:
        prior = json.loads((RUN_DIR / "t0r4_run_1_result.json").read_text(encoding="utf-8"))
        prior_raw = safe_open(RUN_DIR / "t0r4_run_1_raw.safetensors", framework="pt", device="cpu")
...
def smoke() -> dict[str, Any]:
    ...
    return {"kind": "port80b_t0r4_cpu_smoke", "pass": header["pass"] and codec["pass"] and meta_contract,
