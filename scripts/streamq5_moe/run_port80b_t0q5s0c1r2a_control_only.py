#!/usr/bin/env python3
"""S0-C1-R2A standalone real-record control sentinel. No model, GPU, bank, or quality rerun."""
...
ROOT=Path(__file__).resolve().parents[2];S=ROOT/'scripts/streamq5_moe';R=ROOT/'reports/streamq5_moe';D=ROOT/'reports/runs/streamq5_moe/port80b_t0q5s0c1r2a_control_only';RAW=D/'s0c1r2a_raw.safetensors';RES=D/'s0c1r2a_result.json';COM=D/'s0c1r2a_commit.json';FAIL=D/'s0c1r2a_failure.json';BAD=D/'failed_attempts';PR=R/'PORT80B_T0Q5S0C1R2A_CONTROL_ONLY_PREREGISTRATION_2026-08-13.md';LOCK=R/'port80b_t0q5s0c1r2a_runner_lock.json';VL=R/'port80b_t0q5s0c1r2a_verifier_lock.json';VER=S/'verify_port80b_t0q5s0c1r2a_control_only.py';DEP=R/'port80b_t0r4_dependency_execution_lock.json';DIAG=R/'PORT80B_T0Q5S0R5_CONTROL_DIAGNOSIS_2026-08-13.md';CLOSED=R/'PORT80B_T0Q5S0C1R1_CLOSED_PREFLIGHT_RESULT_2026-08-13.json';R2CLOSED=R/'PORT80B_T0Q5S0C1R2_CLOSED_PREFLIGHT_RESULT_2026-08-13.json';R1AD=ROOT/'reports/runs/streamq5_moe/port80b_t0q5s0c1r1a_control_only';R1AFAIL=R1AD/'s0c1r1a_failure.json';R1ARAW=R1AD/'failed_attempts/105c1ea0873d48a896624bf44bb1f7fe_s0c1r1a_raw.safetensors.2cf2eb2e5b454a2c8d5aa93d57cc41b1.inprogress';R5D=ROOT/'reports/runs/streamq5_moe/port80b_t0q5s0r5_selected_route_validation';R5RAW=R5D/'s0r5_raw.safetensors';R5RES=R5D/'s0r5_result.json';R5COM=R5D/'s0r5_commit.json';SHARD=Path.home()/'.cache/huggingface/hub/models--Qwen--Qwen3-Coder-Next/snapshots/a19358a7659bd1f564300250ee189120c49a562f/model-00001-of-00040.safetensors';KEY='model.layers.0.mlp.shared_expert.down_proj.weight';SHARD_SHA='8e9a517133bfbdc6806cf8b61793055a260efeb68e6e019fd90e4bbb1b665d0a';ACK='T0Q5S0C1R2A_CONTROL_ONLY_AFTER_SOURCE_AUDIT';TOKEN='S0C1R2A_AUTHORIZED_AFTER_REPAIR_GO';MAX=1024*1024
...
def bindings():return {'runner_sha256':sha(__file__),'verifier_sha256':sha(VER),'verifier_lock_sha256':sha(VL),'prereg_sha256':sha(PR),'dependency_lock_sha256':sha(DEP),'diagnosis_sha256':sha(DIAG),'closed_preflight_sha256':sha(CLOSED),'r2_closed_preflight_sha256':sha(R2CLOSED),'r1a_failure_sha256':sha(R1AFAIL),'r1a_quarantined_raw_sha256':sha(R1ARAW),'r5_raw_sha256':sha(R5RAW),'r5_result_sha256':sha(R5RES),'r5_commit_sha256':sha(R5COM),'shard_sha256':sha(SHARD)}
def lockcheck():
...
def execute():
 ...
 if committed():return {'kind':'port80b_t0q5s0c1r2a_already_complete','status':'already_complete'}
 moved=recover()
...
def main():
 ...
 if z.phase=='lockcheck':print(json.dumps({'kind':'s0c1r2a_lockcheck',**lockcheck(),'physical_actions':False},indent=2));return 0
