...
def recover_transaction(directory: Path):
    ...
    result, commit = directory / "cap0r1_result.json", directory / "cap0r1_commit.json"
    valid = False
    ...
    for item in list(directory.glob('*.inprogress')) + [x for x in (result, commit, directory/'cap0r1_failure.json') if x.exists()]:
