"""Independent verifier for audited E100-MRHS256 v2 evidence."""
...
def main() -> int:
    ...
    for r in recs:
        ...
        exact = bool(bs) and all(int(b.get("mismatch_count", -1)) == 0
                                 and int(b.get("determinism_mismatch_count", -1)) == 0
                                 and bool(b.get("finite")) for b in bs)
        all_exact = all_exact and exact
        ra, rb, cm = (float(r.get(x, math.nan)) for x in ("reference_a_ms", "reference_b_ms", "mrhs256_ms"))
    ...
    if not all_exact or not all_families: expected = "correctness_failed"
    elif mode == "smoke": expected = "smoke_pass"
    elif perf_pass: expected = "mrhs256_candidate"
    else: expected = "micro_null"
    if p.get("status") != expected: errors.append(f"status mismatch file={p.get('status')} recomputed={expected}")
    if p.get("runner_revision") != "v2_explicit_families": errors.append("result was not produced by audited v2 runner")
    if (p.get("family_support") or {}) != families: errors.append("family support evidence mismatch")

    out = {"kind": "pro_e100_mrhs256_v2_independent_verification",
           "source": str(SRC.relative_to(ROOT)), "source_status": p.get("status"),
           "recomputed_status": expected, "all_exact": all_exact,
           "family_support": families, "all_eight_families": all_families,
           "n4": n4, "n16": n16, "performance_gates": perf,
           "performance_pass": perf_pass, "errors": errors, "passed": not errors}
