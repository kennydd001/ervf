#!/usr/bin/env python3
"""Direct NVRTC compiler and CUDA Driver backend for PH1 NVIDIA N3.
...
from het_next_l0_ph1_nvidia_n3_common import (
    ARGUMENT_MAPS, BUFFER_TABLE, LAUNCHES, RESOURCE_STAGES, host_sample, sha,
)

ROOT = Path(__file__).resolve().parents[2]
CUDA_SOURCE = Path(__file__).with_name("het_next_l0_ph1_nvidia_n3_kernels.cu")
...
def compile_one_program(source: bytes):
    ...
    evidence = {"kind": "ph1_nvidia_n3_one_program_compile", "options": list(OPTIONS), "ledger": [], "artifacts": {}, "cudart_loaded": False}
    ...
    try:
        ...
        active="nvrtcCreateProgram"; code = library.nvrtcCreateProgram(C.byref(program), source, b"ph1_nvidia_n3.cu", 0, None, None)
