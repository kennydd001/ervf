...
RUNNER_LOCK = REPORTS / "port80b_t0r7r_runner_lock.json"
VERIFIER = ROOT / "scripts" / "streamq5_moe" / "verify_port80b_t0r7r_official_cpu_route_repro.py"
VERIFIER_LOCK = REPORTS / "port80b_t0r7r_verifier_lock.json"
RECORD_ARTIFACT = RUN_DIR / "layer0_513_real_q5_records.bin"
RECORD_MANIFEST = RUN_DIR / "layer0_513_real_q5_records_manifest.json"
ACK = "T0R7R_OFFICIAL_CPU_CAPTURE_AFTER_PREFLIGHT"
...
def execution_lock_check() -> dict[str, Any]:
    if not RUNNER_LOCK.is_file() or not VERIFIER.is_file() or not VERIFIER_LOCK.is_file():
        raise RuntimeError("R7-R execution provenance file missing")
    ...
    if mismatched: raise RuntimeError(f"R7-R execution lock mismatch: {mismatched}")
...
def reference(run_index: int) -> dict[str, Any]:
    ...
    raw_path = RUN_DIR / f"t0r7r_capture_{run_index}_raw.safetensors"
    result_path = RUN_DIR / f"t0r7r_capture_{run_index}_result.json"
    ...
    result = {"kind": "port80b_t0r7r_official_cpu_route_repro_reference_and_bank_stage", "run_index": run_index,
...
def smoke() -> dict[str, Any]:
    ...
    return {"kind": "port80b_t0r7r_cpu_smoke", "pass": header["pass"] and codec["pass"] and meta_contract,
...
if __name__ == "__main__":
    ...
    if args.phase == "lockcheck":
        payload = {"kind":"port80b_t0r7r_exact_execution_lockcheck","pass":execution_lock_check()["pass"],
        ...
        try:
            ...
            RUN_DIR.mkdir(parents=True, exist_ok=True)
            failure = RUN_DIR / f"t0r7r_capture_{args.run_index}_failure.json"
            if failure.exists():
            ...
            if partial.exists():
                ...
                target = failed_dir / f"t0r7r_capture_{args.run_index}_partial_bank.bin"
            ...
            memory = psutil.Process().memory_info()
            evidence = {"kind":"port80b_t0r7r_official_cpu_route_repro_failure", "status":"valid_negative_or_blocked_not_pass",
