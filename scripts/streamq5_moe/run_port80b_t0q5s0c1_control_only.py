#!/usr/bin/env python3
"""S0-C1 standalone real-record control sentinel. No model, GPU, bank, or quality rerun."""
from __future__ import annotations
import argparse,gc,hashlib,json,math,os,shutil,traceback,uuid
...
ROOT=Path(__file__).resolve().parents[2];S=ROOT/'scripts/streamq5_moe';R=ROOT/'reports/streamq5_moe';D=ROOT/'reports/runs/streamq5_moe/port80b_t0q5s0c1_control_only';RAW=D/'s0c1_raw.safetensors';RES=D/'s0c1_result.json';COM=D/'s0c1_commit.json';FAIL=D/'s0c1_failure.json';BAD=D/'failed_attempts';PR=R/'PORT80B_T0Q5S0C1_CONTROL_ONLY_PREREGISTRATION_2026-08-13.md';LOCK=R/'port80b_t0q5s0c1_runner_lock.json';VL=R/'port80b_t0q5s0c1_verifier_lock.json';VER=S/'verify_port80b_t0q5s0c1_control_only.py';DEP=R/'port80b_t0r4_dependency_execution_lock.json';DIAG=R/'PORT80B_T0Q5S0R5_CONTROL_DIAGNOSIS_2026-08-13.md';R5D=ROOT/'reports/runs/streamq5_moe/port80b_t0q5s0r5_selected_route_validation';R5RAW=R5D/'s0r5_raw.safetensors';R5RES=R5D/'s0r5_result.json';R5COM=R5D/'s0r5_commit.json';SHARD=Path.home()/'.cache/huggingface/hub/models--Qwen--Qwen3-Coder-Next/snapshots/a19358a7659bd1f564300250ee189120c49a562f/model-00001-of-00040.safetensors';KEY='model.layers.0.mlp.shared_expert.down_proj.weight';SHARD_SHA='8e9a517133bfbdc6806cf8b61793055a260efeb68e6e019fd90e4bbb1b665d0a';ACK='T0Q5S0C1_CONTROL_ONLY_AFTER_SOURCE_AUDIT';TOKEN='S0C1_IMPLEMENTATION_SOURCE_AUDIT_GO';MAX=1024*1024
...
def bindings():return {'runner_sha256':sha(__file__),'verifier_sha256':sha(VER),'verifier_lock_sha256':sha(VL),'prereg_sha256':sha(PR),'dependency_lock_sha256':sha(DEP),'diagnosis_sha256':sha(DIAG),'r5_raw_sha256':sha(R5RAW),'r5_result_sha256':sha(R5RES),'r5_commit_sha256':sha(R5COM),'shard_sha256':sha(SHARD)}
def lockcheck():
...
def execute():
 ...
 if committed():return {'kind':'port80b_t0q5s0c1_already_complete','status':'already_complete'}
...
def main():
 ...
 if z.phase=='lockcheck':print(json.dumps({'kind':'s0c1_lockcheck',**lockcheck(),'physical_actions':False},indent=2));return 0
