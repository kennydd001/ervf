...
def main():
 ...
 try:
  d={}
  for r in ROLES:
   q=od/f'CAP_{a.profile.upper()}_{r.upper()}.json';x=json.loads(q.read_text());d[r]=x
  st={r:d[r].get('status') for r in ROLES}
  if all(v=='measured' for v in st.values()):
  ...
  elif any(v=='infeasible_vram' for v in st.values()):
   # Clean, complete verdict: at least one arm genuinely does not fit in
   # the target GPU's VRAM (real allocation attempt, not a pre-check).
   p.update({'status':'infeasible_vram','role_status':st,'detail':{r:(d[r].get('error') or {}).get('message') for r in ROLES if st[r]!='measured'},'completed_utc':utc_now()})
  else:raise RuntimeError(f'arm statuses: {st}')
