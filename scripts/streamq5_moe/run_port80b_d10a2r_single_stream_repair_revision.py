...
COMPILE_OUT = R / "port80b_d10a2r_single_stream_repair_revision_preflight.json"
COMPILE_REPORT = R / "PORT80B_D10A2R_SINGLE_STREAM_REPAIR_REVISION_PREFLIGHT_REPORT_2026-08-13.md"
COMPONENT_OUT = R / "port80b_d10a2r_single_stream_repair_revision.json"
ENDURANCE_OUT = R / "port80b_d10a2r_single_stream_repair_revision_endurance_10k.json"
...
def compile_phase() -> None:
    ...
    result = {
        "kind": "port80b_d10a2r_single_stream_repair_revision_cpu_preflight",
    ...
    COMPILE_REPORT.write_text(
        "# PORT80B-D10A2-R CPU-only preflight report\n\n"
        ...
        "and did not scan the bank. The executable D10A2-R component gate is implemented; "
...
def component_phase() -> None:
    ...
    if COMPONENT_OUT.exists() or REPORT.exists():
        raise FileExistsError("refusing to overwrite D10A2-R component evidence")
    ram_before = int(psutil.virtual_memory().available)
    if ram_before < MIN_RAM_BEFORE:
        raise RuntimeError(f"hard stop: available RAM below D10A2-R frozen {START_RAM_GATE}-byte start gate")
    ...
    result = {
        "kind": "port80b_d10a2r_single_stream_repair_revision",
        "completed_utc": datetime.now(timezone.utc).isoformat(),
    ...
    REPORT.write_text(
        "# PORT80B-D10A2-R — deterministic single-stream repair revision report\n\n"
...
def endurance_phase(acknowledgement: str | None) -> None:
    locked_compile()
    raise RuntimeError(
        "D10A2-R endurance is unconditionally fail-closed; a new post-component preregistration and runner are required"
    )
