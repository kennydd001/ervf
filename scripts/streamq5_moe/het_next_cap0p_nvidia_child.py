...
def main():
 ...
 try:
  be=Backend();emit({'type':'ready','device':'nvidia','pid':os.getpid(),'topology':top,'identity':be.identity});x=words();want=expected('nvidia',x)
  for epoch in range(1,4):
   if command()!=['START',str(epoch)]:raise RuntimeError('command')
   s,d,o=be.run(x);emit({'type':'result','device':'nvidia','epoch':epoch,'submit_ns':s,'done_ns':d,'output_words':o,'sha256':digest(o),'different_words':sum(i!=j for i,j in zip(o,want))})
  if command()!=['STOP']:raise RuntimeError('stop')
  cleanup=be.close();be=None;emit({'type':'cleanup','device':'nvidia','rows':cleanup});return 0
 except BaseException as e:emit({'type':'failure','device':'nvidia','error':f'{type(e).__name__}:{e}','traceback':traceback.format_exc()});return 2
 finally:
