#!/usr/bin/env python3
"""Direct NVRTC compiler and CUDA Driver backend for PH1 NVIDIA N4.
...
from het_next_l0_ph1_nvidia_n4_common import (
    ARGUMENT_MAPS, BUFFER_TABLE, LAUNCHES, RESOURCE_STAGES, host_sample, sha,
)

ROOT = Path(__file__).resolve().parents[2]
CUDA_SOURCE = Path(__file__).with_name("het_next_l0_ph1_nvidia_n4_kernels.cu")
...
def compile_one_program(source: bytes):
    ...
    evidence = {"kind": "ph1_nvidia_n4_one_program_compile", "options": list(OPTIONS), "ledger": [], "artifacts": {}, "cudart_loaded": False}
    program = C.c_void_p(); failure = None; active = None
    intended = ("nvrtcVersion", "nvrtcCreateProgram", "nvrtcCompileProgram",
    ...
    if file_sha(NVRTC_DLL) != NVRTC_DLL_SHA or file_sha(NVRTC_HEADER) != NVRTC_HEADER_SHA:
        evidence["ledger"] = [{"sequence": index, "op": op, "attempted": False, "code": "not_attempted", "program_identity": 0}
                              for index, op in enumerate((*intended, "nvrtcDestroyProgram"))]
    ...
    try:
        ...
        evidence["ledger"] = [{"sequence": index, "op": op, "attempted": False, "code": "not_attempted", "program_identity": 0}
                              for index, op in enumerate((*intended, "nvrtcDestroyProgram"))]
        raise CompilerFailure(f"nvrtc_load_or_bind:{type(exc).__name__}:{exc}", evidence) from exc
    ...
    try:
        major, minor = C.c_int(), C.c_int(); active="nvrtcVersion"; code = library.nvrtcVersion(C.byref(major), C.byref(minor)); row("nvrtcVersion", code, major=major.value, minor=minor.value); check(code, "nvrtcVersion")
        if (major.value, minor.value) != (13, 3):
            raise RuntimeError(f"nvrtc_version:{major.value}.{minor.value}")
        active="nvrtcCreateProgram"; code = library.nvrtcCreateProgram(C.byref(program), source, b"ph1_nvidia_n4.cu", 0, None, None)
        ...
        active="nvrtcCompileProgram"; code = library.nvrtcCompileProgram(program, len(encoded), option_array); row("nvrtcCompileProgram", code, program_identity=int(program.value))
        compile_code = int(code)
        log_size = C.c_size_t(); active="nvrtcGetProgramLogSize"; log_code = library.nvrtcGetProgramLogSize(program, C.byref(log_size)); row("nvrtcGetProgramLogSize", log_code, program_identity=int(program.value), bytes=log_size.value); check(log_code, "log_size")
        log_buffer = C.create_string_buffer(max(1, log_size.value)); active="nvrtcGetProgramLog"; log_code = library.nvrtcGetProgramLog(program, C.cast(log_buffer, C.c_void_p)); row("nvrtcGetProgramLog", log_code, program_identity=int(program.value), bytes=log_size.value); check(log_code, "log")
        ...
        ptx_size = C.c_size_t(); active="nvrtcGetPTXSize"; code = library.nvrtcGetPTXSize(program, C.byref(ptx_size)); row("nvrtcGetPTXSize", code, program_identity=int(program.value), bytes=ptx_size.value); check(code, "ptx_size")
        if ptx_size.value <= 1:
            raise RuntimeError("empty_ptx")
        ptx_buffer = C.create_string_buffer(ptx_size.value); active="nvrtcGetPTX"; code = library.nvrtcGetPTX(program, C.cast(ptx_buffer, C.c_void_p)); row("nvrtcGetPTX", code, program_identity=int(program.value), bytes=ptx_size.value); check(code, "ptx")
        ...
        cubin_size = C.c_size_t(); active="nvrtcGetCUBINSize"; code = library.nvrtcGetCUBINSize(program, C.byref(cubin_size)); row("nvrtcGetCUBINSize", code, program_identity=int(program.value), bytes=cubin_size.value); check(code, "cubin_size")
        if cubin_size.value == 0:
            raise RuntimeError("empty_cubin")
        cubin_buffer = (C.c_ubyte * cubin_size.value)(); active="nvrtcGetCUBIN"; code = library.nvrtcGetCUBIN(program, C.cast(cubin_buffer, C.c_void_p)); row("nvrtcGetCUBIN", code, program_identity=int(program.value), bytes=cubin_size.value); check(code, "cubin")
        ...
        if active and active not in {item["op"] for item in evidence["ledger"]}:
            evidence["ledger"].append({"sequence":len(evidence["ledger"]),"op":active,"attempted":True,
                                       "code":"ctypes_exception","exception":f"{type(exc).__name__}:{exc}",
                                       "program_identity":int(program.value or 0)})
        failure = exc
    finally:
        attempted = {item["op"] for item in evidence["ledger"]}
        for operation in intended:
            if operation not in attempted:
                evidence["ledger"].append({"sequence": len(evidence["ledger"]), "op": operation,
                                           "attempted": False, "code": "not_attempted",
                                           "program_identity": int(program.value or 0)})
        if program.value:
            identity = int(program.value)
            try:
                active="nvrtcDestroyProgram"; code = library.nvrtcDestroyProgram(C.byref(program)); row("nvrtcDestroyProgram", code, program_identity=identity, pointer_after=int(program.value or 0)); check(code, "nvrtcDestroyProgram")
            except Exception as exc:
                if not any(item["op"]=="nvrtcDestroyProgram" for item in evidence["ledger"]):
                    evidence["ledger"].append({"sequence":len(evidence["ledger"]),"op":"nvrtcDestroyProgram",
                                               "attempted":True,"code":"ctypes_exception",
                                               "exception":f"{type(exc).__name__}:{exc}","program_identity":identity})
                failure = failure or exc
        else:
            evidence["ledger"].append({"sequence": len(evidence["ledger"]), "op": "nvrtcDestroyProgram",
                                       "attempted": False, "code": "not_attempted", "program_identity": 0})
    if failure:
...
class DriverBackend:
    ...
    def _bind(self):
        ...
        def token(value):
            ...
            if name.startswith("LP_"):
                return "POINTER(" + token(value._type_) + ")"
            return {"c_ulong": "c_uint", "c_ulonglong": "c_uint64"}.get(name, name)
        self.abi = {name: {"argtypes": [token(arg) for arg in args], "restype": token(result)}
                    for name, (args, result) in table.items()}

    def _load(self):
        if NVCUDA_DLL.stat().st_size != NVCUDA_BYTES or file_sha(NVCUDA_DLL) != NVCUDA_DLL_SHA:
            raise RuntimeError("nvcuda_identity")
        before = self._cuda_related_modules()
        ...
        after = self._cuda_related_modules()
        forbidden = [row for row in after if "cudart" in row["name"].casefold() or "cupy" in row["path"].casefold()]
        self.module_evidence = {"before": before, "after": after, "forbidden": forbidden,
                                "nvcuda_exact_count": sum(row["sha256"] == NVCUDA_DLL_SHA for row in after)}
        if forbidden or self.module_evidence["nvcuda_exact_count"] != 1:
            raise RuntimeError("runtime_module_surface")
        self.ledger.append({"op": "driver_load", "code": 0, "path": str(loaded.resolve()), "bytes": NVCUDA_BYTES,
                            "sha256": NVCUDA_DLL_SHA, "calling_convention": "WinDLL", "winmode": 0x800,
                            "module_evidence": self.module_evidence})

    @staticmethod
    def _cuda_related_modules():
        import psutil
        rows = []
        seen = set()
        for mapping in psutil.Process().memory_maps(grouped=True):
            path = getattr(mapping, "path", "") or ""
            name = Path(path).name.casefold(); folded = str(path).casefold()
            key = os.path.normcase(path)
            if key not in seen and any(token in folded for token in ("cuda", "nvrtc", "cupy")) and Path(path).is_file():
                seen.add(key)
                rows.append({"name": Path(path).name, "path": str(Path(path).resolve()),
                             "bytes": Path(path).stat().st_size, "sha256": file_sha(Path(path))})
        return sorted(rows, key=lambda row: os.path.normcase(row["path"]))
    ...
    def _sample_device(self, stage):
        ...
        self._ordinary("meminfo", code, stage=stage, free=int(free.value), total=int(total.value),
                       context=int(self.context.value or 0), after_primary_release=self.primary_released)
    ...
    def _launch(self, function_name, label, names, grid, block, pointers):
        ...
        self._ordinary("launch", code, label=label, function=function_name,
                       function_pointer=int(self.functions[function_name].value or 0), grid=list(grid), block=list(block),
                       sharedMemBytes=0, stream=int(self.stream.value), extra=None, argument_names=list(names),
                       argument_values=[int(value.value) for value in variables],
                       parameter_slots=[int(params[index]) for index in range(len(params))]); check(code, f"launch:{label}")
    ...
    def run(self, records, input_bytes: bytes, lut: bytes, cubin: bytes, initial_resources):
        evidence = {"ledger": self.ledger, "context_ledger": self.context_ledger, "resources": self.resources,
                    "outputs": {}, "cleanup_errors": self.cleanup_errors,
                    "static_call_surface": "hash_bound_backend_ast_allowlist",
                    "runtime_modules": {}, "runtime_version": "not_applicable_driver_api_only"}
        ...
        try:
            ...
            try:
                post = self._cuda_related_modules(); self.module_evidence["post_execution"] = post
                self.module_evidence["forbidden"] = [row for row in self.module_evidence.get("after", []) + post
                                                     if "cudart" in row["path"].casefold() or "cupy" in row["path"].casefold()]
            except Exception as exc:
                self.cleanup_errors.append(f"post_runtime_modules:{type(exc).__name__}:{exc}")
        evidence.update({"identity": self.device_identity, "owner_tid": self.owner_tid, "abi": getattr(self, "abi", {}),
                         "loaded_driver": bool(self.lib), "runtime_modules": getattr(self, "module_evidence", {}),
                         "live_owned_resources": len(self.device) + len(self.pinned) + int(bool(self.module.value)) + int(bool(self.stream.value)) + int(self.retained) + int(self.pushed), "primary_released": self.primary_released})
