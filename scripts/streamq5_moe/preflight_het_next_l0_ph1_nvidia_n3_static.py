#!/usr/bin/env python3
"""Closed, CPU-only static PH1 NVIDIA N3 source preflight."""
...
import het_next_l0_ph1_nvidia_n3_transaction as tx

ROOT=Path(__file__).resolve().parents[2];R=ROOT/"reports/streamq5_moe";S=ROOT/"scripts/streamq5_moe";LOCK=R/"het_next_l0_ph1_nvidia_n3_preflight_lock.json";OUT=R/"het_next_l0_ph1_nvidia_n3_static_preflight.json"
FILES=(S/"het_next_l0_ph1_nvidia_n3_common.py",S/"het_next_l0_ph1_nvidia_n3_kernels.cu",S/"het_next_l0_ph1_nvidia_n3_backend.py",S/"het_next_l0_ph1_nvidia_n3_transaction.py",S/"run_het_next_l0_ph1_nvidia_n3.py",S/"verify_het_next_l0_ph1_nvidia_n3.py",Path(__file__))
ACK="ACK-PH1-NVIDIA-N3-STATIC-PREFLIGHT"
ABI_SOURCE={
...
def imports(tree):
 ...
 return out
def hash_lock():
 lock=json.loads(LOCK.read_text());ok=lock.get("kind")=="het_next_l0_ph1_nvidia_n3_preflight_lock"
...
def source_contract(src):
 backend=src["het_next_l0_ph1_nvidia_n3_backend.py"];runner=src["run_het_next_l0_ph1_nvidia_n3.py"];verifier=src["verify_het_next_l0_ph1_nvidia_n3.py"]
 ...
 independent=not ({"het_next_l0_ph1_nvidia_n3_common","het_next_l0_ph1_nvidia_n3_backend","run_het_next_l0_ph1_nvidia_n3","het_next_l0_ph1_nvidia_n3_transaction"}&imports(ast.parse(verifier)))
...
def transaction_simulation():
 with tempfile.TemporaryDirectory(prefix="ph1_nvidia_n3_static_") as td:
...
def verifier_mutations():
 verify=load_inert(S/"verify_het_next_l0_ph1_nvidia_n3.py","ph1_n3_verify_static")
 ...
 with tempfile.TemporaryDirectory(prefix="ph1_n3_verifier_paths_") as td:
  ...
  missing_compile=verify.verify_compile(root/"missing_compile");missing_physical=verify.verify_physical(root/"missing_physical")
...
def cleanup_faults():
 backend=load_inert(S/"het_next_l0_ph1_nvidia_n3_backend.py","ph1_n3_backend_static")
 class Fake:
 ...
 return True
def topology():
 targets=(R/"het_next_l0_ph1_nvidia_n3_compile",R/"het_next_l0_ph1_nvidia_n3_physical",R/"het_next_l0_ph1_nvidia_n3_compile_failures",R/"het_next_l0_ph1_nvidia_n3_physical_failures",R/"het_next_l0_ph1_nvidia_n3_quarantine",OUT)
 return all(not p.exists() for p in targets) and not any(R.glob("het_next_l0_ph1_nvidia_n3*.inprogress*"))
def main():
 ...
 if a.isolated_fixtures:
  result={"cleanup_faults":cleanup_faults(),"verifier_mutations":verifier_mutations()};print(json.dumps(result,separators=(",",":")));return 0 if all(result.values()) else 3
 ...
 isolated=subprocess.run([sys.executable,"-I","-B",str(Path(__file__).resolve()),"--isolated-fixtures"],cwd=str(ROOT),capture_output=True,timeout=120,check=False);isolated_row=json.loads(isolated.stdout) if isolated.returncode==0 else {}
 src=sources();simulation=transaction_simulation();checks={"hash_bindings":hashes,"output_absent":topology(),"kernel_contract":kernel_contract(src["het_next_l0_ph1_nvidia_n3_kernels.cu"]),"source_contract":source_contract(src),"abi_ast_exact":abi_contract(src["het_next_l0_ph1_nvidia_n3_backend.py"]),"abi_mutations":abi_mutations(src["het_next_l0_ph1_nvidia_n3_backend.py"]),"schedule_ast":schedule_contract(src["het_next_l0_ph1_nvidia_n3_backend.py"]),"forbidden_surface":forbidden_surface(src),"transaction_simulation":simulation["pass"],"transaction_details":all(simulation.values()),"isolated_production_fixtures":isolated_row=={"cleanup_faults":True,"verifier_mutations":True},"lock_closed_physical":lock.get("compiler_open") is False and lock.get("physical_open") is False,"audit_binding":lock.get("bindings",{}).get("n2_source_audit",{}).get("sha256")=="c1bbcd0078b3b432e405760e938d4248c2e3a00e2bdb28c1af529a5f5b7feaa8","cardinality_literals":all(x in "\n".join(src.values()) for x in ("2_185_216","675840","post_ordinary_releases_pre_pop","range(30)","len(rows) != 22"))}
 result={"kind":"het_next_l0_ph1_nvidia_n3_static_preflight","checks":checks,"pass":all(checks.values()),"passed":sum(checks.values()),"total":len(checks),"no_payload":True,"no_compiler":True,"no_device":True}
