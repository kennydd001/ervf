...
PREREG = R / "P1C_CORRECTED_ROUTE_CACHE_PREREGISTRATION.md"
LOCK_PATH = R / "p1c_route_input_lock.json"
ROUTE_EVAL_LOCK = R / "p1c_route_evaluator_lock.json"
CACHE_EVAL_LOCK = R / "p1c_cache_evaluator_lock.json"
CAPTURE_PATH = R / "p1c_route_capture_result.json"
VALIDATION_PATH = R / "p1c_cache_validation.json"
TEST_PATH = R / "p1c_cache_test.json"
ROUTE_SCRIPT = ROOT / "scripts/streamq5_moe/capture_p1c_routes.py"
CACHE_SCRIPT = ROOT / "scripts/streamq5_moe/evaluate_p1c_cache.py"
...
def main():
    ...
    add("corrected physical semantics", capture.get("scale_semantics") == "codes selected with FP32 maxabs scale; scale rounded to BF16 before dequantization; output rounded BF16" and "stored_scale = scale.to(torch.bfloat16).float()" in ROUTE_SCRIPT.read_text(encoding="utf-8"))
    ...
    add("split chronology", validation["status"] == "p1c_validation_pass_test_authorized" and test["status"] == "p1c_cache_pass")
    add("phase flags", validation["p1c_pass"] is False and test["p1c_pass"] is True and test["next_phase_authorized"] is True)

    passed = sum(row["pass"] for row in checks); total = len(checks)
    status = "p1c_cache_verification_pass" if passed == total else "p1c_cache_verification_fail"
    payload = {
        "kind": "streamq5_moe_p1c_independent_verification", "status": status,
    ...
    (R / "p1c_cache_verification.json").write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    (R / "P1C_CACHE_VERIFICATION.md").write_text(f"# STREAMQ5-MoE P1C - onafhankelijke corrected-semantics verificatie\n\nUitkomst: **{status}** ({passed}/{total}). Test mean/p95: {payload['test_mean_h2d_ms']:.3f}/{payload['test_p95_h2d_ms']:.3f} ms/token.\n", encoding="utf-8")
